...
class TestProfileRegistry:
    ...
    def test_brainstorm_v1_exists(self):
        ...
        assert "IDEAS" in p.output_contract.required_sections

    def test_draft_and_edit_v1_exists(self):
        p = get_profile("draft_and_edit_v1")
        assert p is not None
        assert len(p.stages) == 3
        assert "DRAFT" in p.output_contract.required_sections
    ...
    def test_output_contract_has_required_fields(self):
        ...
        assert len(c.required_sections) >= 2
...
class TestOutputValidation:
    ...
    def test_valid_output(self):
        text = (
            ...
            "MESSAGE:\nLet's have a cozy night in with pasta and a movie!"
        )
...
class TestChecklistHeuristics:
    ...
    def test_task_planner_default_checklist(self):
        ...
        assert cl == {"food": False, "activity": False, "message": False, "budget": False}

    def test_brainstorm_default_checklist(self):
        cl = _default_checklist("brainstorm_v1")
        assert cl == {"ideas": False, "evaluation": False, "recommendation": False}

    def test_draft_and_edit_default_checklist(self):
        cl = _default_checklist("draft_and_edit_v1")
        assert cl == {"outline": False, "draft": False, "notes": False}

    def test_task_planner_food_detected(self):
        cl = _default_checklist("task_planner_v1")
        sections = {"PLAN": "1) Cook dinner at home with pasta", "BUDGET": "Total: €15", "MESSAGE": "Enjoy!"}
        cl = _update_checklist(cl, sections, "task_planner_v1")
        assert cl["food"] is True
        assert cl["budget"] is True
        assert cl["message"] is True

    def test_task_planner_activity_detected(self):
        cl = _default_checklist("task_planner_v1")
        sections = {"PLAN": "Go to a movie theater after dinner", "BUDGET": "", "MESSAGE": ""}
        cl = _update_checklist(cl, sections, "task_planner_v1")
        assert cl["activity"] is True

    def test_brainstorm_checklist_update(self):
        cl = _default_checklist("brainstorm_v1")
        sections = {"IDEAS": "1) App idea", "EVALUATION": "Feasible", "RECOMMENDATION": "Go for it"}
        cl = _update_checklist(cl, sections, "brainstorm_v1")
        assert _is_checklist_complete(cl) is True

    def test_checklist_incomplete_when_missing(self):
        cl = {"food": True, "activity": False, "message": True, "budget": True}
        assert _is_checklist_complete(cl) is False

    def test_checklist_complete_when_all_true(self):
        cl = {"food": True, "activity": True, "message": True, "budget": True}
        assert _is_checklist_complete(cl) is True
...
class TestSafeFallback:
    ...
    def test_task_planner_fallback_has_sections(self):
        fb = _safe_fallback("task_planner_v1")
        assert "PLAN:" in fb
        assert "BUDGET:" in fb
        assert "MESSAGE:" in fb

    def test_brainstorm_fallback_has_sections(self):
        fb = _safe_fallback("brainstorm_v1")
        assert "IDEAS:" in fb
...
class TestRunCrewTurnIntegration:
    ...
    @pytest.mark.asyncio
    async def test_crew_turn_invalid_output_retries(self):
        """If first output is invalid, engine retries once."""
        room, proj_map, participants = self._std_fixtures()
        call_count = 0

        async def mock_llm_fn(messages, **kwargs):
            nonlocal call_count
            call_count += 1
            if call_count == 1:
                return "This is not structured output at all."
            return self._valid_llm_output()

        patches, _ = self._patches(room, proj_map, participants, llm_side_effect=mock_llm_fn)
    ...
    @pytest.mark.asyncio
    async def test_crew_turn_fallback_on_double_failure(self):
        """If both attempts fail validation, safe fallback is used."""
        room, proj_map, participants = self._std_fixtures()
        patches, _ = self._patches(
            room, proj_map, participants,
            llm_return="Totally unstructured gibberish output.",
        )
        ...
        assert "PLAN:" in msg_content
        assert "Passing this turn" in msg_content
