...
def model(modules, monkeypatch):
    ...
    async def call_model(
        messages, *, temperature=0.2, model="", provider="", base_url=""
    ):
        calls.append({
            "messages": messages,
            "model": model,
...
class TestNotesOverTheSocket:
    ...
    def test_the_setup_summary_model_also_drives_the_rolling_recap(
        self, client, enabled, stt, model, modules
    ):
        with client.websocket_connect("/v1/meetingsense/session") as ws:
            ws.send_json({
                "type": "start",
        ...
        assert model
        assert all(call["provider"] == "ollama" for call in model)
        assert all(call["model"] == "qwen2.5:7b" for call in model)
        assert all(call["base_url"] == "http://localhost:11434" for call in model)
