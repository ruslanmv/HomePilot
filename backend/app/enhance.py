...
- RealESRGAN: Photo enhancement with natural texture recovery
- SwinIR: Artifact removal and restoration
...
class EnhanceMode(str, Enum):
    ...
    PHOTO = "photo"       # RealESRGAN - natural photo enhancement
    RESTORE = "restore"   # SwinIR - artifact/compression removal
    FACES = "faces"       # GFPGAN - face restoration


# Model mapping for each mode
MODE_MODELS = {
    EnhanceMode.PHOTO: "RealESRGAN_x4plus.pth",
    EnhanceMode.RESTORE: "SwinIR_4x.pth",
    EnhanceMode.FACES: "GFPGANv1.4.pth",
}

# Workflow mapping for each mode
MODE_WORKFLOWS = {
    EnhanceMode.PHOTO: "enhance_realesrgan",
    EnhanceMode.RESTORE: "restore_swinir",
    EnhanceMode.FACES: "fix_faces_gfpgan",
}
...
async def enhance_image(req: EnhanceRequest):
    ...
    # Select workflow and model
    workflow_name = MODE_WORKFLOWS[req.mode]
    model_name = MODE_MODELS[req.mode]

    print(f"[ENHANCE] Using workflow={workflow_name}, model={model_name}")
    ...
    try:
        ...
        if req.face_enhance and req.mode != EnhanceMode.FACES:
            ...
            if result.get("media", {}).get("images"):
                enhanced_url = result["media"]["images"][0]
                ...
                face_result = run_workflow("fix_faces_gfpgan", {
                    ...
                    "model_name": MODE_MODELS[EnhanceMode.FACES],
                    "width": out_w,
                    "height": out_h,
