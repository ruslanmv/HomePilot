...
from .lora_loader import scan_installed_loras, get_lora_dir
...
def _download_lora_file(lora_id: str, name: str, url: str, dest: Path, timeout: int = 600) -> dict:
...
def _run_lora_download(lora_id: str, name: str, url: str, dest: Path):
    """Background thread target: download one LoRA, then update shared state."""
    global _lora_download_state

    result = _download_lora_file(lora_id, name, url, dest)
...
async def install_lora(lora_id: str):
    ...
    thread = threading.Thread(
        ...
        args=(lora_id, entry.name, entry.download_url, dest),
        daemon=True,
