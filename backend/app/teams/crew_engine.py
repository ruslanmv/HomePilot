...
def _validate_output(
    *,
    ...
    """Validate LLM output against the profile's output contract.
    ...
    if wc > int(contract.max_words):
        return False, f"too_long({wc}>{contract.max_words})", {"word_count": wc}

    sections = _extract_sections(text)
    required = [s.upper() for s in contract.required_sections]
    ...
    # Profile-specific checks: MESSAGE section should be concise
    msg_section = sections.get("MESSAGE", "")
    if msg_section and _word_count(msg_section) > 50:
        return (
            False,
            "message_too_long(>50_words)",
            {"message_words": _word_count(msg_section), "sections": sections},
        )

    # Budget limit check
    if budget_limit is not None and "BUDGET" in sections:
        m = re.search(
            ...
            try:
                ...
                if total > float(budget_limit) + 1e-6:
                    return (
                        False,
                        "budget_over_limit",
                        {"total": total, "limit": budget_limit, "sections": sections},
                    )
            except ValueError:
                pass
...
def _update_checklist(
    ...
    """Update checklist based on parsed sections (best-effort heuristic)."""
    if profile_id == "task_planner_v1":
        plan_txt = (sections.get("PLAN") or "").lower()
        if any(x in plan_txt for x in [
            "dinner", "meal", "cook", "food", "restaurant", "recipe",
            "eat", "lunch", "snack", "takeout",
        ]):
            checklist["food"] = True
        if any(x in plan_txt for x in [
            "movie", "game", "activity", "walk", "playlist", "hike",
            "concert", "museum", "picnic", "show", "dance",
        ]):
            checklist["activity"] = True
        if sections.get("BUDGET"):
            checklist["budget"] = True
        if sections.get("MESSAGE"):
            checklist["message"] = True
    elif profile_id == "brainstorm_v1":
        if sections.get("IDEAS"):
            checklist["ideas"] = True
        if sections.get("EVALUATION"):
            checklist["evaluation"] = True
        if sections.get("RECOMMENDATION"):
            checklist["recommendation"] = True
    elif profile_id == "draft_and_edit_v1":
        if sections.get("OUTLINE"):
            checklist["outline"] = True
        if sections.get("DRAFT"):
            checklist["draft"] = True
...
def _default_checklist(profile_id: str) -> Dict[str, bool]:
    """Create default checklist for a profile."""
    if profile_id == "task_planner_v1":
        return {"food": False, "activity": False, "message": False, "budget": False}
    elif profile_id == "brainstorm_v1":
        return {"ideas": False, "evaluation": False, "recommendation": False}
    elif profile_id == "draft_and_edit_v1":
        return {"outline": False, "draft": False, "notes": False}
    return {}
...
_FALLBACK_OUTPUTS = {
    "task_planner_v1": (
        "PLAN:\n1) No new information to add at this stage.\n\n"
        "BUDGET:\n- Total: €0\n\n"
        "MESSAGE:\nPassing this turn."
    ),
    "brainstorm_v1": (
        "IDEAS:\n1) No new ideas to add.\n\n"
        "EVALUATION:\nN/A\n\n"
        "RECOMMENDATION:\nPassing this turn."
    ),
    "draft_and_edit_v1": (
...
def _safe_fallback(profile_id: str) -> str:
    return _FALLBACK_OUTPUTS.get(
        profile_id,
        "PLAN:\nPassing this turn.\n\nBUDGET:\nN/A\n\nMESSAGE:\nNo update.",
    )
...
async def run_crew_turn(
    *,
    ...
    task_anchor = (
        ...
        + f"WORKFLOW STAGE: {stage.title} ({stage.id})\n"
        + f"CHECKLIST: {checklist}\n"
        + "CURRENT DRAFT SUMMARY:\n"
        + (draft.get("plan") or draft.get("outline") or draft.get("ideas") or "(empty)")
    ).strip()
    ...
    contract_text = (
        ...
        f"- Max words: {contract.max_words}\n"
        f"- Required sections: {', '.join(contract.required_sections)}\n"
        "Write EXACTLY these sections, each as 'SECTION_NAME:' on its own line.\n"
        "Do not roleplay with other agents. Do not write dialogue. Do not use cliches.\n"
    )
    ...
    # Retry once with corrective hint if invalid
    if not ok:
        corrective = (
            stage_instruction
            + "\n\nCORRECTION REQUIRED:\n"
            + f"- Your last output failed validation: {reason}\n"
            + "- Produce a corrected output that follows the contract strictly.\n"
        )
        chat_messages2 = list(chat_messages[:-1]) + [{"role": "user", "content": corrective}]
        text = (await llm_text(
            chat_messages2,
            provider=provider,
            model=model,
    ...
    # If still invalid after retry, emit safe fallback
    if not ok:
        text = _safe_fallback(profile_id)
        no_progress += 1
        progress["no_progress_count"] = no_progress
    ...
    if isinstance(sections, dict):
        checklist = _update_checklist(checklist, sections, profile_id)
        # Store key draft sections
        for key in ("PLAN", "OUTLINE", "IDEAS", "DRAFT"):
            if key in sections and sections[key].strip():
                draft[key.lower()] = sections[key].strip()
        for key in ("MESSAGE", "RECOMMENDATION"):
            if key in sections and sections[key].strip():
                draft[key.lower()] = sections[key].strip()
