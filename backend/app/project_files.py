...
def delete_item(item_id: str, *, cleanup_file: bool = True) -> bool:
    """Delete an item and optionally its backing file asset + physical file.

    When *cleanup_file* is True (the default) the linked file_assets row
    and the file on disk are also removed so nothing is orphaned.
    """
    ensure_project_items_table()
    con = _db()
    cur = con.cursor()
    ...
    asset_id = ""
    if cleanup_file:
        cur.execute("SELECT asset_id FROM project_items WHERE id = ?", (item_id,))
        row = cur.fetchone()
        if row:
    ...
    # Clean up the backing file asset + physical file on disk
    if removed and asset_id:
        try:
            from .files import delete_asset_and_file
            delete_asset_and_file(asset_id)
        except Exception as e:
            print(f"[ITEMS] Failed to clean up asset {asset_id}: {e}")
