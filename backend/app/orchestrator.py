...
async def _persist_media(
    ...
    """
    If media contains ComfyUI image URLs, download and persist them as
...
async def orchestrate(
    ...
    # Also strip the markdown image tags from text to prevent double-rendering
    # (once via MessageMarkdown, once via the media gallery)
    text_media = None
    try:
...
async def handle_request(mode: Optional[str], payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    log_event(
        ...
        provider_model=payload.get("provider_model") or payload.get("ollama_model") or payload.get("llm_model"),
    )
