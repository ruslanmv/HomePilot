...
async def list_models(
    ...
    try:
        ...
        if err:
            # For Ollama/chat providers, return 200 with empty list + warning
            # instead of 503 — the provider may simply not be running yet
                    ...
                    try:
                        ...
                        if _target_outfit:
                            ...
                            if isinstance(_vp, dict) and any(_vp.get(a) for a in ("front", "left", "right", "back")):
                                ...
                                # If view_pack has no "front", use the outfit's
                                # main image (first image) as the front view.
                                if "front" not in _available:
                                    _outfit_imgs = _target_outfit.get("images") or []
                                    if _outfit_imgs:
                                        _front_candidate = _outfit_imgs[0].get("url", "")
                                        if _front_candidate:
                                            _available["front"] = _front_candidate
