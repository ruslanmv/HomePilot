export function estimateComplexity(messages) {
    const lastUserMessage = [...messages].reverse().find((m) => m.role === "user")?.content ?? "";
    let score = 0;
    if (lastUserMessage.length > 250)
        score += 2;
    if (lastUserMessage.length > 800)
        score += 2;
    ...
    if ((lastUserMessage.match(/\?/g) || []).length >= 2)
        score += 1;
...
export function resolveRoutingPolicy(input) {
    const { mode, messages, hasAttachments = false, remoteAvailable = false, localAvailable = true, monthlyGpuBudgetReached = false, providerStats, } = input;
    ...
    const preferLocalByReliability = Boolean(providerStats &&
        providerStats.remoteSuccessRate < 0.7 &&
        providerStats.localSuccessRate >= providerStats.remoteSuccessRate);
    const preferRemoteByReliability = Boolean(providerStats &&
    ...
    if (mode === "expert") {
        const useRemote = remoteAvailable &&
    ...
    if (mode === "heavy") {
        return {
            ...
            complexityScore,
            reason: remoteAvailable && !monthlyGpuBudgetReached
