              ...
              {modeMenuOpen && modeMenuPos && typeof document !== 'undefined'
                ...
                >
                  {CHAT_REASONING_OPTIONS.map((opt) => {
                    ...
                    const personaUnavailable = opt.id === 'persona' && !allowPersonaMode
                    const Icon = opt.icon
                    return (
                      <button
                        key={opt.id}
    ...
    try {
      ...
      if (!cache.find((p: any) => p.id === currentProject.id)) {
        cache.push({
          ...
          persona_class: (currentProject.persona_agent as any)?.persona_class || 'custom',
          goal: (currentProject as any).agentic?.goal || '',
          style_preset: '',
          character_desc: '',
          created_at: 0,
          photos: [],
          ...
          try {
            ...
            if (cached) {
              const personas = JSON.parse(cached) as Array<{
                ...
                persona_class?: string; goal?: string
    ...
    <div className="hp-app-shell flex bg-black text-white font-sans selection:bg-white/20 overflow-hidden relative">
      ...
      {saveAsPersonaData && (
        <SaveAsPersonaModal
          ...
          batchSiblings={saveAsPersonaData.batchSiblings}
          backendUrl={settingsDraft.backendUrl}
          apiKey={settingsDraft.apiKey}
