...
async function postJson(baseUrl, path, body, apiKey) {
    const url = `${cleanBase(baseUrl)}${path.startsWith('/') ? path : `/${path}`}`;
    const res = await fetch(url, {
        method: 'POST',
        headers: {
...
export default function ModelsView(props) {
    ...
    const [provider, setProvider] = useState(props.providerChat || 'ollama');
    const defaultBaseUrl = useMemo(() => {
        if (modelType === 'chat')
            return props.baseUrlChat || '';
        if (modelType === 'multimodal')
            return props.baseUrlChat || '';
        if (modelType === 'image')
            return props.baseUrlImages || '';
        if (modelType === 'edit')
            return props.baseUrlImages || '';
        if (modelType === 'enhance')
            return props.baseUrlImages || '';
        if (modelType === 'addons')
            return props.baseUrlImages || '';
        if (modelType === 'lora')
    ...
    const [installBusy, setInstallBusy] = useState(null);
    ...
    const [civitaiResults, setCivitaiResults] = useState([]);
    const [civitaiSearchLoading, setCivitaiSearchLoading] = useState(false);
    const [civitaiSearchError, setCivitaiSearchError] = useState(null);
    ...
    const [apiKeyInput, setApiKeyInput] = useState({ huggingface: '', civitai: '' });
    const [apiKeyTesting, setApiKeyTesting] = useState(null);
    ...
    const [avatarModelsError, setAvatarModelsError] = useState(null);
    const [avatarDownloadBusy, setAvatarDownloadBusy] = useState(null); // 'basic' | 'full' | null
    const [avatarDownloadStatus, setAvatarDownloadStatus] = useState(null);
    const [avatarDeleteConfirm, setAvatarDeleteConfirm] = useState(null);
    const [avatarDeleteBusy, setAvatarDeleteBusy] = useState(null);
    const [avatarInstallBusy, setAvatarInstallBusy] = useState(null); // model ID being installed
    // LoRA registry state (additive — Golden Rule 1.0)
    const [loraRegistry, setLoraRegistry] = useState([]);
    const [loraInstalled, setLoraInstalled] = useState([]);
    const [loraLoading, setLoraLoading] = useState(false);
    const [loraError, setLoraError] = useState(null);
    const [loraInstallBusy, setLoraInstallBusy] = useState(null);
    const [loraDownloadBusy, setLoraDownloadBusy] = useState(null);
    const [loraDownloadStatus, setLoraDownloadStatus] = useState(null);
    const [loraDeleteConfirm, setLoraDeleteConfirm] = useState(null);
    const [loraDeleteBusy, setLoraDeleteBusy] = useState(null);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const res = await getJson(backendUrl, '/providers', authKey);
                if (!mounted)
                    return;
                const providerList = Object.entries(res.providers || {}).map(([name, info]) => ({
                    name,
    ...
    const refreshCatalog = async () => {
        ...
        try {
            const data = await getJson(backendUrl, '/model-catalog', authKey);
            setCatalog(data);
        }
        catch (e) {
    ...
    const refreshInstalled = async () => {
        ...
        try {
            ...
            if (baseUrl.trim())
                q.set('base_url', baseUrl.trim());
            // Pass model_type so backend returns correct installed models for this type
            if (modelType && provider === 'comfyui')
                q.set('model_type', modelType);
            // For Ollama multimodal, filter to vision-capable models only
            if (modelType === 'multimodal' && provider === 'ollama')
                q.set('model_type', 'multimodal');
            const data = await getJson(backendUrl, `/models?${q.toString()}`, authKey);
            setInstalled(Array.isArray(data.models) ? data.models : []);
            if (data.error)
                setInstalledError(String(data.error));
        }
        catch (e) {
            setInstalled([]);
    ...
    const supportedForSelection = useMemo(() => {
        let list = [];
    ...
    const isInstalled = useCallback((id) => {
        ...
        if (!id.includes(':') && installedSet.has(`${id}:latest`))
            return true;
    ...
    const merged = useMemo(() => {
        ...
        const isCheckpointFile = (id) => /\.(safetensors|ckpt|pth|onnx|gguf|pt|bin)$/i.test(id);
        const isLlmModel = (id) => id.includes(':') || /^(llama|mistral|gemma|qwen|phi|deepseek|codellama|vicuna|samantha)/i.test(id);
        ...
        for (const m of installed) {
            if (!matchesSupportedEntry(m)) {
                ...
                if (otherTypeIds.has(m))
                    continue;
                // Skip checkpoint files on the chat tab
                if (modelType === 'chat' && isCheckpointFile(m))
                    continue;
                // Skip LLM models on ComfyUI tabs (image/video/edit/enhance)
                if (modelType !== 'chat' && provider === 'comfyui' && isLlmModel(m))
                    continue;
                rows.push({
                    id: m,
                    label: m,
    ...
    const tryInstall = async (modelId, install) => {
        ...
        try {
            ...
            if (provider === 'civitai') {
                ...
                body.civitai_version_id = civitaiVersionId.trim();
                setToast(`Starting Civitai download for version ${civitaiVersionId}...`);
            }
            else {
                setToast(`Starting download for ${modelId}...`);
            }
            const res = await postJson(backendUrl, '/models/install', body, authKey);
            if (res?.ok) {
    ...
    const tryDelete = async (modelId) => {
        ...
        try {
            ...
            const res = await postJson(backendUrl, '/models/delete', body, authKey);
            if (res?.ok) {
    ...
    const installFromCivitaiResult = async (model, versionId) => {
    ...
    const loadApiKeysStatus = async () => {
        try {
            const data = await getJson(backendUrl, '/settings/api-keys', authKey);
            if (data.keys) {
    ...
    const saveApiKey = async (provider) => {
        ...
        try {
            const res = await postJson(backendUrl, '/settings/api-keys', { provider, key }, authKey);
            if (res.ok) {
    ...
    const testApiKey = async (provider) => {
        ...
        try {
            ...
            const res = await postJson(backendUrl, '/settings/api-keys/test', { provider, key: keyToTest }, authKey);
            if (res.valid) {
    ...
    const refreshAvatarModels = useCallback(async () => {
        ...
        try {
            const data = await getJson(backendUrl, '/v1/avatar-models', authKey);
    ...
    const refreshLoraModels = useCallback(async () => {
        ...
        try {
            const [registryRes, installedRes] = await Promise.all([
                getJson(backendUrl, `/v1/lora/registry?spicy=${props.nsfwMode ? 'true' : 'false'}`, authKey),
                getJson(backendUrl, '/v1/lora/installed', authKey),
            ]);
    ...
    const installLora = async (loraId) => {
        setLoraInstallBusy(loraId);
        const entry = loraRegistry.find((l) => l.id === loraId);
    ...
    const downloadAvatarPreset = async (preset) => {
    ...
    const deleteAvatarModel = async (modelId) => {
    ...
    const installSingleAvatarModel = async (modelId) => {
    ...
    return (<div className="h-full w-full bg-black text-white overflow-hidden flex flex-col">
      {/* Header */}
      <div className="px-8 py-6 border-b border-white/10 flex items-center justify-between bg-gradient-to-b from-white/[0.02] to-transparent">
        ...
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => refreshCatalog()} disabled={catalogLoading} className="px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-semibold flex items-center gap-2.5 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
            <RefreshCw size={15} className={catalogLoading ? 'animate-spin' : ''}/>
            Refresh Catalog
          </button>

          <button type="button" onClick={() => refreshInstalled()} disabled={installedLoading} className="px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-semibold flex items-center gap-2.5 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
            <RefreshCw size={15} className={installedLoading ? 'animate-spin' : ''}/>
            Refresh Installed
          </button>

          {/* API Keys Settings Button */}
          <button type="button" onClick={() => setApiKeysExpanded(true)} className="p-2.5 rounded-xl bg-transparent hover:bg-white/5 border border-transparent hover:border-white/10 text-white/40 hover:text-white/70 transition-all" title="API Keys (for gated models)">
            <Key size={16}/>
          </button>
        </div>
      </div>
      ...
      {/* API Keys Modal */}
      {apiKeysExpanded && (<div className="fixed inset-0 z-50 flex items-center justify-center">
          {/* Backdrop */}
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => setApiKeysExpanded(false)}/>
          ...
          <div className="relative bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-white/[0.02]">
              ...
              <button onClick={() => setApiKeysExpanded(false)} className="p-2 rounded-lg hover:bg-white/5 text-white/40 hover:text-white/70 transition-all">
                <X size={18}/>
              </button>
            </div>
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {apiKeysStatus.huggingface?.configured && (<span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${apiKeysStatus.huggingface.source === 'environment'
                ...
                {apiKeysStatus.huggingface?.configured ? (<div className="space-y-2">
                    <div className="flex items-center gap-2 bg-white/5 rounded-lg px-3 py-2">
                      <span className="text-emerald-400 text-sm">✓</span>
                      <span className="text-xs text-white/60 font-mono">{apiKeysStatus.huggingface.masked}</span>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => testApiKey('huggingface')} disabled={apiKeyTesting === 'huggingface'} className="flex-1 px-3 py-2 text-xs font-semibold rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 transition-all disabled:opacity-50">
                        {apiKeyTesting === 'huggingface' ? 'Testing...' : 'Test'}
                      </button>
                      {apiKeysStatus.huggingface.source !== 'environment' && (<button onClick={() => deleteApiKey('huggingface')} className="px-3 py-2 text-xs font-semibold rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-300 border border-red-500/20 transition-all">
                          Remove
                        </button>)}
                    </div>
                  </div>) : (<div className="space-y-2">
                    <input type="password" value={apiKeyInput.huggingface} onChange={(e) => setApiKeyInput(prev => ({ ...prev, huggingface: e.target.value }))} placeholder="hf_xxxxxxxxxxxxxxxxxx" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm font-mono outline-none focus:border-white/30 transition-all placeholder:text-white/20"/>
                    <div className="flex gap-2">
                      <button onClick={() => testApiKey('huggingface')} disabled={!apiKeyInput.huggingface.trim() || apiKeyTesting === 'huggingface'} className="flex-1 px-3 py-2 text-xs font-semibold rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 transition-all disabled:opacity-50">
                        {apiKeyTesting === 'huggingface' ? 'Testing...' : 'Test'}
                      </button>
                      <button onClick={() => saveApiKey('huggingface')} disabled={!apiKeyInput.huggingface.trim() || apiKeySaving === 'huggingface'} className="flex-1 px-3 py-2 text-xs font-semibold rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white transition-all disabled:opacity-50">
                        {apiKeySaving === 'huggingface' ? 'Saving...' : 'Save'}
                      </button>
                    </div>
                    <a href="https://huggingface.co/settings/tokens" target="_blank" rel="noopener noreferrer" className="text-[10px] text-blue-400 hover:text-blue-300">
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {apiKeysStatus.civitai?.configured && (<span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${apiKeysStatus.civitai.source === 'environment'
                    ? 'bg-blue-500/20 text-blue-300'
                    : 'bg-emerald-500/20 text-emerald-300'}`}>
                ...
                {apiKeysStatus.civitai?.configured ? (<div className="space-y-2">
                    <div className="flex items-center gap-2 bg-white/5 rounded-lg px-3 py-2">
                      <span className="text-emerald-400 text-sm">✓</span>
                      <span className="text-xs text-white/60 font-mono">{apiKeysStatus.civitai.masked}</span>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => testApiKey('civitai')} disabled={apiKeyTesting === 'civitai'} className="flex-1 px-3 py-2 text-xs font-semibold rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 transition-all disabled:opacity-50">
                        {apiKeyTesting === 'civitai' ? 'Testing...' : 'Test'}
                      </button>
                      {apiKeysStatus.civitai.source !== 'environment' && (<button onClick={() => deleteApiKey('civitai')} className="px-3 py-2 text-xs font-semibold rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-300 border border-red-500/20 transition-all">
                          Remove
                        </button>)}
                    </div>
                  </div>) : (<div className="space-y-2">
                    <input type="password" value={apiKeyInput.civitai} onChange={(e) => setApiKeyInput(prev => ({ ...prev, civitai: e.target.value }))} placeholder="xxxxxxxxxxxxxxxxxxxxxxxx" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm font-mono outline-none focus:border-white/30 transition-all placeholder:text-white/20"/>
                    <div className="flex gap-2">
                      <button onClick={() => testApiKey('civitai')} disabled={!apiKeyInput.civitai.trim() || apiKeyTesting === 'civitai'} className="flex-1 px-3 py-2 text-xs font-semibold rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 transition-all disabled:opacity-50">
                        {apiKeyTesting === 'civitai' ? 'Testing...' : 'Test'}
                      </button>
                      <button onClick={() => saveApiKey('civitai')} disabled={!apiKeyInput.civitai.trim() || apiKeySaving === 'civitai'} className="flex-1 px-3 py-2 text-xs font-semibold rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white transition-all disabled:opacity-50">
                        {apiKeySaving === 'civitai' ? 'Saving...' : 'Save'}
                      </button>
                    </div>
                    <a href="https://civitai.com/user/account" target="_blank" rel="noopener noreferrer" className="text-[10px] text-blue-400 hover:text-blue-300">
      ...
      <div className="px-8 py-5 border-b border-white/10 bg-white/[0.01]">
        <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr_1fr] gap-4 max-w-7xl">
          <div>
            ...
            <div className="flex flex-col gap-1.5">
              {['chat', 'multimodal', 'image', 'edit', 'video', 'enhance', 'lora', 'addons'].map((t) => (<button key={t} type="button" onClick={() => setModelType(t)} className={`px-4 py-2.5 rounded-lg border text-xs font-bold uppercase tracking-wide transition-all ${modelType === t
          ...
          <div>
            ...
            <select value={provider} onChange={(e) => setProvider(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm font-medium outline-none focus:border-white/30 focus:bg-white/10 transition-all">
              {availableProviders.length > 0 ? (availableProviders.map((p) => (<option key={p.name} value={p.name}>
                    {p.label || p.name}
                  </option>))) : (
        ...
        modelType === 'chat' ? (<>
          ...
          <div>
            ...
            <input value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder={modelType === 'chat' ? 'http://localhost:11434' : 'http://localhost:8188'} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm font-medium outline-none focus:border-white/30 focus:bg-white/10 transition-all placeholder:text-white/30"/>
      ...
      {provider === 'civitai' && (<div className="px-8 py-4 border-b border-white/10 bg-gradient-to-br from-blue-500/5 to-blue-500/0">
          <div className="max-w-7xl">
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                civitaiSearchLoading
                  ...
                  {civitaiSearchLoading ? (<svg className="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>) : ('Search')}
                </button>
              </div>
              {civitaiSearchError && (<div className="mt-2 text-[10px] text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">
                  ⚠️ {civitaiSearchError}
                </div>)}
            </div>

            {/* Search Results */}
            {civitaiResults.length > 0 && (<div className="mb-6">
                <div className="flex items-center justify-between mb-3">
                  <div className="text-[10px] text-cyan-400 font-bold uppercase tracking-wider">
                    Search Results ({civitaiResults.length} models)
                  </div>
                  {civitaiTotalPages > 1 && (<div className="flex items-center gap-2">
                      <button onClick={() => searchCivitai(civitaiPage - 1)} disabled={civitaiPage <= 1 || civitaiSearchLoading} className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-white/5 hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all">
                        ← Prev
                      </button>
                      <span className="text-xs text-white/50">
                        Page {civitaiPage} of {civitaiTotalPages}
                      </span>
                      <button onClick={() => searchCivitai(civitaiPage + 1)} disabled={civitaiPage >= civitaiTotalPages || civitaiSearchLoading} className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-white/5 hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all">
                        Next →
                      </button>
                    </div>)}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide">
                  {civitaiResults.map((model) => (<div key={model.id} className="bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-cyan-500/30 transition-all group">
                      {/* Thumbnail */}
                      <div className="relative h-32 bg-black/50">
                        {model.thumbnail ? (<img src={model.thumbnail} alt={model.name} className="w-full h-full object-cover" loading="lazy"/>) : (<div className="w-full h-full flex items-center justify-center text-white/20">
                            <span className="text-4xl">🖼️</span>
                          </div>)}
                        {model.nsfw && (<span className="absolute top-2 right-2 px-2 py-1 text-[9px] font-bold uppercase bg-red-600 text-white rounded">
                            NSFW
                          </span>)}
                      </div>
                      ...
                      <div className="p-3">
                        ...
                        {model.tags.length > 0 && (<div className="flex flex-wrap gap-1 mb-3">
                            {model.tags.slice(0, 3).map((tag) => (<span key={tag} className="px-2 py-0.5 text-[9px] bg-white/5 rounded text-white/40">
                                {tag}
                              </span>))}
                          </div>)}

                        {/* Version selector and install */}
                        {model.versions.length > 0 && (<div className="flex items-center gap-2">
                            <select className="flex-1 bg-white/5 border border-white/10 rounded-lg px-2 py-1.5 text-xs outline-none focus:border-cyan-500/30" defaultValue={model.versions[0]?.id} id={`version-${model.id}`}>
                              {model.versions.map((v) => (<option key={v.id} value={v.id}>
                                  {v.name} ({(v.sizeKB / 1024 / 1024).toFixed(1)}GB)
                                </option>))}
                            </select>
                            <button type="button" onClick={() => {
                            const select = document.getElementById(`version-${model.id}`);
                            const versionId = select?.value || model.versions[0]?.id;
                            installFromCivitaiResult(model, versionId);
                        }} disabled={installBusy === model.id} className={[
                            ...
                            installBusy === model.id
                              ...
                              {installBusy === model.id ? (<svg className="animate-spin h-3 w-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>) : (<Download size={12}/>)}
                              Install
                            </button>
                          </div>)}

                        {model.link && (<a href={model.link} target="_blank" rel="noopener noreferrer" className="block mt-2 text-[10px] text-cyan-400 hover:text-cyan-300">
                            View on Civitai →
                          </a>)}
                      </div>
                    </div>))}
            ...
            <div className="flex items-start gap-4 mb-6 border-t border-white/10 pt-4">
              <div className="flex-1">
                ...
                <input value={civitaiVersionId} onChange={(e) => setCivitaiVersionId(e.target.value)} placeholder="e.g., 128713 (from Civitai model URL)" className="w-full bg-white/5 border border-blue-500/20 rounded-xl px-4 py-3 text-sm font-medium outline-none focus:border-blue-500/40 focus:bg-white/10 transition-all placeholder:text-white/30"/>
              ...
              <div className="flex-shrink-0 pt-7">
            ...
            }} disabled={!civitaiVersionId.trim() || installBusy !== null} className={[
                ...
                installBusy !== null
                  ...
                  {installBusy !== null && (<span className="absolute inset-0 bg-gradient-to-r from-blue-700 via-blue-600 to-blue-700 animate-shimmer" style={{
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6 scrollbar-hide">
        <div className="flex flex-col gap-4 max-w-7xl mx-auto">
          ...
          {installedError && provider !== 'civitai' ? (<div className={`rounded-xl border p-5 ${installedError.includes('Ollama') || installedError.includes('11434')
                ? 'border-white/10 bg-white/[0.03] text-white/60'
                : 'border-amber-500/30 bg-gradient-to-br from-amber-500/10 to-amber-500/5 text-amber-200'}`}>
              <div className={`font-bold text-sm ${installedError.includes('Ollama') || installedError.includes('11434')
                ? 'text-white/70'
                : 'text-amber-100'}`}>
          ...
          {catalogError && !supportedForSelection.length ? (<div className="rounded-xl border border-blue-500/20 bg-gradient-to-br from-blue-500/10 to-blue-500/5 p-5 text-blue-200">
          ...
          <div className="rounded-2xl border border-white/10 overflow-hidden bg-gradient-to-b from-white/[0.02] to-transparent">
            ...
            <div className="divide-y divide-white/5">
              {merged.length === 0 ? (<div className="p-12 text-white/50 text-sm text-center font-medium">
                  {provider === 'civitai' ? (<div className="space-y-2">
                    ...
                    </div>) : ('No models found. Try changing provider/base URL and refresh.')}
                </div>) : (merged.map((row) => {
            const statusKind = row.status === 'installed'
                ? 'ok'
                : row.status === 'missing'
                    ? 'warn'
                    : 'muted';
            const statusLabel = row.status === 'installed'
                ? 'Installed'
                : row.status === 'missing'
                    ? 'Available'
                    : 'Custom';
            ...
            return (<div key={row.id} className="p-5 flex items-center justify-between gap-4 hover:bg-white/[0.02] transition-all group">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2.5 mb-3">
                          ...
                          {row.recommended ? (<div className="px-3 py-1.5 rounded-lg border border-blue-500/30 bg-blue-500/10 text-[10px] font-bold uppercase tracking-wider text-blue-200">
                              ⭐ Recommended
                            </div>) : null}
                          {row.recommended_nsfw && props.nsfwMode ? (<div className="px-3 py-1.5 rounded-lg border border-pink-500/30 bg-pink-500/10 text-[10px] font-bold uppercase tracking-wider text-pink-200">
                              🔥 NSFW Pick
                            </div>) : null}
                          {row.nsfw ? (<div className="px-3 py-1.5 rounded-lg border border-red-500/30 bg-red-500/10 text-[10px] font-bold uppercase tracking-wider text-red-200">
                              🌶️ Adult
                            </div>) : null}
                          {isCivitai && row.civitai_version_id ? (<div className="px-3 py-1.5 rounded-lg border border-cyan-500/30 bg-cyan-500/10 text-[10px] font-bold uppercase tracking-wider text-cyan-200">
                              v{row.civitai_version_id}
                            </div>) : null}
                          {row.protected && (row.status === 'installed' || row.status === 'installed_unsupported') ? (<div className="px-3 py-1.5 rounded-lg border border-white/20 bg-white/5 text-[10px] font-bold uppercase tracking-wider text-white/50">
                              🔒 Default
                            </div>) : null}
                        </div>
                        ...
                        {row.description ? (<div className="text-[11px] text-white/30 truncate mt-1">{row.description}</div>) : null}

                        {/* Pack metadata - shows file count, required nodes, and hints */}
                        {row.install?.files && row.install.files.length > 1 ? (<div className="text-[11px] text-purple-400/80 mt-1">
                            Pack: <span className="font-semibold">{row.install.files.length}</span> files
                          </div>) : null}

                        {row.install?.requires_custom_nodes && row.install.requires_custom_nodes.length > 0 ? (<div className="text-[11px] text-amber-400/70 mt-1 truncate">
                            Requires:{" "}
                        ...
                        {row.install?.hint ? (<div className="text-[11px] text-white/25 mt-1 truncate">
                            {row.install.hint}
                          </div>) : null}

                        {isCivitai && row.civitai_url ? (<a href={row.civitai_url} target="_blank" rel="noopener noreferrer" className="text-[11px] text-blue-400 hover:text-blue-300 underline mt-1 inline-block">
                            View on Civitai →
                          </a>) : null}
                      </div>

                      <div className="flex items-center gap-3 flex-shrink-0">
                        <button type="button" className="px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold flex items-center gap-2 transition-all hover:scale-105 active:scale-95" onClick={() => {
                    ...
                    setToast('Model ID copied to clipboard');
                        ...
                        {canDownload ? (<button type="button" className={[
                    ...
                    ].join(" ")} disabled={installBusy === row.id} onClick={() => {
                        ...
                        if (isCivitai && row.civitai_version_id) {
                            ...
                            {installBusy === row.id && (<span className="absolute inset-0 bg-gradient-to-r from-blue-600 via-blue-500 to-blue-600 animate-shimmer" style={{
                            backgroundSize: '200% 100%',
                            animation: 'shimmer 2s infinite linear'
                        }}/>)}
                        ...
                        {canDelete ? (deleteConfirm === row.id ? (<div className="flex items-center gap-2">
                              <button type="button" className="px-4 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-xs font-bold text-white flex items-center gap-2 transition-all hover:scale-105 active:scale-95" disabled={deleteBusy === row.id} onClick={() => void tryDelete(row.id)}>
                                {deleteBusy === row.id ? (<>
                                    <svg className="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                              ...
                              <button type="button" className="px-3 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold transition-all" onClick={() => setDeleteConfirm(null)}>
                                Cancel
                              </button>
                            </div>) : (<button type="button" className="px-4 py-2.5 rounded-xl bg-white/5 hover:bg-red-600/20 border border-white/10 hover:border-red-500/30 text-xs font-bold flex items-center gap-2 transition-all hover:scale-105 active:scale-95 text-white/60 hover:text-red-400" onClick={() => setDeleteConfirm(row.id)} title="Delete this model from disk">
                              <Trash2 size={14}/>
                              <span>Delete</span>
                            </button>)) : null}
                      </div>
          ...
          {modelType === 'addons' && (<div className="rounded-2xl border border-purple-500/20 overflow-hidden bg-gradient-to-b from-purple-500/[0.03] to-transparent">
              <div className="bg-purple-500/5 px-6 py-4 flex items-center justify-between border-b border-purple-500/20">
                ...
                <div className="flex items-center gap-3">
                  {avatarModelsLoading ? (<div className="text-xs text-purple-300/50 font-semibold">Loading...</div>) : (<div className="text-xs text-purple-300/50 font-semibold">
                      {avatarModels.filter(m => m.installed).length} / {avatarModels.length} Installed
                    </div>)}
                  <button type="button" onClick={() => refreshAvatarModels()} disabled={avatarModelsLoading} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 transition-all disabled:opacity-50" title="Refresh avatar models">
                    <RefreshCw size={13} className={avatarModelsLoading ? 'animate-spin' : ''}/>
                  </button>
                </div>
              </div>

              {avatarModelsError ? (<div className="p-5 text-xs text-purple-300/60">
                  Could not load avatar models: {avatarModelsError}
                </div>) : (<>
                  {/* Feature readiness dashboard */}
                  {Object.keys(avatarFeatures).length > 0 && (<div className="px-6 py-4 border-b border-purple-500/10">
                      <div className="text-[10px] text-white/40 font-bold uppercase tracking-wider mb-3">Feature Readiness</div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        {Object.entries(avatarFeatures).map(([featId, feat]) => (<div key={featId} className={`rounded-xl border p-3 transition-all ${feat.ready
                            ? 'border-emerald-500/20 bg-emerald-500/5'
                            : 'border-white/10 bg-white/[0.02]'}`}>
                            <div className="flex items-center gap-2 mb-1.5">
                              {feat.ready ? (<CheckCircle2 size={13} className="text-emerald-400 flex-shrink-0"/>) : (<XCircle size={13} className="text-white/30 flex-shrink-0"/>)}
                            ...
                            {!feat.ready && feat.required_missing.length > 0 && (<div className="mt-2 text-[10px] text-amber-400/70">
                                Missing: {feat.required_missing.join(', ')}
                              </div>)}
                            {feat.ready && !feat.recommended_installed && feat.recommended_note && (<div className="mt-2 text-[10px] text-blue-300/60">
                  ...
                  <div className="px-6 py-4 border-b border-purple-500/10 flex flex-wrap items-center gap-3">
                    ...
                    <button type="button" onClick={() => downloadAvatarPreset('basic')} disabled={avatarDownloadBusy !== null} className={[
                    "px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide flex items-center gap-2.5 transition-all shadow-lg relative overflow-hidden",
                    ...
                    <button type="button" onClick={() => downloadAvatarPreset('full')} disabled={avatarDownloadBusy !== null} className={[
                    "px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide flex items-center gap-2.5 transition-all shadow-lg relative overflow-hidden",
                  ...
                  {avatarDownloadStatus && (avatarDownloadStatus.running || avatarDownloadStatus.finished) && (<div className="px-6 py-4 border-b border-purple-500/10 bg-gradient-to-r from-purple-500/[0.03] to-transparent">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2.5">
                          {avatarDownloadStatus.running && (<svg className="animate-spin h-4 w-4 text-purple-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>)}
                          {avatarDownloadStatus.finished && (<CheckCircle2 size={16} className="text-emerald-400"/>)}
                        ...
                        <div className="text-[10px] text-white/40 font-mono">
                          ...
                          {avatarDownloadStatus.downloaded_bytes > 0 && (<> | {formatBytes(avatarDownloadStatus.downloaded_bytes)}</>)}
                        </div>
                      </div>

                      {/* Progress bar */}
                      {avatarDownloadStatus.total_models > 0 && (<div className="h-2 bg-white/10 rounded-full overflow-hidden mb-3">
                          <div className={`h-full rounded-full transition-all duration-700 ${avatarDownloadStatus.finished ? 'bg-emerald-500' : 'bg-purple-500'}`} style={{
                            width: `${Math.round(((avatarDownloadStatus.results?.length || 0) / avatarDownloadStatus.total_models) * 100)}%`,
                        }}/>
                        </div>)}

                      {/* Per-model status rows */}
                      <div className="space-y-1">
                        {(avatarDownloadStatus.results || []).map((r) => (<div key={r.id} className="flex items-center gap-2 text-[11px]">
                            {r.status === 'installed' ? (<CheckCircle2 size={11} className="text-emerald-400 shrink-0"/>) : r.status === 'already_installed' ? (<CheckCircle2 size={11} className="text-white/30 shrink-0"/>) : (<XCircle size={11} className="text-red-400 shrink-0"/>)}
                        ...
                        {avatarDownloadStatus.running && avatarDownloadStatus.current_model && (<div className="flex items-center gap-2 text-[11px]">
                      ...
                      {avatarDownloadStatus.finished && (<button type="button" onClick={() => setAvatarDownloadStatus(null)} className="mt-3 text-[10px] text-white/30 hover:text-white/50 transition-colors">
                          Dismiss
                        </button>)}
                    </div>)}
                  ...
                  <div className="divide-y divide-purple-500/10">
                    ...
                    return (<div key={m.id} className="p-5 flex items-center justify-between gap-4 hover:bg-white/[0.02] transition-all group">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2.5 mb-2 flex-wrap">
                              ...
                              {m.is_default && (<div className="px-3 py-1.5 rounded-lg border border-blue-500/30 bg-blue-500/10 text-[10px] font-bold uppercase tracking-wider text-blue-200">
                                  Core
                                </div>)}
                              {isRecommended && (<div className="px-3 py-1.5 rounded-lg border border-purple-500/30 bg-purple-500/10 text-[10px] font-bold uppercase tracking-wider text-purple-200">
                                  Recommended
                                </div>)}
                              {m.license && (<div className="px-2.5 py-1 rounded-lg border border-white/10 bg-white/5 text-[10px] font-medium text-white/40 flex items-center gap-1">
                                  <Shield size={10}/>
                                  {m.license}
                                </div>)}
                              {m.commercial_use_ok === true && (<div className="px-2.5 py-1 rounded-lg border border-emerald-500/20 bg-emerald-500/5 text-[10px] font-medium text-emerald-300/70">
                                  Commercial OK
                                </div>)}
                              {m.commercial_use_ok === false && (<div className="px-2.5 py-1 rounded-lg border border-amber-500/20 bg-amber-500/5 text-[10px] font-medium text-amber-300/70">
                                  Non-Commercial
                                </div>)}
                            </div>
                            ...
                            {m.description && (<div className="text-[11px] text-white/30 mt-1">{m.description}</div>)}
                            {m.requires && m.requires.length > 0 && (<div className="text-[11px] text-amber-400/70 mt-1 truncate">
                                Requires: <span className="font-semibold">{m.requires.join(', ')}</span>
                              </div>)}

                            {/* Feature tags: which features this model enables */}
                            {m.used_by && m.used_by.length > 0 && (<div className="flex items-center gap-1.5 mt-2 flex-wrap">
                                ...
                                return (<span key={`${ub.feature}-${ub.role}`} className={`px-2 py-0.5 rounded text-[9px] font-semibold uppercase tracking-wider border ${isRequired
                                        ? `border-${fl.color}-500/30 bg-${fl.color}-500/10 text-${fl.color}-300`
                                        : 'border-white/10 bg-white/5 text-white/40'}`} title={isRequired ? `Required for ${fl.label}` : `Recommended for ${fl.label}`}>
                          ...
                          <div className="flex items-center gap-3 flex-shrink-0">
                            {m.homepage && (<a href={m.homepage} target="_blank" rel="noopener noreferrer" className="px-3 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold flex items-center gap-2 transition-all text-white/50 hover:text-white/70" title="View project page">
                                <ExternalLink size={13}/>
                                <span>Info</span>
                              </a>)}

                            {/* Install button — only for not-installed models with a download URL */}
                            {!m.installed && m.download_url && (<button type="button" className="px-4 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-xs font-bold text-white flex items-center gap-2 transition-all hover:scale-105 active:scale-95 shadow-lg shadow-purple-600/20 disabled:opacity-50 disabled:cursor-wait" disabled={avatarInstallBusy === m.id || avatarDownloadBusy !== null} onClick={() => installSingleAvatarModel(m.id)} title={`Download and install ${m.name}`}>
                            ...
                            {m.installed && (avatarDeleteConfirm === m.id ? (<div className="flex items-center gap-2">
                                  <button type="button" className="px-4 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-xs font-bold text-white flex items-center gap-2 transition-all hover:scale-105 active:scale-95" disabled={avatarDeleteBusy === m.id} onClick={() => deleteAvatarModel(m.id)}>
                                  ...
                                  <button type="button" className="px-3 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold transition-all" onClick={() => setAvatarDeleteConfirm(null)}>
                                    Cancel
                                  </button>
                                </div>) : (<button type="button" className="px-4 py-2.5 rounded-xl bg-white/5 hover:bg-red-600/20 border border-white/10 hover:border-red-500/30 text-xs font-bold flex items-center gap-2 transition-all hover:scale-105 active:scale-95 text-white/60 hover:text-red-400" onClick={() => setAvatarDeleteConfirm(m.id)} title="Uninstall this model to free disk space">
                                  <Trash2 size={13}/>
                                  <span>Uninstall</span>
                                </button>))}
                          </div>
                    ...
                    {avatarModels.length === 0 && !avatarModelsLoading && (<div className="p-8 text-center text-white/40 text-sm">
          ...
          {modelType === 'lora' && (<div className="rounded-2xl border border-cyan-500/20 overflow-hidden bg-gradient-to-b from-cyan-500/[0.03] to-transparent">
              <div className="bg-cyan-500/5 px-6 py-4 flex items-center justify-between border-b border-cyan-500/20">
                ...
                <div className="flex items-center gap-3">
                  {loraLoading ? (<div className="text-xs text-cyan-300/50 font-semibold">Loading...</div>) : (<div className="text-xs text-cyan-300/50 font-semibold">
                      {loraInstalled.length} Installed / {loraRegistry.length} Available
                    </div>)}
                  <button type="button" onClick={() => refreshLoraModels()} disabled={loraLoading} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 transition-all disabled:opacity-50" title="Refresh LoRA models">
                    <RefreshCw size={13} className={loraLoading ? 'animate-spin' : ''}/>
                  </button>
                </div>
              </div>

              {loraError ? (<div className="p-5 text-xs text-cyan-300/60">
                  ...
                  <div className="px-6 py-4 border-b border-cyan-500/10">
                    ...
                    {!props.nsfwMode && (<div className="mt-2 text-[10px] text-white/25 flex items-center gap-1.5">
                        <Shield size={10}/>
                  ...
                  {loraDownloadStatus && (loraDownloadStatus.running || loraDownloadStatus.finished) && (<div className="px-6 py-4 border-b border-cyan-500/10">
                      <div className="flex items-center gap-3 mb-2">
                        {loraDownloadStatus.running && (<svg className="animate-spin h-4 w-4 text-cyan-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                          </svg>)}
                        ...
                        <span className="text-[10px] text-white/30 ml-auto">
                          ...
                          {loraDownloadStatus.downloaded_bytes > 0 && (<> | {formatBytes(loraDownloadStatus.downloaded_bytes)}</>)}
                        </span>
                      </div>
                      {loraDownloadStatus.total_models > 0 && (<div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full transition-all duration-500 ${loraDownloadStatus.finished ? 'bg-emerald-500' : 'bg-cyan-500'}`} style={{
                            width: `${loraDownloadStatus.finished ? 100 :
                                ((loraDownloadStatus.results?.length || 0) / loraDownloadStatus.total_models) * 100}%`,
                        }}/>
                        </div>)}
                      {loraDownloadStatus.running && loraDownloadStatus.current_model && (<div className="mt-2 flex items-center gap-2">
                          <div className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse"/>
                          <span className="text-[10px] text-white/40">
                            Installing: {loraRegistry.find((l) => l.id === loraDownloadStatus.current_model)?.name || loraDownloadStatus.current_model}
                          </span>
                        </div>)}
                      {loraDownloadStatus.finished && (<button type="button" onClick={() => setLoraDownloadStatus(null)} className="mt-3 text-[10px] text-white/30 hover:text-white/50 transition-colors">
                          Dismiss
                        </button>)}
                    </div>)}

                  {/* Installed LoRAs */}
                  {loraInstalled.length > 0 && (<div className="px-6 py-4 border-b border-cyan-500/10">
                      <div className="text-[10px] text-white/40 font-bold uppercase tracking-wider mb-3">Installed LoRAs</div>
                      <div className="flex flex-wrap gap-2">
                        {loraInstalled.map((l) => (<div key={l.id} className="px-3 py-2 rounded-xl border border-emerald-500/20 bg-emerald-500/5 text-[11px] font-semibold text-emerald-200 flex items-center gap-2">
                  ...
                  {/* Registry LoRAs */}
                  <div className="divide-y divide-white/5">
                    {loraRegistry.map((lora) => {
                    const isInstalled = loraInstalled.some((i) => i.id === lora.id);
                    return (<div key={lora.id} className="p-5 flex items-center justify-between gap-4 hover:bg-white/[0.02] transition-all group">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2.5 mb-2 flex-wrap">
                              <div className={`px-3 py-1.5 rounded-lg border text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 ${isInstalled
                            ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-200'
                            : 'border-white/10 bg-white/5 text-white/40'}`}>
                                {isInstalled ? (<><CheckCircle2 size={10}/> Installed</>) : (<><Download size={10}/> Available</>)}
                              </div>
                              <div className="px-2.5 py-1 rounded-lg border border-white/10 bg-white/5 text-[10px] font-medium text-white/40">
                                {lora.base}
                              </div>
                              {lora.recommended ? (<div className="px-3 py-1.5 rounded-lg border border-blue-500/30 bg-blue-500/10 text-[10px] font-bold uppercase tracking-wider text-blue-200">
                                  ⭐ Recommended
                                </div>) : null}
                              {lora.recommended_nsfw && props.nsfwMode ? (<div className="px-3 py-1.5 rounded-lg border border-pink-500/30 bg-pink-500/10 text-[10px] font-bold uppercase tracking-wider text-pink-200">
                                  🔥 NSFW Pick
                                </div>) : null}
                              {lora.gated ? (<div className="px-2.5 py-1 rounded-lg border border-red-500/30 bg-red-500/10 text-[10px] font-medium text-red-200">
                                  🌶️ Adult
                                </div>) : null}
                              {lora.size_mb > 0 && (<div className="text-[10px] text-white/25">{lora.size_mb} MB</div>)}
                            </div>
                            ...
                            {lora.description && (<div className="text-[11px] text-white/30 mt-1">{lora.description}</div>)}
                            {lora.trigger_words && lora.trigger_words.length > 0 && (<div className="flex items-center gap-1.5 mt-2 flex-wrap">
                                <span className="text-[10px] text-white/25">Trigger words:</span>
                                {lora.trigger_words.map((tw) => (<span key={tw} className="px-2 py-0.5 rounded text-[9px] font-semibold uppercase tracking-wider border border-cyan-500/20 bg-cyan-500/5 text-cyan-300/70">
                                    {tw}
                                  </span>))}
                              </div>)}
                          </div>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            {lora.model_url && (<a href={lora.model_url} target="_blank" rel="noopener noreferrer" className="px-3 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold flex items-center gap-2 transition-all text-white/50 hover:text-white/70" title="View model page">
                                <ExternalLink size={13}/>
                                <span>View</span>
                              </a>)}

                            {/* Install button — not installed, has download URL */}
                            {!isInstalled && lora.download_url && (<button type="button" className="px-4 py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-xs font-bold text-white flex items-center gap-2 transition-all hover:scale-105 active:scale-95 shadow-lg shadow-cyan-600/20 disabled:opacity-50 disabled:cursor-wait" disabled={loraInstallBusy === lora.id || loraDownloadBusy !== null} onClick={() => installLora(lora.id)} title={`Install ${lora.name}`}>
                            ...
                            {isInstalled && (loraDeleteConfirm === lora.id ? (<div className="flex items-center gap-2">
                                  <button type="button" className="px-4 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-xs font-bold text-white flex items-center gap-2 transition-all hover:scale-105 active:scale-95" disabled={loraDeleteBusy === lora.id} onClick={() => deleteLora(lora.id)}>
                                  ...
                                  <button type="button" className="px-3 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold transition-all" onClick={() => setLoraDeleteConfirm(null)}>
                                    Cancel
                                  </button>
                                </div>) : (<button type="button" className="px-4 py-2.5 rounded-xl bg-white/5 hover:bg-red-600/20 border border-white/10 hover:border-red-500/30 text-xs font-bold flex items-center gap-2 transition-all hover:scale-105 active:scale-95 text-white/60 hover:text-red-400" onClick={() => setLoraDeleteConfirm(lora.id)} title="Delete this LoRA to free disk space">
                                  <Trash2 size={13}/>
                                  <span>Delete</span>
                                </button>))}
                          </div>
                    ...
                    {loraRegistry.length === 0 && !loraLoading && (<div className="p-8 text-center text-white/40 text-sm">
      ...
      {/* Toast notification */}
      {toast ? (<div className="fixed bottom-8 right-8 z-50 bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/20 rounded-2xl px-6 py-4 text-sm text-white shadow-2xl shadow-black/40 animate-in slide-in-from-bottom-4 flex items-center gap-3 min-w-[320px]">
          {toast.includes('Successfully') || toast.includes('installed') ? (<div className="flex-shrink-0">
              <CheckCircle2 size={18} className="text-emerald-400"/>
            </div>) : toast.includes('failed') || toast.includes('error') || toast.includes('Error') ? (<div className="flex-shrink-0">
              <XCircle size={18} className="text-red-400"/>
            </div>) : (<div className="w-2 h-2 rounded-full bg-blue-400 animate-pulse flex-shrink-0"/>)}
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-white/90 truncate">{toast}</div>
            {installBusy && (<div className="text-[10px] text-white/50 mt-1 font-medium">
