 ...
 *  scoping. Idempotent. */
export function registerUserScopedKey(baseKey) {
    if (baseKey)
        registry.add(baseKey);
}
/** Inspect the current registry (primarily for tests / debugging). */
export function registeredUserScopedKeys() {
...
export function persistActiveStateForUser(userId) {
    if (!userId)
        return;
...
export function restoreActiveStateForUser(userId) {
    if (!userId)
        return;
    for (const base of registry) {
        try {
...
export function clearActiveUiState() {
    for (const base of registry) {
        try {
            localStorage.removeItem(base);
        }
