...
export function install() {
    ...
    svc.speak = ((text, callbacks = {}) => {
        ...
        return provider
            ...
            if (_originalSpeak)
                return _originalSpeak(text, callbacks);
        });
    });
    svc.stopSpeaking = (() => {
        const engineId = getActiveTtsEngineId();
...
export function uninstall() {
    ...
    if (!svc)
        return;
    if (_originalSpeak)
        svc.speak = _originalSpeak;
    if (_originalStop)
        svc.stopSpeaking = _originalStop;
    _installed = false;
    _originalSpeak = null;
    _originalStop = null;
}
