...
import {
  ...
  speak as piperSpeak,
  stop as piperStop,
  synthesizeToBlob as piperSynthesize,
} from '../../piperTts'
import { getFallbackBaseUrl, isNetworkyError } from './mirror'

const ID = 'piper-wasm'

async function _withMirrorFallback<T>(
  first: () => Promise<T>,
  retry: (baseUrl: string) => Promise<T>,
): Promise<T> {
  try {
    return await first()
  } catch (err) {
    if (!isNetworkyError(err)) throw err
...
const PiperProvider: TtsProvider = {
  id: ID,
  ...
  async speak(text: string, opts: TtsSpeakOptions = {}): Promise<void> {
    ...
    if (opts.voiceId) setSelectedVoiceId(opts.voiceId)
    ...
    await _withMirrorFallback(
      () => piperSpeak(text, { voiceId, rate: opts.rate, onEnd: opts.onEnd, onError: opts.onError }),
      async (baseUrl) => {
  ...
  async synthesizeToBlob(text: string, opts: TtsSynthOptions = {}): Promise<Blob> {
    const voiceId = opts.voiceId || getSelectedVoiceId()
    return _withMirrorFallback(
      () => piperSynthesize(text, { voiceId }),
      (baseUrl) => piperSynthesize(text, { voiceId, baseUrl }),
    )
  },
    ...
    return [
      {
        ...
        label: 'Piper voice',
        description:
          'Voice model fetched from HuggingFace on first use (~20 MB) and cached in OPFS.',
