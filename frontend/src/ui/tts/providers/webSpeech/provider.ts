...
function _voicesNative(): SpeechSynthesisVoice[] {
...
function _toTtsVoice(v: SpeechSynthesisVoice): TtsVoice {
  return {
...
const WebSpeechProvider: TtsProvider = {
  ...
  async init(): Promise<void> {
    ...
    await new Promise<void>((resolve) => {
      ...
      syn.onvoiceschanged = () => {
        try { prev?.call(syn, new Event('voiceschanged')) } catch {}
        done()
      }
  ...
  async speak(text: string, opts: TtsSpeakOptions = {}): Promise<void> {
    ...
    if (opts.voiceId) {
      const voice = _voicesNative().find(
        (v) => v.voiceURI === opts.voiceId || v.name === opts.voiceId,
      )
