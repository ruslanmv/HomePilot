...
export function McpUninstallDialog({ server, backendUrl, apiKey, onClose, onUninstalled }) {
    ...
    const handleUninstall = async () => {
        ...
        try {
            ...
            const res = await fetch(`${backendUrl}/v1/agentic/registry/${encodeURIComponent(server.id)}/unregister`, { method: 'POST', headers });
            if (!res.ok) {
    ...
    return (<div className="fixed inset-0 z-[60] flex items-center justify-center" onClick={onClose}>
      ...
      {/* Dialog */}
      <div className="relative w-full max-w-sm bg-[#0f0f18] border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-fade-scale-in" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        ...
        <div className="flex items-center gap-3 px-5 pb-5">
          <button onClick={onClose} disabled={busy} className="flex-1 px-4 py-2.5 text-sm font-medium text-white/60 bg-white/5 hover:bg-white/10 rounded-xl transition-colors border border-white/10">
            Cancel
          </button>
          <button onClick={handleUninstall} disabled={busy} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-red-200 bg-red-500/20 hover:bg-red-500/30 rounded-xl transition-colors border border-red-500/30 disabled:opacity-50">
