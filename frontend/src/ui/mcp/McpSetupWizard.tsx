...
type WizardState = 'instructions' | 'credentials' | 'connecting' | 'success' | 'error'

function authIcon(authType: string) {
  if (isOpenAuth(authType)) return Globe
  if (needsOAuthFlow(authType)) return Lock
  return Key
}
...
export function McpSetupWizard({ server, backendUrl, apiKey: appApiKey, onClose, onComplete }: Props) {
  ...
  const [state, setState] = useState<WizardState>('instructions')
  ...
  const handleSubmitCredential = async () => {
    ...
    try {
      ...
      if (appApiKey) headers['x-api-key'] = appApiKey
      ...
      if (toolMatch) setToolCount(parseInt(toolMatch[1], 10))
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-xl bg-white/5 border border-white/10 p-5 space-y-3">
          ...
          {guideStep.link && (
            <a
              href={guideStep.link.url}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-sm font-medium text-purple-300 hover:text-purple-200 transition-colors mt-2"
            >
          ...
          {guide.steps.map((s, i) => (
            ...
            >
              ...
              <span className={`text-xs ${
                i === step ? 'text-white/80 font-medium' : i < step ? 'text-white/50' : 'text-white/30'
              }`}>
                {s.title}
              </span>
            </div>
      ...
      <div className="rounded-xl bg-white/5 border border-white/10 p-5 space-y-4">
        ...
        <div className="relative">
          <input
            ...
            placeholder={guide.credentialPlaceholder || 'Paste your key here...'}
            className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 pr-10 text-sm text-white placeholder-white/25 focus:outline-none focus:border-cyan-500/50 transition-colors font-mono"
            autoFocus
            onKeyDown={(e) => {
              if (e.key === 'Enter' && credential.trim()) handleSubmitCredential()
            }}
          />
          <button
            type="button"
  ...
  const renderOAuthInfo = () => (
    <div className="space-y-5">
      ...
      <div className="rounded-xl bg-amber-500/5 border border-amber-500/20 p-5 space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="shrink-0 p-2 text-white/40 hover:text-white/70 bg-white/5 hover:bg-white/10 rounded-lg transition-colors"
            title="Copy URL"
          >
      ...
      <div className="rounded-xl bg-white/5 border border-white/10 p-4">
        ...
        <a
          href={`https://${new URL(server.url).hostname}`}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 text-xs font-medium text-cyan-300 hover:text-cyan-200 transition-colors mt-2"
        >
          <Globe size={12} />
  ...
  const renderError = () => (
    <div className="flex flex-col items-center justify-center py-10 space-y-5">
      ...
      <div className="flex gap-3">
        <button
          onClick={() => { setState('instructions'); setStep(0); setError(null) }}
          className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 text-white/70 text-sm rounded-xl transition-colors border border-white/10"
        >
          <ArrowLeft size={14} />
          Go Back
        </button>
        <button
          onClick={() => { setState(requiresCred ? 'credentials' : 'instructions'); setError(null) }}
          className="flex items-center gap-2 px-4 py-2 bg-red-500/20 hover:bg-red-500/30 text-red-300 text-sm rounded-xl transition-colors"
        >
          Try Again
        </button>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      ...
      >
        ...
        <div className="sticky top-0 bg-[#0b0b12]/95 backdrop-blur border-b border-white/10 px-6 py-4 z-10">
          ...
          {(showInstructions || showCredentials) && !requiresOAuth && (
            <StepIndicator
              current={showCredentials ? guide.steps.length : step}
              total={totalStepsForIndicator}
            />
          )}
        </div>
        ...
        {/* Footer navigation */}
        {(showInstructions || showCredentials) && !showSuccess && !showError && (
          <div className="sticky bottom-0 bg-[#0b0b12]/95 backdrop-blur border-t border-white/10 px-6 py-4 flex items-center justify-between">
            <button
              onClick={handleBack}
              disabled={step === 0 && !showCredentials}
              className="flex items-center gap-1.5 px-3 py-2 text-sm text-white/50 hover:text-white/80 disabled:opacity-30 disabled:cursor-default transition-colors"
            >
              <ArrowLeft size={14} />
              Back
            </button>
            ...
            {showCredentials ? (
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-purple-500 hover:bg-purple-600 disabled:bg-white/10 disabled:text-white/30 text-white text-sm font-medium rounded-xl transition-all"
              >
                Connect
                <ArrowRight size={14} />
              </button>
              ...
              <button
                onClick={onClose}
                className="flex items-center gap-2 px-5 py-2.5 bg-white/10 hover:bg-white/15 text-white text-sm font-medium rounded-xl transition-all"
              >
                Done
              </button>
            ) : nextLabel ? (
