...
import { Search, Download, ExternalLink, Globe, AlertCircle } from 'lucide-react'
...
export function MarketplaceSearch({ backendUrl, apiKey, onInstalled }: Props) {
  ...
  const doSearch = useCallback(async (q: string) => {
    ...
    try {
      const res = await fetch(
        `${backendUrl}/v1/marketplace/search?q=${encodeURIComponent(q)}&type=mcp_server&limit=10`,
        { headers: headers() },
      )
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-6 space-y-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search Matrix Hub for MCP servers..."
          ...
          {results.map((item) => (
            ...
            >
              <div className="min-w-0 flex-1 mr-4">
                ...
                {item.author && (
                  <p className="text-xs text-white/30 mt-1">by {item.author}</p>
                )}
              </div>
              <button
                onClick={() => handleInstall(item)}
                disabled={installing === item.id || !item.install_url}
                className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-purple-500/20 hover:bg-purple-500/30 text-purple-300 rounded-lg transition-colors disabled:opacity-50"
              >
                {installing === item.id ? (
      ...
      {!searching && query && results.length === 0 && !error && (
        <p className="text-sm text-white/40 py-4 text-center">No servers found for &quot;{query}&quot;</p>
      )}
    </div>
  )
}
