...
import { X, Globe, Lock, Key, Shield, ExternalLink, Tag, Server, Radio, Trash2, Edit3, Check, Copy, AlertCircle, } from 'lucide-react';
import { needsCredentialInput, needsOAuthFlow, isOpenAuth } from './setupInstructions';
function authBadgeInfo(authType) {
...
export function McpServerDetailsDrawer({ server, backendUrl, apiKey, onClose, onSetup, onUninstall }) {
      ...
      <div className="relative w-full max-w-md bg-[#0b0b12] border-l border-white/10 h-full overflow-y-auto animate-slide-in-right" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <InfoCard label="Auth Type" value={auth.label} icon={<AuthIcon size={14}/>} badgeColor={auth.color}/>
            <InfoCard label="Transport" value={server.transport || 'Auto-detect'} icon={<Radio size={14}/>}/>
            <InfoCard label="Security" value={server.secure ? 'Encrypted' : 'Standard'} icon={<Shield size={14}/>}/>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-1">
                {urlSaved && (<span className="text-[10px] text-emerald-400 mr-1">Saved</span>)}
                <button onClick={handleCopyUrl} className="p-1 text-white/30 hover:text-white/60 transition-colors rounded" title="Copy URL">
                  {copiedUrl ? <Check size={12} className="text-emerald-400"/> : <Copy size={12}/>}
                </button>
                {server.is_registered && (<button onClick={() => {
                if (editingUrl) {
                    setEditingUrl(false);
            ...
            }} className="p-1 text-white/30 hover:text-white/60 transition-colors rounded" title={editingUrl ? 'Cancel edit' : 'Edit URL'}>
                    {editingUrl ? <X size={12}/> : <Edit3 size={12}/>}
                  </button>)}
              </div>
            </div>

            {editingUrl ? (<div className="space-y-2">
                <div className="flex items-center gap-2">
                  <Server size={13} className="text-white/30 shrink-0"/>
                  <input type="text" value={urlValue} onChange={(e) => setUrlValue(e.target.value)} className="flex-1 text-xs text-white bg-black/30 border border-cyan-500/30 rounded-lg px-3 py-2 font-mono focus:outline-none focus:border-cyan-500/50 transition-colors" autoFocus onKeyDown={(e) => {
                if (e.key === 'Enter')
                    handleSaveUrl();
                if (e.key === 'Escape') {
                    setEditingUrl(false);
                ...
                {urlError && (<div className="flex items-center gap-1 text-red-400 text-[11px]">
                    <AlertCircle size={10}/>
                    {urlError}
                  </div>)}
                <div className="flex items-center gap-2 justify-end">
                  <button onClick={() => { setEditingUrl(false); setUrlValue(server.url); setUrlError(null); }} className="px-2.5 py-1 text-xs text-white/50 hover:text-white/70 bg-white/5 rounded-lg transition-colors">
                    Cancel
                  </button>
                  <button onClick={handleSaveUrl} disabled={urlSaving || !urlValue.trim()} className="px-2.5 py-1 text-xs text-cyan-300 bg-cyan-500/20 hover:bg-cyan-500/30 rounded-lg transition-colors disabled:opacity-50">
          ...
          {/* Tags */}
          {server.tags.length > 0 && (<div>
              <h3 className="text-xs font-semibold text-white/40 uppercase tracking-wide mb-2">
                Tags
              </h3>
              <div className="flex flex-wrap gap-1.5">
                {server.tags.map((tag) => (<span key={tag} className="text-xs px-2.5 py-1 rounded-full bg-white/5 text-white/50 border border-white/5">
                    {tag}
                  </span>))}
              </div>
            </div>)}
          ...
          {/* Documentation link */}
          {server.documentation_url && (<a href={server.documentation_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 w-full px-4 py-3 text-sm font-medium text-cyan-300 bg-cyan-500/10 hover:bg-cyan-500/20 rounded-xl transition-colors border border-cyan-500/20">
              <ExternalLink size={16}/>
              View Documentation
            </a>)}

          {/* Setup button (if installed but needs config) */}
          {server.is_registered && server.requires_oauth_config && onSetup && (<button onClick={onSetup} className="flex items-center gap-2 w-full justify-center px-4 py-3 text-sm font-medium text-amber-200 bg-amber-500/20 hover:bg-amber-500/30 rounded-xl transition-colors border border-amber-500/30">
              <Key size={16}/>
              Complete Setup
            </button>)}

          {/* Uninstall button (if installed) */}
          {server.is_registered && onUninstall && (<div className="pt-3 border-t border-white/5">
              <button onClick={onUninstall} className="flex items-center gap-2 w-full justify-center px-4 py-2.5 text-sm font-medium text-red-300/70 hover:text-red-300 bg-transparent hover:bg-red-500/10 rounded-xl transition-colors">
                <Trash2 size={14}/>
                Uninstall Server
              </button>
            </div>)}
...
// ── Tiny info card ────────────────────────────────────────────────────
function InfoCard({ label, value, icon, badgeColor, }) {
    return (<div className="rounded-xl bg-white/5 border border-white/5 p-3">
      ...
      {badgeColor ? (<span className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-full ${badgeColor}`}>
          {value}
        </span>) : (<span className="text-xs text-white/70 font-medium">{value}</span>)}
    </div>);
}
