...
export type SetupStep = {
  title: string
  description: string
  /** If present, render as a clickable link */
  link?: { label: string; url: string }
}

export type SetupGuide = {
  steps: SetupStep[]
  /** Label for the credential field (e.g. "Personal Access Token") */
...
const SERVER_GUIDES: Record<string, Partial<SetupGuide>> = {
  // ── API Key servers ────────────────────────────────────────────────────
  hubspot: {
    steps: [
...
const AUTH_TYPE_GUIDES: Record<string, SetupGuide> = {
  'Open': {
    steps: [
...
export function needsOAuthFlow(authType: string): boolean {
  return ['OAuth2.1', 'OAuth'].includes(authType)
}

/** Returns true if the server works immediately without any config. */
export function isOpenAuth(authType: string): boolean {
