...
export function McpServersEmptyState({ loading, error, onAddServer }) {
    if (loading) {
    ...
    return (<div className="flex flex-col items-center justify-center h-64 text-white/50">
      ...
      {onAddServer && (<button onClick={onAddServer} className="flex items-center gap-2 bg-white/10 hover:bg-white/20 px-4 py-2 rounded-full text-sm font-medium transition-colors">
          <Plus size={16}/>
          Add Server
        </button>)}
    </div>);
