...
import { X, Radio, Boxes, Copy, Check, RefreshCw, AlertTriangle, CheckCircle } from 'lucide-react'
...
export function ServerDetailDrawer({ server, catalogTools, backendUrl, apiKey, onClose, onRepaired }: Props) {
  ...
  const handleRepair = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        if (data?.sync) setRepairResult(data.sync)
      ...
      >
        ...
        <div className="px-6 py-6 space-y-6">
          ...
          <div>
            ...
            {isStale && (
              <div className="flex items-start gap-2.5 px-3 py-2.5 mb-3 rounded-lg border border-amber-500/20 bg-amber-500/5">
                ...
                <div className="text-xs text-amber-200/80 leading-relaxed">
                  <strong>No skills linked yet.</strong> This skill set exists, but Forge
                  hasn't imported tools from your MCP servers.
                  <ol className="mt-1.5 ml-3 list-decimal space-y-0.5 text-white/50">
                    <li>Ensure MCP servers are running (ports 9101–9120)</li>
                    <li>Click <strong className="text-amber-300">Recompute Tool Links</strong> below</li>
                    <li>If Forge runs in Docker, set <code className="text-amber-300">MCP_TOOL_HOST=host.docker.internal</code></li>
                  </ol>
                </div>
              </div>
            )}
            ...
            {repairResult && !isStale && (
              <div className={`flex items-start gap-2.5 px-3 py-2.5 mb-3 rounded-lg border ${
                repairResult.mcp_servers_reachable > 0
                ...
                <div className="text-xs text-white/60 leading-relaxed">
                  {repairResult.tools_registered > 0
                    ? `Imported ${repairResult.tools_registered} new skill${repairResult.tools_registered !== 1 ? 's' : ''}. `
                    : ''}
                  {(repairResult.tools_updated ?? 0) > 0
                    ? `Updated ${repairResult.tools_updated} skill${repairResult.tools_updated !== 1 ? 's' : ''}. `
                    : ''}
                  {repairResult.mcp_servers_reachable}/{repairResult.mcp_servers_total} MCP servers reachable.
                  {repairResult.mcp_servers_reachable === 0 && (
                    <span className="text-amber-300"> No servers responded on {repairResult.mcp_tool_host || '127.0.0.1'}.</span>
                  )}
                </div>
              </div>
            )}
