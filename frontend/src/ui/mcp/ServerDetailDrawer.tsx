        ...
        <div className="px-6 py-6 space-y-5">
          ...
          {repairResult && (
            <div className={`flex items-start gap-2.5 px-3 py-2.5 rounded-lg border ${
              ...
              <CheckCircle size={14} className={`shrink-0 mt-0.5 ${
                repairResult.mcp_servers_reachable > 0 ? 'text-emerald-400' : 'text-amber-400'
              }`} />
          ...
          <div>
            ...
            {!loadingTools && effectiveTools.length > 0 ? (
              ...
              <p className="text-sm text-white/40">No tools linked yet. Try Sync All below.</p>
            ) : null}
          </div>
          ...
          {!isGateway && (
            <button
              ...
              disabled={repairing}
              className="flex items-center gap-2 w-full justify-center px-4 py-2.5 text-xs font-medium text-violet-300 bg-violet-500/10 hover:bg-violet-500/20 rounded-xl transition-colors disabled:opacity-50 border border-violet-500/20"
            >
