...
export function useForgeRegistry({ backendUrl, apiKey, enabled = true }) {
    ...
    const fetchRegistry = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`${backendUrl}/v1/agentic/registry/servers?${params.toString()}`, { headers: headers() });
            if (!res.ok)
    ...
    const patchServerLocally = useCallback((serverId, registered) => {
        setData((prev) => {
            if (!prev)
                return prev;
            return {
                ...prev,
                servers: prev.servers.map((s) => s.id === serverId
    ...
    const install = useCallback(async (serverId, serverApiKey) => {
        try {
            const body = {};
            if (serverApiKey)
                body.api_key = serverApiKey;
            const res = await fetch(`${backendUrl}/v1/agentic/registry/${encodeURIComponent(serverId)}/register`, {
                method: 'POST',
    ...
    const uninstall = useCallback(async (serverId) => {
        try {
            const res = await fetch(`${backendUrl}/v1/agentic/registry/${encodeURIComponent(serverId)}/unregister`, {
                method: 'POST',
                headers: { ...headers(), 'Content-Type': 'application/json' },
            });
            const json = await res.json().catch(() => ({}));
    ...
    const allServers = useMemo(() => data?.servers || [], [data]);
    ...
    // Derived counts
    const needsSetupCount = useMemo(() => allServers.filter((s) => s.is_registered && s.requires_oauth_config).length, [allServers]);
    return {
        servers,
        total: data?.total ?? 0,
