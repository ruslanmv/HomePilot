...
function gatewayToInstalled(g, toolMap) {
...
export function useInstalledServers({ backendUrl, apiKey }) {
    ...
    const catalogTools = useMemo(() => catalog?.tools || [], [catalog]);
    const servers = useMemo(() => {
        ...
        let combined = [...gateways, ...virtualServers].filter((s) => s.kind === 'gateway' || s.toolCount > 0);
        if (search.trim()) {
            const q = search.toLowerCase();
            combined = combined.filter((s) => s.name.toLowerCase().includes(q) ||
    ...
    const counts = useMemo(() => {
        ...
        const installedVirtualServers = (catalog?.servers || []).filter((s) => (s.tool_ids || []).length > 0).length;
        return {
            gateways,
