...
export type StudioParams = {
  ...
  // Callback to switch to Creator Studio mode (optional projectId for resuming)
                ...
                <div
                  ...
                  onClick={() => {
                    if (project.type === 'play') {
                      ...
                      setView('player')
                    } else if (props.onOpenCreatorStudio) {
                      // Open Creator Studio with project ID for resuming
      ...
      {currentStory && currentStory.scenes.length > 0 && (
        <SceneChips
          scenes={currentStory.scenes.map((scene) => ({
            idx: scene.idx,
            status: scene.image_url ? 'ready' : (isGeneratingImage && scene.idx === currentSceneIndex ? 'generating' : 'pending'),
            thumbnailUrl: scene.image_url,
          } as SceneChipData))}
          activeIndex={currentSceneIndex}
          onSelect={(idx) => setCurrentSceneIndex(idx)}
