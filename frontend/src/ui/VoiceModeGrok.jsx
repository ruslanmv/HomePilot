...
import { Settings, Mic, Volume2, VolumeX, SlidersHorizontal, ChevronDown, PenLine, Zap, Radio, AlertCircle, X, ZoomIn, ZoomOut, Maximize2, Download, } from 'lucide-react';
...
import { getDefaultVoice, getVoiceById, findBrowserVoice, LS_VOICE_ID, } from './voice/voices';
import { getDefaultPersonality, getPersonalityById, LS_PERSONALITY_ID, } from './voice/personalities';
 ...
 */
function parseInlineMarkdown(line, onImageClick) {
    ...
    while ((match = RE_INLINE.exec(line)) !== null) {
        ...
        if (isImage) {
            nodes.push(<img key={`img-${match.index}`} src={url} alt={alt || 'Photo'} className="inline-block w-72 max-h-96 h-auto object-contain rounded-xl border border-white/10 bg-black/20 cursor-zoom-in hover:opacity-90 transition-opacity my-2" loading="lazy" onClick={() => onImageClick?.(url)} onError={(e) => { e.target.style.display = 'none'; }}/>);
        }
        else {
            ...
            if (looksLikeImage) {
                ...
                nodes.push(<img key={`lnkimg-${match.index}`} src={url} alt={alt || 'Photo'} className="inline-block w-72 max-h-96 h-auto object-contain rounded-xl border border-white/10 bg-black/20 cursor-zoom-in hover:opacity-90 transition-opacity my-2" loading="lazy" onClick={() => onImageClick?.(url)} onError={(e) => { e.target.style.display = 'none'; }}/>);
            }
            else {
                nodes.push(<a key={`lnk-${match.index}`} href={url} target="_blank" rel="noreferrer" className="text-blue-400 underline hover:text-blue-300">
            {alt}
          </a>);
            }
        }
 ...
 */
function useSmoothedLevel(level) {
 ...
 * - Peak: warm orange
 */
function intensityColor(level01) {
    if (level01 < 0.35)
        return 'rgba(255,255,255,0.65)'; // cool white/gray
    if (level01 < 0.70)
        return 'rgba(151,196,255,0.85)'; // icy blue
    if (level01 < 0.90)
        return 'rgba(190,120,255,0.90)'; // purple/magenta
 ...
 */
function MiniStudioMeter({ audioLevel, isUserActive, isAiSpeaking, }) {
    ...
    return (<div className={`hp-mini-studio ${idle ? 'hp-mini-studio--idle' : 'hp-mini-studio--hot'} ${isAiSpeaking ? 'hp-mini-studio--ai' : ''}`} style={{ ['--hp-meter-color']: barColor }} aria-hidden="true">
      {bars.map((b, i) => (<span key={i} className="hp-mini-studio-bar" style={{ transform: `scaleY(${b})` }}/>))}
    </div>);
}
/**
 ...
 */
function useTypewriterText(text, speed, onProgress, enabled = true) {
 ...
 */
function RenderTypedMessage({ fullText, typingSpeed, onProgress, animate, media, onImageClick, }) {
    ...
    const renderInline = (text) => isTyping ? text : parseInlineMarkdown(text, onImageClick);
    return (<div className="hp-message-assistant text-[17px]">
      {lines.map((line, i) => {
            ...
            if (isBullet) {
                return (<div key={i} className="ml-4 my-1 flex gap-2">
              <span>•</span>
              <span>
                {renderInline(content)}
                {showCursor && (<span className="inline-block w-[2px] h-[1em] bg-white/80 ml-[1px] hp-cursor align-middle"/>)}
              </span>
            </div>);
            }
            return (<p key={i} className="my-2 whitespace-pre-wrap">
            {renderInline(line)}
            {showCursor && (<span className="inline-block w-[2px] h-[1em] bg-white/80 ml-[1px] hp-cursor align-middle"/>)}
          </p>);
        })}
      ...
      {!isTyping && media?.images?.length ? (<div className="mt-3 flex gap-3 overflow-x-auto hp-fade-in">
          {[...new Set(media.images)].map((src, i) => {
                const resolved = resolveFileUrl(src);
                return (<img key={src || i} src={resolved} alt={`Generated ${i + 1}`} className="w-72 max-h-96 h-auto object-contain rounded-xl border border-white/10 bg-black/20 cursor-zoom-in hover:opacity-90 transition-opacity" loading="lazy" onClick={() => onImageClick?.(resolved)} onError={(e) => { e.target.style.display = 'none'; }}/>);
            })}
        </div>) : null}

      {/* Render generated video AFTER typewriter animation completes */}
      {!isTyping && media?.video_url ? (<div className="mt-3 hp-fade-in">
          <video src={media.video_url} controls playsInline className="w-full max-w-xl rounded-xl border border-white/10 bg-black/20"/>
        </div>) : null}
    </div>);
}
/**
 ...
 */
function VoiceLightbox({ src, onClose }) {
    ...
    return (<div className="fixed inset-0 z-50 bg-black/95 backdrop-blur-sm flex flex-col" onClick={onClose}>
      ...
      <div className="absolute top-0 left-0 right-0 flex items-center justify-between p-4 bg-gradient-to-b from-black/80 to-transparent z-10">
        <div className="flex items-center gap-2">
          <button onClick={(e) => { e.stopPropagation(); handleDownload(); }} className="p-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors text-white" title="Download">
            <Download size={18}/>
          </button>
          <div className="w-px h-6 bg-white/20"/>
          <button onClick={(e) => { e.stopPropagation(); setZoom((z) => Math.max(z - 25, 25)); }} className="p-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors text-white" title="Zoom out" disabled={zoom <= 25}>
            <ZoomOut size={18}/>
          </button>
          <span className="text-sm font-medium px-2 min-w-[4rem] text-center text-white">
            {zoom}%
          </span>
          <button onClick={(e) => { e.stopPropagation(); setZoom((z) => Math.min(z + 25, 300)); }} className="p-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors text-white" title="Zoom in" disabled={zoom >= 300}>
            <ZoomIn size={18}/>
          </button>
          <button onClick={(e) => { e.stopPropagation(); setZoom(100); }} className="p-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors text-white" title="Reset zoom">
            <Maximize2 size={18}/>
          </button>
        </div>
        <button onClick={onClose} className="p-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors text-white" aria-label="Close">
          <X size={24}/>
        </button>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-16 overflow-auto">
        <img src={resolvedSrc} onClick={(e) => e.stopPropagation()} className="rounded-lg shadow-2xl border border-white/10 transition-transform" style={{
...
export default function VoiceModeGrok({ onSendText, onClose, onNewChat, messages: controlledMessages, setMessages: setControlledMessages, }) {
    ...
    const historyIdsRef = useRef(null);
    ...
    const [activeVoice, setActiveVoice] = useState(() => {
        if (typeof window !== 'undefined') {
            ...
            if (savedId) {
                const voice = getVoiceById(savedId);
                if (voice)
                    return voice;
            }
        }
    ...
    const [activePersonality, setActivePersonality] = useState(() => {
        if (typeof window !== 'undefined') {
            ...
            if (savedId) {
                ...
                const personality = getPersonalityById(savedId);
                if (personality)
                    return personality;
            }
        }
    ...
    function handleVoiceInput(text) {
        ...
        if (!isControlled) {
            const userMsg = { id: `user-${Date.now()}`, role: 'user', text: text.trim() };
    ...
    const prevPersonalityRef = useRef(activePersonality.id);
    ...
    useEffect(() => {
        ...
        window.addEventListener('hp:assistant_message', handler);
        return () => window.removeEventListener('hp:assistant_message', handler);
    }, [isControlled]);
    ...
    const glowOpacity = isSpeaking ? 0.38 : // Brighter glow when AI speaks
        isThinking ? 0.18 :
            isListening ? 0.15 :
                isIdle ? 0.10 :
                    0.06;
    // Typing speed synced with TTS state for premium feel
    const typingSpeed = isSpeaking ? 7 : // Fast while TTS is speaking
    ...
    if (!window.SpeechService) {
        return (<div className="hp-voice-root flex items-center justify-center">
        <Starfield />
    ...
    return (<div className="hp-voice-root">
      {/* Settings Modal - Contains advanced audio settings */}
      <SettingsModal isOpen={showSystemSettings} onClose={() => setShowSystemSettings(false)} showAudioMeter={showAudioMeter} setShowAudioMeter={setShowAudioMeter} browserVoices={voice.voices} selectedBrowserVoice={voice.selectedVoice} setSelectedBrowserVoice={voice.setSelectedVoice}/>
      ...
      <header className="absolute top-0 right-0 z-30 p-4 flex items-center gap-2 hp-fade-in">
        <button onClick={() => setShowSystemSettings(true)} className="w-9 h-9 flex items-center justify-center rounded-full bg-white/5 border border-white/10 text-white/60 hover:bg-white/10 hover:text-white transition-colors" title="System Settings">
          <Settings size={18}/>
        </button>
        <button onClick={clearConversation} className="w-9 h-9 flex items-center justify-center rounded-full bg-white/5 border border-white/10 text-white/60 hover:bg-white/10 hover:text-white transition-colors" title="New Chat">
          <PenLine size={16}/>
        </button>
      </header>

      {/* Center Content Area */}
      <div className="flex-1 flex flex-col items-center px-6 overflow-y-auto pb-48 hp-scroll relative z-10" style={{ minHeight: 0 }} ref={scrollRef}>
        {messages.length === 0 ? (
        /* Idle State */
        <div className="flex-1 flex flex-col items-center justify-center gap-4 text-white/50 hp-fade-in">
            {/* Waveform icon */}
            <div className="flex gap-1 items-end h-6">
              <div className="w-[3px] bg-white/60 rounded-full hp-pulse" style={{ height: '8px', animationDelay: '0s' }}/>
              <div className="w-[3px] bg-white/60 rounded-full hp-pulse" style={{ height: '16px', animationDelay: '0.1s' }}/>
              <div className="w-[3px] bg-white/60 rounded-full hp-pulse" style={{ height: '24px', animationDelay: '0.2s' }}/>
              <div className="w-[3px] bg-white/60 rounded-full hp-pulse" style={{ height: '16px', animationDelay: '0.3s' }}/>
              <div className="w-[3px] bg-white/60 rounded-full hp-pulse" style={{ height: '8px', animationDelay: '0.4s' }}/>
            </div>
            ...
            {!voice.sttSupported && (<div className="flex items-center gap-2 text-sm text-amber-300/80 max-w-md text-center px-4 py-2 rounded-lg bg-amber-500/10 border border-amber-500/20">
                <AlertCircle size={16} className="shrink-0"/>
                <span>Speech recognition not supported in this browser. Try Chrome (HTTPS/localhost).</span>
              </div>)}
            {voice.lastError && (<div className="flex items-center gap-2 text-sm text-red-300/80 max-w-md text-center px-4 py-2 rounded-lg bg-red-500/10 border border-red-500/20">
                <AlertCircle size={16} className="shrink-0"/>
                <span>{getErrorMessage(voice.lastError)}</span>
              </div>)}

            {/* Hands-free mode indicator */}
            {voice.isHandsFree && voice.state === 'IDLE' && (<div className="flex items-center gap-2 text-xs text-white/40">
                <Zap size={12}/>
        ...
        <div className="w-full max-w-3xl space-y-6 pt-16 hp-slide-up">
            {messages.map((msg, idx) => {
                ...
                if (msg.callMemory) {
                    const cm = msg.callMemory;
                    return (<PhoneCallDivider key={msg.id} durationSec={cm.durationSec ?? 0} endedAt={cm.endedAt}/>);
                }
                return (<div key={msg.id} className={msg.role === 'user' ? 'flex justify-end' : ''}>
                  {msg.role === 'user' ? (<div className="hp-message-user">
                      <p className="text-white/90 text-[17px] leading-relaxed">{msg.text}</p>
                    </div>) : (<RenderTypedMessage fullText={msg.text} typingSpeed={typingSpeed} onProgress={scrollToBottom} animate={idx === messages.length - 1 && !historyIdsRef.current?.has(msg.id)} media={msg.media} onImageClick={(src) => setLightbox(src)}/>)}
                </div>);
            })}

            {/* Show interim text while listening */}
            {voice.interimText && (<div className="flex justify-end">
        ...
        <div className="max-w-3xl mx-auto">
          <div className="hp-voicebar">
            ...
            <div className="hp-glow-wrap absolute -left-[60px] -right-[60px] -bottom-[62px] h-[100px] overflow-hidden rounded-b-[28px]">
              <div className={`absolute inset-0 transition-opacity duration-500 ${isSpeaking ? 'hp-glow-breath' : ''}`} style={{ opacity: glowOpacity }}>
            ...
            <div className="relative p-4 pb-16">
              ...
              {voice.isHandsFree && voice.state !== 'OFF' && showAudioMeter && (<div className="mb-3 px-1">
                  <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all duration-100 ${isListening
                ? 'bg-gradient-to-r from-green-400 to-green-300'
                : 'bg-white/50'}`} style={{ width: `${Math.min(voice.audioLevel * 500, 100)}%` }}/>
                  </div>
              ...
              <form onSubmit={handleSubmit}>
                <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder={isListening
            ? 'Listening...'
            : voice.isHandsFree
                ? 'Hands-free mode - speak anytime...'
                : 'How can HomePilot help?'} className="hp-input"/>
              </form>

              {/* Controls Row */}
              <div className="absolute left-3 right-3 bottom-3 flex items-center justify-between" ref={menuRef}>
                <div className="flex items-center gap-2 relative">
                  {/* Mic Button with Mini Level Meter */}
                  <button onClick={toggleMic} className={`h-10 px-4 rounded-full border flex items-center gap-2.5 transition-all ${isListening
            ? 'bg-[#97C4FF]/20 border-[#97C4FF]/40 text-white'
            : 'bg-white/5 border-white/10 text-white/70 hover:bg-white/10'}`}>
                    <MiniStudioMeter audioLevel={voice.audioLevel} isUserActive={voice.state === 'LISTENING' || voice.state === 'IDLE'} isAiSpeaking={voice.state === 'SPEAKING'}/>
                    <Mic size={18}/>
                  </button>

                  {/* Volume Toggle */}
                  <button onClick={() => setIsMuted(!isMuted)} className="w-10 h-10 rounded-full border border-white/10 bg-white/5 flex items-center justify-center text-white/70 hover:bg-white/10 transition-colors">
                    {isMuted ? (<VolumeX size={18} className="opacity-50"/>) : (<Volume2 size={18}/>)}
                  </button>

                  {/* Voice Settings Trigger */}
                  <button onClick={() => setShowVoiceSettings(!showVoiceSettings)} className={`h-10 px-3.5 rounded-full border flex items-center gap-2 transition-all text-sm ${showVoiceSettings
            ...
            : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                    ...
                    <ChevronDown size={14} className={`text-white/40 transition-transform ${showVoiceSettings ? 'rotate-180' : ''}`}/>
                  </button>

                  {/* Voice Settings Panel */}
                  {/* Voice Settings Panel - Voice persona, personality, speed */}
                  <VoiceSettingsPanel isOpen={showVoiceSettings} activeVoice={activeVoice} setActiveVoice={setActiveVoice} activePersonality={activePersonality} setActivePersonality={setActivePersonality} speed={speed} setSpeed={setSpeed}/>
                </div>

                {/* Hands-Free Toggle (Auto/Manual) - Replaces close button */}
                <button onClick={() => voice.setHandsFree(!voice.isHandsFree)} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all shadow-lg ${voice.isHandsFree
            ? 'bg-white text-black hover:bg-gray-200'
            : 'bg-white/10 border border-white/20 text-white/80 hover:bg-white/20'}`} title={voice.isHandsFree
            ? 'Hands-free mode ON (Auto) - Click to switch to Manual'
            : 'Manual mode - Click to enable Hands-free (Auto)'}>
                  {voice.isHandsFree ? (<Zap size={20}/>) : (<Radio size={20}/>)}
                </button>
              </div>
            </div>
          </div>
      ...
      {lightbox && (<VoiceLightbox src={lightbox} onClose={() => setLightbox(null)}/>)}
    </div>);
}
