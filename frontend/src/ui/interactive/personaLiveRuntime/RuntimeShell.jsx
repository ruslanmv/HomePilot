...
import { Eye, EyeOff, Loader2, Lock, Menu, Send, Volume2, VolumeX, X, Zap, } from "lucide-react";
...
function mapEmotionToExpression(emotion) {
    ...
    if (e.includes("play") || e.includes("flirt"))
        return "smirk";
    if (e.includes("warm") || e.includes("happy"))
        return "smile";
    if (e.includes("shy"))
        return "blush";
    return "idle";
...
function SceneBadge({ scene }) {
    return (<div className="absolute top-3 left-3 px-3 py-1 bg-black/50 rounded-full text-xs border border-white/10">
      {scene.icon || "📍"} {scene.label || "Unknown scene"}
    </div>);
}
function ConversationOverlay({ messages }) {
    if (!messages.length)
        return null;
    return (<div className="space-y-2">
      {messages.map((message, index) => {
            const isUser = message.sender === "user";
            return (<div key={`${message.sender}-${index}-${message.text.slice(0, 12)}`} className={isUser ? "flex justify-end" : "flex justify-start"}>
            <div className={[
...
function useSceneTransition(type) {
    if (type === "scene")
        return "duration-300 ease-out scale-[1.01] saturate-105";
    if (type === "hard")
        return "duration-300 ease-in opacity-95";
    return "duration-150 ease-linear";
}
export function RuntimeShell({ api, experience, onExit }) {
    ...
    const [transitionType, setTransitionType] = useState("soft");
    ...
    const overlayMessages = useMemo(() => {
        ...
        return dialogueText ? [{ sender: "ai", text: dialogueText }] : [];
    ...
    useEffect(() => {
        ...
        setCharacterState((prev) => ({
            ...
            emotion: state.emotionalState.mood || "neutral",
        }));
    ...
    function applyCharacterTurn(opts) {
        ...
        setCharacterState((prev) => ({
            ...prev,
            emotion: opts.emotion || prev.emotion,
    ...
    async function fireAction(actionId) {
        ...
        try {
            ...
            setTransitionType((res.scene_context ? "scene" : "soft"));
            ...
            applyCharacterTurn({
                ...
                aiText: dialogueText,
                emotion: (res.emotional_state?.mood || state?.emotionalState.mood || "neutral"),
            });
            setState((prev) => {
                if (!prev)
                    return prev;
                return {
                    ...
                    sceneContext: res.scene_context || prev.sceneContext,
                    sceneMemory: res.scene_memory || prev.sceneMemory,
                    emotionalState: res.emotional_state || prev.emotionalState,
                };
            });
    ...
    return (<div className="h-full min-h-0 flex flex-col bg-black">
      ...
      <div className="lg:hidden relative flex-1 min-h-0 overflow-hidden">
        <div className={`absolute inset-0 transition-all ${transitionClass}`} style={{ transform: `scale(${cameraPreset.zoom}) translateY(${cameraPreset.yOffset}%)` }}>
          {state.currentMedia.url ? (state.currentMedia.type === "video" ? (<video src={state.currentMedia.url} className="absolute inset-0 w-full h-full object-cover" autoPlay loop muted={muted} playsInline/>) : (<img src={state.currentMedia.url} alt={state.persona.name} className="absolute inset-0 w-full h-full object-cover"/>)) : (<div className="absolute inset-0 grid place-items-center text-[#888] text-sm">No render yet</div>)}
        </div>
        ...
        <button type="button" onClick={() => setShowMobileDrawer(true)} className="absolute right-3 bottom-24 z-20 inline-flex items-center gap-1.5 px-3 py-2 rounded-full bg-[#8b5cf6]/90 text-white text-xs shadow-lg">
        ...
        <div className="absolute left-0 right-0 bottom-0 z-20 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
          <div className="flex items-center gap-2 rounded-2xl bg-black/55 backdrop-blur border border-white/15 px-2 py-2">
            <input value={chat} onChange={(e) => setChat(e.target.value)} placeholder="Message her..." className="flex-1 bg-transparent text-sm outline-none px-2"/>
        ...
        {showMobileDrawer && (<div className="absolute inset-0 z-30">
            ...
            <div className="absolute left-0 right-0 bottom-0 rounded-t-2xl border-t border-white/10 bg-[#101010] p-4 max-h-[65vh] overflow-y-auto">
              ...
              {mobileTab === "actions" ? (<>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {state.actions.available.map((a) => (<button key={a.id} type="button" onClick={() => { void fireAction(a.id); setShowMobileDrawer(false); }} disabled={sending || !!pendingJobId} className="text-left rounded-md border border-[#3a3a3a] bg-[#1a1a1a] px-3 py-2 text-sm disabled:opacity-50">{a.label}</button>))}
                  </div>
                  <div className="text-xs text-[#aaa] mt-4 mb-2">Locked</div>
                  <div className="grid grid-cols-1 gap-2">
                    {state.actions.locked.map((a) => (<div key={a.id} className="rounded-md border border-[#2d2d2d] bg-[#151515] px-3 py-2 text-sm text-[#7c7c7c] flex items-center justify-between">
                        <span>{a.label}</span>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {state.versions.slice(0, 8).map((v) => (<button key={v.id} type="button" onClick={() => { void restore(v.id); setShowMobileDrawer(false); }} className={["relative w-full aspect-square rounded border overflow-hidden", v.active ? "border-[#8b5cf6]" : "border-[#3a3a3a]"].join(" ")}>
      ...
      <div className="hidden lg:grid flex-1 min-h-0 lg:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)] gap-4 p-4">
        <div className="min-h-0 flex flex-col gap-3">
          <div className="relative flex-1 min-h-[320px] rounded-2xl border border-[#343434] overflow-hidden bg-[#101010]">
            <div className={`absolute inset-0 transition-all ${transitionClass}`} style={{ transform: `scale(${cameraPreset.zoom}) translateY(${cameraPreset.yOffset}%)` }}>
              {state.currentMedia.url ? (state.currentMedia.type === "video" ? (<video src={state.currentMedia.url} className="absolute inset-0 w-full h-full object-cover" autoPlay loop muted playsInline/>) : (<img src={state.currentMedia.url} alt={state.persona.name} className="absolute inset-0 w-full h-full object-cover"/>)) : (<div className="absolute inset-0 grid place-items-center text-[#666] text-sm">No render yet</div>)}
            </div>
            ...
            <div className="absolute left-4 right-4 bottom-4 flex items-center gap-2 rounded-full bg-black/55 border border-white/15 backdrop-blur px-2 py-2">
              <input value={chat} onChange={(e) => setChat(e.target.value)} placeholder="Ask anything" className="flex-1 bg-transparent text-sm outline-none px-2"/>
            ...
            {state.currentMedia.status === "rendering" && (<div className="absolute inset-0 bg-black/35 grid place-items-center pointer-events-none">
        ...
        <div className="min-h-0 overflow-y-auto rounded-2xl border border-[#2f2f2f] bg-[#121212] p-5 space-y-4">
          ...
          <div className="space-y-2">
            {state.actions.available.map((a) => (<button key={a.id} type="button" onClick={() => fireAction(a.id)} disabled={sending || !!pendingJobId} className="w-full text-left rounded-xl border border-[#2f2f2f] bg-[#1a1a1a] hover:border-[#8b5cf6] px-4 py-3 text-sm disabled:opacity-50 flex items-center justify-between">
                <span>{a.label}</span>
                <span className="w-8 h-8 rounded-full bg-[#ec4899] text-white grid place-items-center">▶</span>
              </button>))}
            {state.actions.locked.map((a) => (<div key={a.id} className="w-full rounded-xl border border-[#2a2a2a] bg-[#171717] px-4 py-3 text-sm text-[#8d8d8d] flex items-center justify-between">
                <span>{a.label}</span>
          ...
          <div className="pt-2">
            ...
            <div className="grid grid-cols-4 gap-2">
              {state.versions.slice(0, 8).map((v) => (<button key={v.id} type="button" onClick={() => restore(v.id)} className={["relative w-full aspect-square rounded border overflow-hidden", v.active ? "border-[#8b5cf6]" : "border-[#3a3a3a]"].join(" ")}>
                  {v.thumb_url ? <img src={v.thumb_url} alt={v.id} className="w-full h-full object-cover"/> : <div className="w-full h-full bg-[#222]"/>}
                </button>))}
            </div>
          </div>
...
function _mapRuntime(payload) {
    const persona = (payload.persona || {});
    const session = (payload.session || {});
    const currentMedia = (payload.current_media || {});
    const actions = (payload.actions || {});
    const dialogue = (payload.dialogue || {});
    const versions = Array.isArray(payload.versions) ? payload.versions : [];
    const sceneContext = (payload.scene_context || {});
    const sceneMemory = (payload.scene_memory || {});
    ...
    return {
        ...
        currentMedia: {
            ...
            status: String(currentMedia.status || "idle"),
        },
        ...
        sceneMemory: {
            ...
            emotional_state: {
                mood: String(sceneMemory.emotional_state?.mood || "guarded"),
                intensity: Number(sceneMemory.emotional_state?.intensity || 25),
            },
        },
        emotionalState: {
        ...
        actions: {
            ...
            locked: (Array.isArray(actions.locked) ? actions.locked : []),
        },
        versions: versions,
    };
}
