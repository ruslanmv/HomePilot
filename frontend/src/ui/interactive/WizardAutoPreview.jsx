 ...
 * 'Smart defaults' when the heuristic fallback fired.
 *
 * Production fixes in this version:
 *   - Persona-linked experiences persist exact persona image URLs.
 *   - Persona live defaults to image rendering unless overridden.
 *   - Payload generation is centralized and trimmed consistently.
 *   - Submitting state resets cleanly on all failure paths.
 */
...
export function WizardAutoPreview({
  ...
  const buildCreatePayload = useCallback(() => {
    const renderMediaType =
      cleanOptional(form.render_media_type) ||
      interaction.render_media_type ||
      (personaLive ? "image" : "video");

    return {
      title: String(form.title || "").trim(),
      description: String(form.prompt || "").trim(),
      ...
      audience_profile: {
        ...
        locale_hint: cleanOptional(form.audience_locale_hint),
        ...
        persona_image_fit: cleanOptional(interaction.persona_image_fit) || (personaLive ? "contain" : undefined),
        persona_image_position:
          cleanOptional(interaction.persona_image_position) || (personaLive ? "center" : undefined),
        ...
        <div className="px-4 pb-4 grid grid-cols-2 gap-3 text-sm">
          ...
          <Field label="Render media">
            <select
              ...
              onChange={(e) => patch("render_media_type", e.target.value)}
              disabled={disabled}
              className={INPUT_CLS}
            >
