...
import { ArrowLeft, ArrowRight, Maximize, Minimize, Pause, Play, RotateCcw, SkipBack, SkipForward, Volume2, VolumeX, } from "lucide-react";
import { useAsyncResource, useToast } from "./ui";
const SKIP_SECONDS = 10;
export function StandardPlayer({ api, sessionId, scene, onExit, onResolved, }) {
    ...
    const catalog = useAsyncResource((signal) => api.getCatalog(sessionId, signal), [api, sessionId]);
    ...
    const skipBy = useCallback((delta) => {
        if (isImage) {
            setCurrentTime((prev) => {
                ...
                if (target >= duration && choices.length > 0)
                    setDecisionOpen(true);
                return target;
            });
            return;
        ...
        if (!v)
            return;
        const target = Math.max(0, Math.min((v.duration || 0) - 0.1, (v.currentTime || 0) + delta));
    ...
    const fireChoice = useCallback(async (action) => {
    ...
    return (<div ref={containerRef} className="relative w-full h-full bg-black text-white flex items-center justify-center">
      {isVideo ? (<video ref={videoRef} src={url} autoPlay muted={muted} playsInline className="w-full h-full object-contain" onClick={togglePlay}/>) : isImage ? (imageError ? (<div className="w-full h-full flex flex-col items-center justify-center text-white/75 text-sm gap-3">
          ...
          </div>) : (<img src={url} className="w-full h-full object-contain animate-[pulse_12s_ease-in-out_infinite]" alt={scene?.prompt || "Scene"} onError={() => setImageError(true)}/>)) : (<div className="w-full h-full flex items-center justify-center text-white/60 text-sm">
      ...
      <CenterControls playing={playing} onPlay={togglePlay} onBack={() => skipBy(-SKIP_SECONDS)} onForward={() => skipBy(SKIP_SECONDS)}/>
      <BottomBar playing={playing} muted={muted} onTogglePlay={togglePlay} onToggleMute={() => setMuted((m) => !m)} pct={pct} currentTime={currentTime} duration={duration} onSeek={onSeek} isFullscreen={isFullscreen} onToggleFullscreen={toggleFullscreen}/>

      {decisionOpen && choices.length > 0 && (<DecisionModal choices={choices} firing={firing} onPick={fireChoice} onClose={() => setDecisionOpen(false)}/>)}
    </div>);
}
// ── Controls ────────────────────────────────────────────────────
function TopBar({ onRestart, onNext, onExit, }) {
    return (<div className="absolute top-0 inset-x-0 z-20 px-4 py-3 flex items-center justify-between pointer-events-none">
...
function CenterControls({ playing, onPlay, onBack, onForward, }) {
    // Center playback + scrubbing overlay. Mirrors the reference
    // screenshot's three-icon cluster: back-10, play/pause, fwd-10.
    return (<div className="absolute inset-0 z-10 flex items-center justify-center gap-8 pointer-events-none">
      <button type="button" onClick={onBack} aria-label={`Back ${SKIP_SECONDS} seconds`} className="pointer-events-auto w-14 h-14 rounded-full bg-black/40 hover:bg-black/60 border border-white/30 flex items-center justify-center backdrop-blur-sm transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white">
        <span className="relative inline-flex items-center justify-center">
          <SkipBack className="w-6 h-6"/>
          <span className="absolute text-[10px] font-bold pointer-events-none">10</span>
        </span>
      </button>
      <button type="button" onClick={onPlay} aria-label={playing ? "Pause" : "Play"} className="pointer-events-auto w-20 h-20 rounded-full bg-black/40 hover:bg-black/60 border border-white/30 flex items-center justify-center backdrop-blur-sm transition-transform hover:scale-105 focus:outline-none focus-visible:ring-2 focus-visible:ring-white">
        {playing
            ? <Pause className="w-9 h-9 fill-current"/>
            : <Play className="w-9 h-9 fill-current ml-1"/>}
      </button>
      <button type="button" onClick={onForward} aria-label={`Forward ${SKIP_SECONDS} seconds`} className="pointer-events-auto w-14 h-14 rounded-full bg-black/40 hover:bg-black/60 border border-white/30 flex items-center justify-center backdrop-blur-sm transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white">
        <span className="relative inline-flex items-center justify-center">
...
function BottomBar({ playing, muted, onTogglePlay, onToggleMute, pct, currentTime, duration, onSeek, isFullscreen, onToggleFullscreen, }) {
    return (<div className="absolute bottom-0 inset-x-0 z-20 px-4 py-3 bg-gradient-to-t from-black/80 to-transparent">
      <input type="range" min={0} max={100} step={0.1} value={pct} onChange={onSeek} aria-label="Seek" className="w-full h-1.5 accent-[#ec4899] cursor-pointer" style={{
...
function IconBtn({ children, onClick, label, }) {
    return (<button type="button" onClick={onClick} aria-label={label} className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white">
      {children}
    </button>);
}
// ── Decision modal ─────────────────────────────────────────────
function DecisionModal({ choices, firing, onPick, onClose, }) {
    ...
    return (<div role="dialog" aria-modal="true" aria-label="What to do next?" className="absolute inset-0 z-30 flex items-center justify-center p-6 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-3xl rounded-3xl bg-black/60 border border-white/15 backdrop-blur-md px-6 py-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className={[
          ...
          {choices.slice(0, 4).map((c) => (<ChoiceCard key={c.id} choice={c} firing={firing === c.id} onClick={() => onPick(c)}/>))}
        </div>
      </div>
    </div>);
}
function ChoiceCard({ choice, firing, onClick, }) {
    return (<button type="button" onClick={onClick} disabled={firing} aria-label={`Pick: ${choice.label}`} className={[
      ...
      <div className="absolute inset-0 opacity-80 group-hover:opacity-100 transition-opacity" style={{
      ...
      <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/85 to-transparent">
        ...
        {choice.intent_code && (<div className="text-[10px] text-white/60 truncate mt-0.5">
            {choice.intent_code}
          </div>)}
      </div>
    </button>);
