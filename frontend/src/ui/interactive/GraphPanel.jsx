...
import { ArrowRight, Check, GitBranch, Image as ImageIcon, Loader2, Pencil, Play, RotateCw, X, } from "lucide-react";
import { GeneratingPanel } from "./GeneratingPanel";
import { EmptyState, ErrorBanner, Panel, PrimaryButton, SecondaryButton, SkeletonRow, useAsyncResource, useToast, } from "./ui";
const KIND_ORDER = [
...
export function GraphPanel({ api, projectId }) {
    ...
    const missingAssetCount = useMemo(() => {
        const nodes = resource.data?.nodes || [];
        return nodes.filter((n) => RENDERABLE_KINDS.has(n.kind) && (n.asset_ids || []).length === 0).length;
    }, [resource.data]);
    ...
    const handleSavePatch = useCallback(async (nodeId, patch) => {
        try {
    ...
    return (<Panel title="Scene graph" subtitle="Edit, regenerate, or preview any scene. Changes save as you go." actions={missingAssetCount > 0 ? (<PrimaryButton onClick={regenerateAllMissing} icon={<RotateCw className="w-3.5 h-3.5" aria-hidden/>} size="sm">
            Regenerate all ({missingAssetCount} missing)
          </PrimaryButton>) : undefined}>
      {resource.error ? (<ErrorBanner title="Couldn't load the graph" message={resource.error} onRetry={resource.reload}/>) : resource.loading && !resource.data ? (<div className="flex flex-col gap-2" aria-busy="true">
          {Array.from({ length: 4 }).map((_, i) => <SkeletonRow key={i}/>)}
        </div>) : (resource.data?.nodes || []).length === 0 ? (<EmptyState icon={<GitBranch className="w-12 h-12" aria-hidden/>} title="No nodes yet" description="The planner didn't seed a graph. Re-run the wizard, or open the Catalog tab to add scenes manually."/>) : (<div className="flex flex-col gap-5">
          {KIND_ORDER.map((kind) => {
                ...
                <ul className="flex flex-col gap-2">
                  {bucket.map((node) => (<NodeRow key={node.id} node={node} outbound={outboundByFrom.get(node.id) || []} titleById={titleById} onSave={handleSavePatch} onRegenerate={handleRegenerate} onPreview={() => setPreviewNode(node)}/>))}
                </ul>
              </section>);
            })}
        </div>)}

      {bulkOpen && (<GeneratingPanel title={bulkTotal > 0
                ? `Regenerating scenes · ${bulkDone} of ${bulkTotal}`
                : "Regenerating scenes"} description={bulkTitle ? `Now rendering: ${bulkTitle}` : "Starting the render pipeline…"} spinnerSize="large" accentClassName="text-[#c4b5fd]" progress={bulkTotal > 0 ? Math.round((bulkDone / bulkTotal) * 100) : undefined} progressLabel={bulkTotal > 0 ? `${bulkDone} / ${bulkTotal} scenes` : undefined}/>)}

      {previewNode && (<PreviewModal api={api} node={previewNode} onClose={() => setPreviewNode(null)} onRegenerate={() => {
                setPreviewNode(null);
...
function NodeRow({ node, outbound, titleById, onSave, onRegenerate, onPreview, }) {
    ...
    const saveEdit = async () => {
        ...
        if (draftNarration !== (node.narration || ""))
            patch.narration = draftNarration;
        if (draftImagePrompt !== (node.image_prompt || ""))
            patch.image_prompt = draftImagePrompt;
    ...
    return (<li className="bg-[#121212] border border-[#3f3f3f] rounded-md p-3 flex flex-col gap-2">
      {editing ? (<div className="flex flex-col gap-2">
          <FieldRow label="Title">
            <input type="text" value={draftTitle} onChange={(e) => setDraftTitle(e.target.value)} className="w-full bg-[#0f0f0f] border border-[#3f3f3f] rounded-md px-2.5 py-1.5 text-sm outline-none focus:border-[#3ea6ff]" autoFocus/>
          </FieldRow>
          <FieldRow label="Narration">
            <textarea value={draftNarration} onChange={(e) => setDraftNarration(e.target.value)} rows={3} className="w-full bg-[#0f0f0f] border border-[#3f3f3f] rounded-md px-2.5 py-1.5 text-sm outline-none focus:border-[#3ea6ff] resize-y"/>
          </FieldRow>
          <FieldRow label="Image prompt">
            <textarea value={draftImagePrompt} onChange={(e) => setDraftImagePrompt(e.target.value)} rows={2} placeholder="Optional visual direction for the renderer" className="w-full bg-[#0f0f0f] border border-[#3f3f3f] rounded-md px-2.5 py-1.5 text-sm outline-none focus:border-[#3ea6ff] resize-y"/>
          </FieldRow>
          <div className="flex items-center justify-end gap-2 mt-1">
            <SecondaryButton onClick={() => setEditing(false)} size="sm" disabled={saving}>
              Cancel
            </SecondaryButton>
            <PrimaryButton onClick={saveEdit} size="sm" icon={saving
                ? <Loader2 className="w-3.5 h-3.5 animate-spin" aria-hidden/>
                : <Check className="w-3.5 h-3.5" aria-hidden/>} disabled={saving}>
              Save
            </PrimaryButton>
          </div>
          ...
          <div className="min-w-0 flex-1">
            ...
            {node.narration && (<p className="text-xs text-[#aaa] mt-0.5 line-clamp-2">{node.narration}</p>)}
            {!hasAsset && canRender && (<div className="text-[11px] text-amber-300/90 mt-1 inline-flex items-center gap-1">
          ...
          <div className="flex items-center gap-1 shrink-0">
            {canRender && (<>
                ...
                <IconAction label={regenerating ? "Regenerating…" : "Regenerate"} onClick={kickRegenerate} disabled={regenerating}>
                  {regenerating
                    ? <Loader2 className="w-3.5 h-3.5 animate-spin" aria-hidden/>
      ...
      {outbound.length > 0 && !editing && (<div className="flex items-center gap-2 flex-wrap">
          <ArrowRight className="w-3.5 h-3.5 text-[#777]" aria-hidden/>
          {outbound
                .slice()
                .sort((a, b) => (a.ordinal || 0) - (b.ordinal || 0))
                .map((edge) => (<span key={edge.id} className="text-[11px] bg-[#1f1f1f] border border-[#3f3f3f] rounded px-2 py-0.5 text-[#cfd8dc]" title={`edge ${edge.id} · ${edge.trigger_kind}`}>
...
function IconAction({ children, label, onClick, disabled, }) {
    return (<button type="button" onClick={onClick} aria-label={label} title={label} disabled={disabled} className="w-7 h-7 rounded-md hover:bg-white/10 text-[#aaa] hover:text-[#f1f1f1] flex items-center justify-center transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#3ea6ff] disabled:opacity-50 disabled:cursor-not-allowed">
      {children}
    </button>);
}
// ── Preview modal (EDIT-4) ────────────────────────────────────
function PreviewModal({ api, node, onClose, onRegenerate, }) {
    ...
    const urlRes = useAsyncResource((signal) => assetId ? api.resolveAssetUrl(assetId, signal) : Promise.resolve(null), [api, assetId]);
    const src = urlRes.data || "";
    const looksLikeVideo = /\.(mp4|webm|mov|mkv|m4v)(\?|$)/i.test(src);
    return (<div role="dialog" aria-modal="true" aria-label={`Preview: ${node.title}`} className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/75 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-2xl rounded-xl bg-[#0f0f0f] border border-[#3f3f3f] overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <header className="flex items-center justify-between gap-4 px-4 py-3 border-b border-[#2a2a2a]">
          ...
          <button type="button" onClick={onClose} aria-label="Close preview" className="w-8 h-8 rounded-md hover:bg-white/10 flex items-center justify-center">
            <X className="w-4 h-4" aria-hidden/>
          </button>
        </header>
        <div className="aspect-video bg-black flex items-center justify-center">
          {urlRes.loading ? (<Loader2 className="w-6 h-6 text-[#555] animate-spin" aria-hidden/>) : !src ? (<div className="text-center p-8">
            ...
            </div>) : looksLikeVideo ? (<video src={src} controls autoPlay className="max-w-full max-h-full"/>) : (<img src={src} alt={node.title} className="max-w-full max-h-full object-contain"/>)}
        </div>
        {node.narration && (<div className="px-4 py-3 border-t border-[#2a2a2a] text-sm text-[#cfd8dc]">
            {node.narration}
          </div>)}
        <footer className="flex items-center justify-end gap-2 px-4 py-3 border-t border-[#2a2a2a]">
          ...
          <PrimaryButton onClick={onRegenerate} size="sm" icon={<RotateCw className="w-3.5 h-3.5" aria-hidden/>}>
            Regenerate
          </PrimaryButton>
        </footer>
      </div>
