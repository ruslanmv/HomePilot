...
export function PrimaryButton({ size = "md", loading, icon, children, className, disabled, ...rest }) {
    return (<button type="button" disabled={disabled || loading} className={[
...
export function SecondaryButton({ size = "md", loading, icon, children, className, disabled, ...rest }) {
    return (<button type="button" disabled={disabled || loading} className={[
            "inline-flex items-center gap-2 rounded-md font-medium",
...
export function DangerButton({ size = "md", loading, icon, children, className, disabled, ...rest }) {
    return (<button type="button" disabled={disabled || loading} className={[
            "inline-flex items-center gap-2 rounded-md font-medium",
...
export function Panel({ title, subtitle, actions, children, className, }) {
    return (<section className={[
      ...
      {(title || actions) && (<header className="flex items-start justify-between gap-3 px-5 py-4 border-b border-[#3f3f3f]">
          <div>
...
export function EmptyState({ icon, title, description, action, }) {
    return (<div className="border border-dashed border-[#3f3f3f] rounded-lg p-10 text-center bg-[#151515]">
      <div className="w-12 h-12 mx-auto text-[#3ea6ff] mb-3" aria-hidden>
        {icon}
      </div>
      <div className="text-base font-medium text-[#f1f1f1]">{title}</div>
      {description && (<p className="text-sm text-[#aaa] mt-1 max-w-md mx-auto">{description}</p>)}
...
export function SkeletonCard() {
    return (<div className="bg-[#1f1f1f] border border-[#3f3f3f] rounded-lg p-5 animate-pulse" role="status" aria-label="Loading project">
...
export function SkeletonRow() {
    return (<div className="bg-[#1f1f1f] border border-[#3f3f3f] rounded p-3 animate-pulse flex gap-3 items-center" role="status" aria-label="Loading row">
...
export function ErrorBanner({ title = "Something went wrong", message, onRetry, }) {
    return (<div role="alert" className="border border-red-500/40 bg-red-500/5 rounded-md p-4 flex gap-3 items-start">
      ...
      <div className="flex-1 min-w-0">
        ...
        {onRetry && (<button type="button" onClick={onRetry} className="mt-3 text-xs font-medium text-[#3ea6ff] hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-[#3ea6ff] rounded">
            Try again
          </button>)}
      </div>
    </div>);
...
export function ToastProvider({ children }) {
    ...
    return (<ToastContext.Provider value={ctx}>
      {children}
      <div aria-live="polite" aria-atomic="false" className="fixed bottom-4 right-4 z-[60] flex flex-col gap-2 pointer-events-none">
...
function ToastCard({ item, onClose }) {
    ...
    return (<div role="status" className={[
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.message && (<div className="text-xs text-[#cfd8dc] mt-0.5 break-words">{item.message}</div>)}
      </div>
      <button type="button" onClick={onClose} aria-label="Dismiss notification" className="text-[#aaa] hover:text-[#f1f1f1] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#3ea6ff] rounded">
        <X className="w-4 h-4"/>
      </button>
    </div>);
}
export function useAsyncResource(load, deps) {
...
// ────────────────────────────────────────────────────────────────
export function Modal({ open, onClose, title, children, footer, widthClass = "max-w-lg", }) {
    useEffect(() => {
        if (!open)
            return;
        const onKey = (e) => {
    ...
    if (!open)
        return null;
    return (<div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true" aria-label={title} onClick={(e) => {
      ...
      <div className={[
        ...
        <header className="flex items-center justify-between px-5 py-3 border-b border-[#3f3f3f]">
          ...
          <button type="button" onClick={onClose} aria-label="Close dialog" className="text-[#aaa] hover:text-[#f1f1f1] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#3ea6ff] rounded p-1">
            <X className="w-4 h-4"/>
          </button>
        </header>
        <div className="px-5 py-4 max-h-[70vh] overflow-y-auto">{children}</div>
        {footer && (<footer className="flex items-center justify-end gap-2 px-5 py-3 border-t border-[#3f3f3f]">
            {footer}
          </footer>)}
      </div>
    </div>);
