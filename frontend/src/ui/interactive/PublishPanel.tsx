...
import type { Publication, PublishResult } from "./types";
import {
  EmptyState,
  ErrorBanner,
  Panel,
...
type Channel = "web_embed" | "studio_preview" | "export";
...
export function PublishPanel({ api, projectId }: PublishPanelProps) {
  const toast = useToast();
  const [channel, setChannel] = useState<Channel>("web_embed");
  const [publishing, setPublishing] = useState(false);
  const [lastResult, setLastResult] = useState<PublishResult | null>(null);

  const publications = useAsyncResource<Publication[]>(
    (signal) => api.listPublications(projectId, signal),
    [api, projectId],
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      <Panel
        title="Publish this experience"
        subtitle="Pick a channel. Publishing runs QA first — a 'fail' verdict will block the attempt."
        actions={
          <PrimaryButton
            onClick={publishNow}
            loading={publishing}
      ...
      <Panel title="Version history" subtitle="Every successful publish lands here, newest first.">
        {publications.error ? (
          ...
          <EmptyState
            icon={<Clock className="w-12 h-12" aria-hidden />}
            title="No publications yet"
            description="Publishing above will create the first version record."
          />
        ) : (
...
function ResultBanner({ result }: { result: PublishResult }) {
  const palette = ({
  ...
  return (
    <div className={["mt-4 rounded-md border p-3 flex items-start gap-3", palette.border, palette.bg].join(" ")}>
      {palette.icon}
      <div className={["text-sm flex-1 min-w-0", palette.text].join(" ")}>
        ...
        {result.status === "blocked" && result.qa && (
        ...
        {result.publication && result.status === "published" && (
          <div className="text-xs mt-1.5 text-[#aaa]">
...
function PublicationRow({ pub }: { pub: Publication }) {
  const digest = typeof pub.metadata?.digest === "string"
    ? (pub.metadata.digest as string).slice(0, 12)
    : null;
  return (
    <li className="bg-[#121212] border border-[#3f3f3f] rounded-md p-3 flex items-center justify-between gap-3">
