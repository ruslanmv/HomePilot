...
import {
  AlertTriangle, CheckCircle2, Info, Play, RefreshCw, ShieldCheck, XCircle,
} from "lucide-react";
import type { InteractiveApi } from "./api";
import { InteractiveApiError } from "./types";
import type { QAIssue, QAResult } from "./types";
import {
  ErrorBanner, Panel, PrimaryButton, SecondaryButton, useToast,
} from "./ui";
...
type SummaryState =
  | { kind: "empty" }
  | { kind: "result"; result: QAResult; at: Date | null };

export function QAPanel({ api, projectId }: QAPanelProps) {
  const toast = useToast();
  const [state, setState] = useState<SummaryState>({ kind: "empty" });
  ...
  useEffect(() => {
    ...
    api.latestReport(projectId, ctrl.signal)
      .then((resp) => {
        if (cancelled) return;
        const report = (resp.report || {}) as Record<string, unknown>;
        const summary = report.summary as { verdict?: string; counts?: QAResult["counts"] } | undefined;
        const issues = report.issues as QAIssue[] | undefined;
        const createdAt = report.created_at as string | undefined;
        if (summary?.verdict && Array.isArray(issues)) {
          setState({
            kind: "result",
            result: {
              verdict: summary.verdict as QAResult["verdict"],
      ...
      .catch((err: Error) => {
        ...
        setBootstrapError((err as InteractiveApiError).message || "Couldn't load the latest report.");
      })
  ...
  const grouped = useMemo(() => groupIssues(
    state.kind === "result" ? state.result.issues : [],
  ), [state]);

  return (
    <Panel
      title="Quality checks"
      subtitle="Structural + content checks run before publish."
      actions={
        <div className="flex gap-2">
          ...
          {state.kind === "empty" && !bootstrapping && (
            <PrimaryButton
              onClick={runNow}
              size="sm"
              loading={running}
    ...
    >
      {bootstrapError && (
        <div className="mb-4">
          <ErrorBanner
            title="Couldn't fetch the latest report"
      ...
      {bootstrapping ? (
        <div className="text-sm text-[#aaa]">Loading latest report…</div>
      ) : state.kind === "empty" ? (
...
function VerdictBanner({ result, at }: { result: QAResult; at: Date | null }) {
  const style = VERDICT_STYLES[result.verdict];
  return (
    <div className={["rounded-md border p-3 flex items-start gap-3", style.border, style.bg].join(" ")}>
      {style.icon}
...
function IssueGroup({ severity, issues }: { severity: QAIssue["severity"]; issues: QAIssue[] }) {
...
function IssueRow({ issue }: { issue: QAIssue }) {
  ...
  for (const k of ["node_id", "edge_id", "action_id", "rule_id", "profile_id"]) {
    const v = issue[k];
    if (typeof v === "string" && v) extras.push([k.replace(/_id$/, ""), v]);
  }
  return (
    <li className="bg-[#121212] border border-[#3f3f3f] rounded-md p-3">
      <div className="flex items-center gap-2 flex-wrap">
        <code className="text-[11px] bg-[#1f1f1f] border border-[#3f3f3f] rounded px-1.5 py-0.5 text-[#cfd8dc]">
          {issue.code}
        </code>
        {extras.map(([k, v]) => (
          <code key={`${k}:${v}`} className="text-[11px] bg-[#1f1f1f] border border-[#3f3f3f] rounded px-1.5 py-0.5 text-[#777]">
            {k}:{v}
          </code>
        ))}
      </div>
...
function groupIssues(issues: QAIssue[]) {
  const groups: Record<QAIssue["severity"], QAIssue[]> = { error: [], warning: [], info: [] };
  for (const iss of issues) {
    const bucket = groups[iss.severity] || groups.info;
    bucket.push(iss);
  }
  (Object.keys(groups) as Array<QAIssue["severity"]>).forEach((k) => {
