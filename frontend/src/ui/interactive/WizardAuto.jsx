...
export function WizardAuto({ backendUrl, apiKey, onPlanned, onSwitchToAdvanced, onCancel, }) {
    ...
    const [interactionType, setInteractionType] = useState("standard_project");
    ...
    const [renderMediaType, setRenderMediaType] = useState("video");
    ...
    return (<div className="relative flex flex-col h-full w-full">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        <div className="max-w-2xl mx-auto px-6 py-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-5">
            ...
            setPersonaId("");
            setPersonaLabel("");
        }} aria-pressed={interactionType === "standard_project"} disabled={generating} className={[
            ...
            <button type="button" onClick={() => setInteractionType("persona_live_play")} aria-pressed={interactionType === "persona_live_play"} disabled={generating} className={[
            "text-left bg-[#121212] border rounded-lg p-3 transition-colors",
          ...
          <div className="mb-5 flex items-center justify-between gap-3 bg-[#121212] border border-[#3f3f3f] rounded-md px-3 py-2.5">
            ...
            <select aria-label="Render media type" value={renderMediaType} onChange={(e) => setRenderMediaType((e.target.value === "image" ? "image" : "video"))} disabled={generating} className="shrink-0 bg-[#0f0f0f] border border-[#3f3f3f] rounded-md px-2 py-1.5 text-xs outline-none focus:border-[#8b5cf6] focus:ring-1 focus:ring-[#8b5cf6]/50 disabled:opacity-60">
              <option value="video">Video (full)</option>
              <option value="image">Image (feasibility)</option>
            </select>
          </div>

          {interactionType === "persona_live_play" && (<div className="mb-5">
              ...
              <select id="ix_auto_persona" value={personaId} onChange={(e) => {
                const selected = personaOptions.find((p) => p.id === e.target.value);
                setPersonaId(e.target.value);
                setPersonaLabel(selected?.label || "");
            }} disabled={generating} className="mt-1.5 w-full bg-[#121212] border border-[#3f3f3f] rounded-md px-3 py-2.5 text-sm outline-none focus:border-[#8b5cf6] focus:ring-1 focus:ring-[#8b5cf6]/50 disabled:opacity-60">
                <option value="">Select persona…</option>
                {personaOptions.map((p) => (<option key={p.id} value={p.id}>{p.label}</option>))}
              </select>
              {personaOptions.length === 0 && (<p className="text-[11px] text-amber-300 mt-1.5">
          ...
          <textarea id="ix_auto_idea" value={idea} onChange={(e) => setIdea(e.target.value)} onKeyDown={onKeyDown} placeholder={`e.g. ${IDEA_PLACEHOLDER}`} rows={4} disabled={generating} className={[
          ...
          {error && (<div className="mt-5">
              <ErrorBanner title="Couldn't plan your project" message={error} onRetry={runPlan}/>
            </div>)}

          <div className="mt-6 flex items-center justify-between">
            <button type="button" onClick={onSwitchToAdvanced} className="text-xs font-medium text-[#aaa] hover:text-[#f1f1f1] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#3ea6ff] rounded px-1">
              Advanced settings →
            </button>
            <PrimaryButton onClick={runPlan} disabled={!canSubmit} loading={generating} size="lg" icon={!generating ? <Sparkles className="w-4 h-4" aria-hidden/> : undefined}>
              Generate project
            </PrimaryButton>
          </div>
      ...
      {generating && (<GeneratingPanel title="Generating your project" description={<>
              Turning <span className="text-white/80">"{idea.trim().slice(0, 70)}{idea.trim().length > 70 ? "…" : ""}"</span>
              {" "}into a complete starting point.
            </>} steps={AI_STEPS} activeStep={step} accentClassName="text-[#c4b5fd]"/>)}
