...
import { ErrorBanner, useAsyncResource, useToast, } from "./ui";
import { WizardShell } from "./WizardShell";
import { DEFAULT_WIZARD_FORM, toCreatePayload, toPlanPayload, } from "./wizardState";
...
export function InteractiveWizard({ backendUrl, apiKey, onCreated, onCancel, }) {
    const api = useMemo(() => createInteractiveApi(backendUrl, apiKey), [backendUrl, apiKey]);
    const toast = useToast();
    const [form, setFormState] = useState(DEFAULT_WIZARD_FORM);
    const [step, setStep] = useState(0);
    const [submitting, setSubmitting] = useState(false);
    const setForm = useCallback((patch) => setFormState((prev) => ({ ...prev, ...patch })), []);
    ...
    const limits = useMemo(() => health.data?.limits || {
        max_branches: 12,
        max_depth: 6,
    ...
    return (<WizardShell steps={STEPS} activeIndex={step} title={titles[step]} subtitle={subtitles[step]} canGoBack canGoNext={canGoNext} submitting={submitting} nextLabel={isLast ? "Create project" : "Next"} onBack={goBack} onNext={goNext}>
      {health.error && step === 2 && (<div className="mb-4">
          <ErrorBanner title="Couldn't read backend caps" message={`${health.error} — using permissive defaults.`} onRetry={health.reload}/>
        </div>)}
      ...
      {step === 0 && (<p className="mt-6 text-xs text-[#777] flex items-center gap-2">
