...
export function Step0Prompt({ form, setForm }) {
    ...
    return (<div className="flex flex-col gap-5">
      ...
      <FieldLabel label="Interaction type" hint="Choose what kind of interactive video to build.">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <button type="button" onClick={() => setForm({
            ...
            persona_project_id: "",
            persona_label: "",
        })} aria-pressed={form.interaction_type === "standard_project"} className={[
          ...
          <button type="button" onClick={() => setForm({ interaction_type: "persona_live_play" })} aria-pressed={form.interaction_type === "persona_live_play"} className={[
            "text-left bg-[#121212] border rounded-md p-3 transition-colors",
            "focus:outline-none focus-visible:ring-2 focus-visible:ring-[#3ea6ff]",
      ...
      {form.interaction_type === "persona_live_play" && (<FieldLabel htmlFor="ix_persona_pick" label="Persona" required hint="Select the persona that should drive live-play animation and conversation.">
          <select id="ix_persona_pick" value={form.persona_project_id} onChange={(e) => {
            ...
            {personaOptions.map((persona) => (<option key={persona.id} value={persona.id}>{persona.label}</option>))}
          </select>
          {personaOptions.length === 0 && (<p className="text-[11px] text-amber-300 mt-1">
          ...
          {form.persona_project_id && (() => {
                ...
                if (!selected)
                    return null;
                return (<div className="mt-2.5 flex items-center gap-2.5 rounded-md border border-[#3a2a58] bg-[#130f1f] p-2.5">
                {selected.avatar_url ? (<img src={selected.avatar_url} alt={selected.label} className="w-12 h-12 rounded-md object-cover border border-[#51347f]"/>) : (<div className="w-12 h-12 rounded-md bg-[#24173a] border border-[#51347f]"/>)}
      ...
      <FieldLabel htmlFor="ix_title" label="Project title" required>
        <input id="ix_title" type="text" value={form.title} onChange={(e) => setForm({ title: e.target.value })} placeholder="e.g. Onboard new sales reps to our pricing tiers" maxLength={120} className="w-full bg-[#121212] border border-[#3f3f3f] rounded-md px-3 py-2.5 text-sm outline-none focus:border-[#3ea6ff] focus:ring-1 focus:ring-[#3ea6ff]/50"/>
      </FieldLabel>

      <FieldLabel htmlFor="ix_prompt" label={form.interaction_type === "persona_live_play" ? "Session vibe" : "Prompt"} required hint={form.interaction_type === "persona_live_play"
        ...
        <textarea id="ix_prompt" rows={5} value={form.prompt} onChange={(e) => setForm({ prompt: e.target.value })} placeholder={form.interaction_type === "persona_live_play"
            ? "e.g. teasing and flirt with progressive unlocks, playful dominant tone"
            : "e.g. Walk a new hire through our 3 pricing tiers in 4 branches; each branch ends with a quiz question."} className="w-full bg-[#121212] border border-[#3f3f3f] rounded-md px-3 py-2.5 text-sm outline-none focus:border-[#3ea6ff] focus:ring-1 focus:ring-[#3ea6ff]/50 resize-y"/>
        <PromptCounter value={form.prompt}/>
      </FieldLabel>

      <FieldLabel label="Render media" hint="Image = fast still-frame scenes (low GPU, good for feasibility tests). Video = full Animate/SVD clips.">
        <RenderMediaSelect value={form.render_media_type} onChange={(v) => setForm({ render_media_type: v })}/>
      </FieldLabel>
      ...
      <FieldLabel label="Experience mode" hint="Selects the policy profile + scene templates downstream.">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {MODE_OPTIONS.map((m) => {
            ...
            return (<button key={m.value} type="button" onClick={() => setForm({ experience_mode: m.value, policy_profile_id: m.value })} aria-pressed={selected} className={[
                    "text-left bg-[#121212] border rounded-md p-3 transition-colors",
                    "focus:outline-none focus-visible:ring-2 focus-visible:ring-[#3ea6ff]",
                    selected
                        ? "border-[#3ea6ff] bg-[rgba(62,166,255,0.08)] ring-1 ring-[#3ea6ff]"
...
function FieldLabel({ label, hint, required, htmlFor, children, }) {
...
function PromptCounter({ value }) {
    const len = value.trim().length;
    const ok = len >= 1;
    return (<div className="mt-0.5" aria-live="polite">
      <div className={["text-[11px]", ok ? "text-[#777]" : "text-amber-400"].join(" ")}>
        {len} characters{ok ? " ✓" : ""}
      </div>
      {!ok && (<div className="text-[11px] text-[#777] mt-1">
          Type at least one character to enable the Next button.
        </div>)}
    </div>);
}
function RenderMediaSelect({ value, onChange, }) {
    ...
    return (<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
      {options.map((opt) => {
            ...
            return (<button key={opt.value} type="button" onClick={() => onChange(opt.value)} aria-pressed={selected} className={[
                    "text-left bg-[#121212] border rounded-md p-3 transition-colors",
                    "focus:outline-none focus-visible:ring-2 focus-visible:ring-[#3ea6ff]",
                    selected
                        ? "border-[#3ea6ff] bg-[rgba(62,166,255,0.08)] ring-1 ring-[#3ea6ff]"
