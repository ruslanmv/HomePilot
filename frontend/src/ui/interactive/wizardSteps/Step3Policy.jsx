...
export function Step3Policy({ form, setForm }) {
    const selected = PROFILES.find((p) => p.id === form.policy_profile_id);
    return (<div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {PROFILES.map((p) => {
            const isSelected = form.policy_profile_id === p.id;
            return (<button key={p.id} type="button" onClick={() => setForm({ policy_profile_id: p.id })} aria-pressed={isSelected} className={[
