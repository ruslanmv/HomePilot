...
export function Step4Review({ form, api }) {
    ...
    return (<div className="flex flex-col gap-5">
      ...
      <PreviewCard loading={loading} error={error} intent={preview} personaLive={form.interaction_type === "persona_live_play"}/>
    </div>);
}
function SummaryCard({ form }) {
    const personaLive = form.interaction_type === "persona_live_play";
    return (<section className="rounded-md border border-[#3f3f3f] bg-[#121212] divide-y divide-[#2a2a2a]">
      <Row label={personaLive ? "Session type" : "Interaction"} value={personaLive
...
function PreviewCard({ loading, error, intent, personaLive, }) {
    const persona = !!personaLive;
    return (<section className="rounded-md border border-[#3f3f3f] bg-[#121212] p-4">
      <header className="text-xs font-medium text-[#cfd8dc] mb-3">
        {persona ? "Live session preview" : "Planner preview"}
      </header>
      {loading && (<div className="flex items-center gap-2 text-sm text-[#aaa]">
      ...
      {!loading && !error && intent && (<dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-xs">
...
function Row({ label, value, multiline }) {
    return (<div className="flex gap-4 px-4 py-2.5">
...
function DLRow({ term, value }) {
    return (<div className="flex gap-3 min-w-0">
