...
export function Step1Audience({ form, setForm }) {
    return (<div className="flex flex-col gap-5">
      <Field label="Viewer role" hint="Drives the persona of branch narration.">
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {ROLE_OPTIONS.map((r) => (<ChoiceCard key={r.value} selected={form.audience_role === r.value} onClick={() => setForm({ audience_role: r.value })} label={r.label} hint={r.hint}/>))}
        </div>
      </Field>
      ...
      <Field label="Skill level" hint="Pacing and vocabulary baseline.">
        <div className="inline-flex border border-[#3f3f3f] bg-[#121212] rounded-md p-1" role="group" aria-label="Skill level">
          {LEVEL_OPTIONS.map((lv) => {
            ...
            return (<button key={lv.value} type="button" onClick={() => setForm({ audience_level: lv.value })} aria-pressed={selected} className={[
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field htmlFor="ix_lang" label="Authoring language">
          <select id="ix_lang" value={form.audience_language} onChange={(e) => setForm({ audience_language: e.target.value })} className="w-full bg-[#121212] border border-[#3f3f3f] rounded-md px-3 py-2.5 text-sm outline-none focus:border-[#3ea6ff] focus:ring-1 focus:ring-[#3ea6ff]/50">
            {LANGUAGE_OPTIONS.map((l) => (<option key={l.value} value={l.value}>{l.label} ({l.value})</option>))}
          </select>
        </Field>

        <Field htmlFor="ix_locale" label="Locale hint" hint="Optional: country / region cue, e.g. 'us-west', 'italy'.">
          <input id="ix_locale" type="text" value={form.audience_locale_hint} onChange={(e) => setForm({ audience_locale_hint: e.target.value })} placeholder="(optional)" maxLength={32} className="w-full bg-[#121212] border border-[#3f3f3f] rounded-md px-3 py-2.5 text-sm outline-none focus:border-[#3ea6ff] focus:ring-1 focus:ring-[#3ea6ff]/50"/>
        </Field>
      </div>
    </div>);
}
...
function ChoiceCard({ selected, onClick, label, hint, }) {
    return (<button type="button" onClick={onClick} aria-pressed={selected} className={[
