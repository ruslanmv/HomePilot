...
import { ErrorBanner, SecondaryButton, SkeletonRow, useAsyncResource, } from "./ui";
export function XPRewardsSheet({ open, onClose, api, sessionId, }) {
    const resource = useAsyncResource((signal) => (open
        ? api.getProgress(sessionId, signal)
        : Promise.resolve({ progress: {}, descriptions: {}, mood: "", affinity_score: 0 })), [api, sessionId, open]);
    const schemes = useMemo(() => {
        const d = resource.data?.descriptions || {};
        return Object.entries(d);
    }, [resource.data]);
    return (<BottomSheet open={open} onClose={onClose} title="XP Rewards" subtitle="Level up by chatting, trying actions, and completing moments." actions={<SecondaryButton onClick={onClose} size="sm">
          Done
        </SecondaryButton>}>
      {resource.error ? (<ErrorBanner title="Couldn't load progress" message={resource.error} onRetry={resource.reload}/>) : resource.loading && !resource.data ? (<div className="flex flex-col gap-2" aria-busy="true">
          {Array.from({ length: 2 }).map((_, i) => <SkeletonRow key={i}/>)}
        </div>) : schemes.length === 0 ? (<DefaultLevelPanel />) : (<div className="flex flex-col gap-4">
          {schemes.map(([scheme, desc]) => (<SchemeCard key={scheme} scheme={scheme} desc={desc}/>))}
        </div>)}
      <HowToEarn />
    </BottomSheet>);
}
...
function SchemeCard({ scheme, desc, }) {
    ...
    return (<section className="bg-[#121212] border border-[#3f3f3f] rounded-lg p-4">
      ...
      <div className="mt-3 h-2 rounded-full bg-[#1f1f1f] overflow-hidden">
        <div className="h-full bg-gradient-to-r from-[#3ea6ff] to-[#6366f1]" style={{ width: `${pct}%` }} aria-hidden/>
      </div>
...
function LevelBadge({ level }) {
    return (<span className="inline-flex items-center justify-center w-11 h-11 rounded-full bg-black border border-white/15 text-base font-bold text-[#f1f1f1]">
      {level}
    </span>);
}
function HowToEarn() {
    ...
    return (<section className="mt-4 bg-[#121212] border border-[#3f3f3f] rounded-lg">
      <button type="button" onClick={toggle} className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium text-[#f1f1f1] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#3ea6ff] rounded-lg" aria-expanded={open}>
      ...
      {open && (<ul className="px-4 pb-4 text-xs text-[#cfd8dc] space-y-1.5 list-disc ml-5">
