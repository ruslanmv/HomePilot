...
import { User, LogIn, UserPlus, ArrowRight, Eye, EyeOff, Mail, AlertCircle, CheckCircle2, ChevronRight, } from 'lucide-react';
import { resolveFileUrl } from '../resolveFileUrl';
export default function AuthScreen({ backendUrl, onAuthenticated, logoutMessage, recentUsers, }) {
    ...
    const handleQuickSelect = (recentUser) => {
        setMode('login');
    ...
    return (<div style={{
      ...
      {showLogoutBanner && logoutMessage && (<div style={{
                display: 'flex',
                alignItems: 'center',
                gap: 10,
                padding: '12px 20px',
          ...
          <button type="button" onClick={() => setShowLogoutBanner(false)} style={{
                marginLeft: 'auto',
                background: 'none',
                border: 'none',
                color: T.greenText,
      ...
      {showRecentUsers && (<div style={{
          ...
          {filteredRecent.map((recent) => (<button key={recent.username} type="button" onClick={() => handleQuickSelect(recent)} style={{
                    width: '100%',
                    display: 'flex',
                    alignItems: 'center',
                    gap: 12,
              ...
              {/* Avatar */}
              {recent.avatar_url ? (<img src={resolveFileUrl(recent.avatar_url)} alt="" style={{
                        width: 36,
                        height: 36,
                        borderRadius: '50%',
                        objectFit: 'cover',
      ...
      <div style={{
        ...
        <div style={{
          ...
          {['login', 'register'].map(m => (<button key={m} onClick={() => { setMode(m); setError(''); }} style={{
                flex: 1,
                padding: '10px 0',
                borderRadius: 8,
                border: 'none',
              ...
              {m === 'login' ? (<span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                  <LogIn size={14}/> Sign In
                </span>) : (<span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type="text" value={username} onChange={e => setUsername(e.target.value)} placeholder="Enter username" required autoFocus={!showRecentUsers} minLength={2} maxLength={32} style={inputStyle} {...focusHandlers}/>
            </div>
          </div>
          ...
          {mode === 'register' && (<div style={{ marginBottom: 18 }}>
              ...
              <input type="text" value={displayName} onChange={e => setDisplayName(e.target.value)} placeholder="How should we call you?" maxLength={64} style={inputStyleNoIcon} {...focusHandlers}/>
            </div>)}
          ...
          <div style={{ marginBottom: mode === 'register' ? 18 : 24 }}>
            ...
            <div style={{ position: 'relative' }}>
              <input id="auth-password-input" type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder={mode === 'register' ? 'Leave empty for no password' : 'Enter password'} maxLength={128} style={{
            ...inputStyleNoIcon,
            paddingRight: 40,
        }} {...focusHandlers}/>
          ...
          {mode === 'register' && (<div style={{ marginBottom: 24 }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="your@email.com" maxLength={256} style={inputStyle} {...focusHandlers}/>
              </div>
            </div>)}
        ...
        }} onMouseEnter={e => { if (username.trim() && !loading)
        e.currentTarget.style.background = T.blueHover; }} onMouseLeave={e => { e.currentTarget.style.background = T.blue; }}>
        ...
        {mode === 'login' && logoutMessage && (<div style={{
                marginTop: 16,
                padding: '10px 14px',
                borderRadius: 10,
