...
import { ArrowLeft, Search, Shirt, Image as ImageIcon, FileText, Loader2, Eye, Lock, Package, AlertCircle, Trash2, RotateCcw, } from 'lucide-react';
import { fetchInventoryCategories, searchInventory, deleteInventoryItem, } from '../inventoryApi';
import { PersonaDocumentsPanel } from './PersonaDocumentsPanel';
// ---------------------------------------------------------------------------
// Sensitivity helpers (mirrors backend inventory.py)
// ---------------------------------------------------------------------------
const SENS_ORDER = { safe: 0, sensitive: 1, explicit: 2 };
function classifySensitivity(label) {
...
function allowedBySensitivity(itemSens, maxSens) {
    return (SENS_ORDER[itemSens] ?? 0) <= (SENS_ORDER[maxSens] ?? 0);
}
// ---------------------------------------------------------------------------
// Build InventoryItem[] from local draft state
// ---------------------------------------------------------------------------
function buildDraftItems(draft, sensitivityMax) {
    const out = [];
    ...
    for (const s of draft.sets) {
        for (const img of s.images) {
            if (!img.url)
                continue;
            if (!allowedBySensitivity('safe', sensitivityMax))
                continue;
            out.push({
                id: img.id,
                type: 'image',
    ...
    for (const outfit of draft.outfits) {
        ...
        if (!allowedBySensitivity(sens, sensitivityMax))
            continue;
...
// Main component
// ---------------------------------------------------------------------------
export function InventoryView({ projectId, backendUrl, apiKey, onBack, onShowInChat, activeSelection, onSetActiveLook, draftAppearance }) {
    // State
    ...
    const mergedItems = useMemo(() => {
        ...
        const extras = draftItems.filter(d => {
            ...
            if (q) {
                ...
                if (!label.includes(q) && !desc.includes(q) && !tags.includes(q))
                    return false;
            }
            return true;
        });
    ...
    // -----------------------------------------------------------------------
    const handleSetActiveLook = useCallback((item) => {
        if (!item.set_id || !item.image_id)
            return;
    ...
    const renderSidebar = () => (<div className="w-44 shrink-0 border-r border-white/10 py-3 space-y-0.5">
      {/* All items */}
      <SidebarItem icon={Package} label="All Items" count={totalItems} color="text-purple-400" active={selectedCategory === 'all'} onClick={() => setSelectedCategory('all')}/>
      {/* Per-category */}
      {mergedCategories.map((cat) => (<SidebarItem key={cat.type} icon={CATEGORY_ICONS[cat.type] || Package} label={cat.label} count={cat.count} color={CATEGORY_COLORS[cat.type] || 'text-white/50'} active={selectedCategory === cat.type} onClick={() => setSelectedCategory(cat.type)}/>))}
    </div>);
    // -----------------------------------------------------------------------
    // Render: item card
    // -----------------------------------------------------------------------
    const renderItemCard = (item) => {
        ...
        {(isSelected || (isOutfit && item.equipped)) && (<div className="absolute top-1.5 right-1.5 px-1.5 py-0.5 rounded-md bg-amber-500/80 text-[7px] text-white font-bold uppercase tracking-wider z-10">
            Equipped
          </div>)}
        ...
        <div className={[
          ...
          <ActionButton icon={Trash2} label="Delete" onClick={() => handleDelete(item)} variant="danger"/>
        </div>

        {/* Image preview — primary click selects (like Wardrobe) */}
        {(isImage || isOutfit) && (<div className="aspect-[3/4] bg-black/30 relative overflow-hidden cursor-pointer" onClick={() => {
                    if (canSetActive) {
                        handleSetActiveLook(item);
                    }
                }}>
            {isSensitive && !isSpicy ? (<div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-black/60">
                <Lock size={20} className="text-white/30"/>
                <span className="text-[10px] text-white/30">Locked</span>
              </div>) : imgUrl ? (<img src={imgUrl} alt={item.label} className="w-full h-full object-cover" loading="lazy" onError={(e) => { e.target.style.display = 'none'; }}/>) : (<div className="absolute inset-0 flex items-center justify-center">
            ...
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-end justify-center pb-2 gap-1.5 opacity-0 group-hover:opacity-100">
              {onShowInChat && imgUrl && (<ActionButton icon={Eye} label="Show in chat" onClick={() => onShowInChat(item, imgUrl)}/>)}
            </div>
          </div>)}

        {/* File icon for documents */}
        {isFile && (<div className="aspect-[4/3] bg-black/20 flex items-center justify-center relative">
        ...
        <div className="px-2.5 py-2">
          ...
          <div className="flex items-center gap-1 mt-1 flex-wrap">
            ...
            {isSensitive && (<span className={`text-[9px] flex items-center gap-0.5 ${item.sensitivity === 'explicit' ? 'text-red-400/60' : 'text-orange-400/60'}`}>
                <Lock size={8}/> {item.sensitivity}
              </span>)}
            {/* 360 preview badge for outfits with view packs */}
            {isOutfit && item.interactive_preview && (<span className="text-[9px] flex items-center gap-0.5 text-violet-300/60">
                <RotateCcw size={8}/> 360
              </span>)}
          </div>
          {/* Tags */}
          {item.tags && item.tags.length > 0 && (<div className="flex flex-wrap gap-1 mt-1.5">
              {item.tags.slice(0, 3).map((tag) => (<span key={tag} className="text-[9px] px-1.5 py-0.5 rounded-full bg-white/5 text-white/30 border border-white/5">
                  {tag}
                </span>))}
            </div>)}
          {/* View angle chips for outfits with view packs */}
          {isOutfit && item.available_views && item.available_views.length > 0 && (<div className="flex flex-wrap gap-1 mt-1.5">
              {item.available_views.map((angle) => (<span key={angle} className="text-[8px] px-1.5 py-0.5 rounded-full bg-violet-500/8 text-violet-300/50 border border-violet-500/15 capitalize">
                  {angle}
                </span>))}
            </div>)}
          {isFile && item.size_bytes != null && item.size_bytes > 0 && (<div className="text-[9px] text-white/25 mt-1">
    ...
    const renderLoadoutSummary = () => (<div className="flex items-center gap-4 px-4 py-2.5 bg-white/[0.03] border-b border-white/5">
      {/* Mini Active Look */}
      {activeLookItem?.url && (<div className="w-8 h-8 rounded-lg overflow-hidden border border-amber-500/30 shrink-0">
          <img src={resolveImageUrl(activeLookItem) || ''} alt="Active Look" className="w-full h-full object-cover"/>
        </div>)}
    ...
    if (loading && items.length === 0) {
        return (<div className="flex-1 flex items-center justify-center py-20">
    ...
    return (<div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10 bg-white/[0.02]">
        <button onClick={onBack} className="flex items-center gap-1.5 text-xs text-white/50 hover:text-white transition-colors shrink-0">
          <ArrowLeft size={14}/>
          <span>Back</span>
        </button>
        <div className="flex-1 relative">
          <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30"/>
          <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search inventory..." className="w-full pl-9 pr-3 py-1.5 text-xs bg-white/5 border border-white/10 rounded-lg text-white placeholder:text-white/25 focus:outline-none focus:border-white/20 transition-colors"/>
        </div>
        <div className="text-[10px] text-white/30 shrink-0">
          {searchLoading ? (<Loader2 size={12} className="animate-spin"/>) : (`${mergedItems.length} items`)}
        </div>
      </div>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3">
          {error && (<div className="flex items-center gap-2 px-3 py-2 mb-3 rounded-lg bg-red-500/10 border border-red-500/20 text-xs text-red-400">
              <AlertCircle size={14}/>
              {error}
            </div>)}

          {/* Knowledge Base panel (shown when Documents category is selected) */}
          {selectedCategory === 'file' && (<div className="mb-4 p-3 rounded-xl bg-white/[0.02] border border-white/5">
              <PersonaDocumentsPanel projectId={projectId} backendUrl={backendUrl} apiKey={apiKey} onChanged={() => setRefreshKey((k) => k + 1)}/>
            </div>)}

          {mergedItems.length === 0 && !searchLoading && selectedCategory !== 'file' && (<div className="flex flex-col items-center justify-center py-16 gap-2 text-white/30">
          ...
          {selectedCategory !== 'file' && (<div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
              {mergedItems.map(renderItemCard)}
            </div>)}

          {selectedCategory !== 'file' && items.length < totalCount && !searchLoading && (<div className="text-center py-4">
...
// Sidebar item
// ---------------------------------------------------------------------------
function SidebarItem({ icon: Icon, label, count, color, active, onClick, }) {
    return (<button onClick={onClick} className={[
      ...
      {count != null && (<span className="text-[10px] text-white/30 tabular-nums">{count}</span>)}
    </button>);
}
// ---------------------------------------------------------------------------
...
function ActionButton({ icon: Icon, label, onClick, variant, }) {
    ...
    return (<button onClick={(e) => { e.stopPropagation(); onClick(); }} title={label} className={[
