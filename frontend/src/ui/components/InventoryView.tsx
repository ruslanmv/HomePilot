...
export function InventoryView({ projectId, backendUrl, apiKey, onBack, onShowInChat, activeSelection, onSetActiveLook }: Props) {
  // State
  ...
  // -----------------------------------------------------------------------
  const totalItems = categories.reduce((sum, c) => sum + (c.count || 0), 0)
  ...
  // -----------------------------------------------------------------------
  const activeLookItem = items.find((i) => i.is_active_look)
  const outfitCount = categories.find((c) => c.type === 'outfit')?.count || 0
  const photoCount = categories.find((c) => c.type === 'image')?.count || 0
  const docCount = categories.find((c) => c.type === 'file')?.count || 0
    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3">
          ...
          {items.length === 0 && !searchLoading && selectedCategory !== 'file' && (
