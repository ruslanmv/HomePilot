...
import { PERSONALITY_CAPS } from './personalityCaps';
import { getPersonalitiesByCategory, getCategoryLabel, canAccessAdultContent, } from './personalityGating';
export default function PersonalityList({ activePersonality, onSelect, personas = [], }) {
    const [showList, setShowList] = useState(false);
    const [dropdownPos, setDropdownPos] = useState({ top: 0, left: 0, height: 400 });
    const triggerRef = useRef(null);
    const btnInactive = 'bg-[#1F1F1F] border border-transparent hover:bg-[#2A2A2A] hover:border-white/5';
    const panelBg = 'bg-[#121212]/95 backdrop-blur-xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.9)]';
    ...
    return (<div className="flex flex-col gap-3">
      ...
      {/* Trigger button */}
      <button ref={triggerRef} type="button" onClick={() => setShowList(!showList)} className={`w-full h-[48px] px-4 rounded-[16px] flex items-center justify-between transition-colors ${btnInactive}`}>
      ...
      {showList && typeof document !== 'undefined' &&
            ...
            <div className="fixed inset-0 z-[99]" data-hp-voice-portal="true" onClick={() => setShowList(false)}/>

            {/* Dropdown panel */}
            <div data-hp-voice-portal="true" className={`fixed w-[280px] rounded-[24px] overflow-y-auto hp-scroll flex flex-col p-3 z-[100] hp-fade-in ${panelBg}`} style={{
                    top: dropdownPos.top,
              ...
              {['general', 'wellness', 'kids', 'adult'].map((category) => {
                    const personalities = personalitiesByCategory[category];
                    if (!personalities || personalities.length === 0)
                        return null;
                    return (<div key={category} className="mb-2">
                    {/* Category Header */}
                    <div className={`flex items-center gap-2 px-2 py-1.5 text-[10px] font-bold uppercase tracking-wider ${category === 'adult' ? 'text-orange-400/70' : 'text-white/40'}`}>
                    ...
                    <div className="flex flex-col gap-0.5">
                      {personalities.map((p) => {
                            ...
                            return (<button key={p.id} type="button" onClick={() => handleSelect(p)} className={`
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {caps && (<span className="text-[9px] text-white/30 block truncate">
                                  {caps.responseStyle.tone}
                                </span>)}
                            </div>
                            {p.mature && (<span className="text-[8px] text-orange-400/80 px-1.5 py-0.5 bg-orange-500/10 rounded font-medium">
                                18+
                              </span>)}
              ...
              {personas.length > 0 && (<div className="mb-2">
                    ...
                    {personas.map((p) => {
                        ...
                        return (<button key={p.id} type="button" onClick={() => handleSelect(p)} className={`
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {p.personaTone && (<span className="text-[9px] text-white/30 block truncate">
                                {p.personaTone}
                              </span>)}
                          </div>
              ...
              {!adultAllowed && (<div className="mt-2 p-2 rounded-[10px] bg-white/5 border border-white/5">
