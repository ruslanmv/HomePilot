...
export default function VoiceSettingsPanel({ isOpen, activeVoice, setActiveVoice, activePersonality, setActivePersonality, speed, setSpeed, }) {
    ...
    const fetchPersonaProjects = useRef(() => {
        ...
        const apiKey = typeof window !== 'undefined' ? localStorage.getItem('homepilot_api_key') || '' : '';
        const headers = {};
        if (apiKey)
        ...
        fetch(`${backendUrl.replace(/\/+$/, '')}/projects`, { headers })
            ...
            if (!data?.projects)
                return;
            const personaProjects = data.projects.filter((p) => p.project_type === 'persona' && p.persona_agent);
            // Cache for prompt assembly in App.tsx — include full appearance data
            const cache = personaProjects.map((p) => {
            ...
            const entries = personaProjects.map((p) => ({
                id: `persona:${p.id}`,
                label: p.persona_agent?.label || p.name || 'Persona',
                icon: User,
    ...
    return (<div className={`absolute bottom-[60px] left-0 w-[340px] rounded-[24px] overflow-visible hp-fade-in z-[60] ${panelBg}`}>
      <div className="p-5 flex flex-col gap-5 max-h-[60vh] overflow-y-auto hp-scroll">
        ...
        {/* Personality Section */}
        <PersonalityList activePersonality={activePersonality} onSelect={setActivePersonality} personas={personaEntries}/>

        {/* Custom Instructions - only when Custom personality is selected */}
        {activePersonality.id === 'custom' && (<div className="rounded-[20px] bg-white/[0.03] p-4">
            ...
            <textarea value={customPrompt} onChange={(e) => {
                ...
                setCustomDirty(true);
                setSavedFlash(false);
            }} placeholder={"e.g. You are a witty British butler. Be dry, formal, and slightly sarcastic. Keep answers short."} className="w-full min-h-[100px] rounded-[14px] bg-black/30 border border-white/[0.06] p-3 text-[13px] leading-relaxed text-white/90 placeholder-white/20 outline-none focus:border-white/20 resize-none hp-scroll transition-colors"/>

            <div className="mt-2 flex items-center justify-between">
              <span className={`text-[10px] tabular-nums ${customPrompt.length > MAX_CUSTOM_PROMPT_LENGTH * 0.9
                ? 'text-orange-400/60'
                : 'text-white/20'}`}>
                {customPrompt.length}/{MAX_CUSTOM_PROMPT_LENGTH}
              </span>

              <button type="button" disabled={!customDirty} onClick={() => {
            ...
            }} className={`h-8 px-4 rounded-full text-[12px] font-semibold transition-all duration-200 ${savedFlash
