...
export function createVAD(onSpeechStart, onSpeechEnd, config) {
    const cfg = { ...DEFAULT_CONFIG, ...config };
    const callbacks = { onSpeechStart, onSpeechEnd };
    ...
    function updateNoiseFloor(level) {
        ...
        if (level < noiseFloor * 2) {
            noiseFloor = Math.max(cfg.noiseFloorMin, noiseFloor * cfg.noiseFloorDecay + level * (1 - cfg.noiseFloorDecay));
        }
    }
