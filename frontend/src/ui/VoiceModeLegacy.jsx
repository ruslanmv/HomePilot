 ...
 * VoiceModeLegacy.tsx
 *
 * Original HomePilot Voice Mode component (preserved for backwards compatibility)
 * This is the legacy implementation before the Grok-style UI upgrade.
 *
 * @deprecated Use VoiceModeGrok.tsx for the new UI
 */
...
export default function VoiceModeLegacy({ onSendText, }) {
      ...
      <div className={`w-full max-w-2xl mx-auto rounded-3xl border ${THEME.border} ${THEME.surface} overflow-hidden shadow-2xl transition-all duration-300`}>
        ...
        <div className={`flex items-center justify-between px-6 py-4 border-b ${THEME.border}`}>
          <div className="flex items-center gap-3">
            ...
            {/* State indicator */}
            <div className={`px-2 py-0.5 rounded-full text-[10px] uppercase tracking-wider ${voice.state === 'OFF' ? 'bg-gray-800 text-gray-400' :
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowVoiceSettings(!showVoiceSettings)} className={`p-2 rounded-full ${THEME.textDim} hover:text-white ${THEME.surfaceHover} transition-colors`}>
        ...
        <div className="relative h-64 flex flex-col items-center justify-center p-6 bg-gradient-to-b from-transparent to-black/20">
          ...
          <div className="relative mb-6">
            ...
            : voice.state === 'SPEAKING'
                ...
                : voice.state === 'THINKING'
                    ...
                    : 'bg-[#222] scale-100'}`}>
              {isListening ? (<div className="space-y-1 flex gap-1 h-8 items-center">
                ...
                </div>) : voice.state === 'SPEAKING' ? (<Volume2 className="text-white" size={28}/>) : voice.state === 'THINKING' ? (<div className="w-6 h-6 border-2 border-black border-t-transparent rounded-full animate-spin"/>) : (<MicOff className="text-white/30" size={28}/>)}
            </div>
            {isListening && (<div className="absolute inset-0 rounded-full border border-white/20 animate-ping"></div>)}
          </div>

          {/* Audio Level Visualization (hands-free mode) */}
          {voice.isHandsFree && voice.state !== 'OFF' && (<div className="w-full max-w-xs mb-4">
              <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-green-500 to-green-300 transition-all duration-100" style={{ width: `${Math.min(voice.audioLevel * 500, 100)}%` }}/>
              </div>
          ...
          <div className="text-center min-h-[3rem] max-w-lg px-4">
            {voice.interimText ? (<div className="space-y-2">
              ...
              </div>) : (<span className={`text-sm ${THEME.textDim}`}>
        ...
        {/* Settings Expandable */}
        {showVoiceSettings && (<div className={`px-6 py-4 border-t ${THEME.border} bg-[#111] animate-in slide-in-from-top-2`}>
            <label className={`text-xs font-semibold uppercase tracking-wider ${THEME.textDim} mb-3 block`}>Voice Persona</label>
            <select className={`w-full bg-[#1a1a1a] border ${THEME.border} rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-white/30 transition-colors appearance-none cursor-pointer`} value={voice.selectedVoice} onChange={(e) => voice.setSelectedVoice(e.target.value)}>
              {voice.voices.length === 0 && <option value="">Loading voices...</option>}
              {voice.voices.map((v) => (<option key={v.voiceURI} value={v.voiceURI}>
                  {v.name} ({v.lang})
                </option>))}
            </select>
        ...
        <div className={`p-4 border-t ${THEME.border} grid grid-cols-3 gap-3 bg-[#0f0f0f]`}>
          ...
          {/* TTS Toggle */}
          <button onClick={() => voice.setTtsEnabled(!voice.isTtsEnabled)} className={`flex flex-col items-center justify-center gap-1 py-3 rounded-xl border border-transparent transition-all ${voice.isTtsEnabled ? "bg-white/10 text-white" : "hover:bg-white/5 text-[#666]"}`}>
        ...
        }} className={`flex flex-col items-center justify-center gap-1 py-3 rounded-xl transition-all shadow-lg ${isListening
            ? "bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500/20"
            : voice.state === 'SPEAKING'
                ? "bg-blue-500/10 text-blue-500 border border-blue-500/20 hover:bg-blue-500/20"
                : "bg-white text-black hover:bg-gray-200"}`}>
            {voice.state === 'SPEAKING' ? (<VolumeX size={24} strokeWidth={2.5}/>) : (<Mic size={24} strokeWidth={2.5}/>)}
          </button>

          {/* Hands Free Toggle */}
          <button onClick={() => voice.setHandsFree(!voice.isHandsFree)} className={`flex flex-col items-center justify-center gap-1 py-3 rounded-xl border transition-all ${voice.isHandsFree
