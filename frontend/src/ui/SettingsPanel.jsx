...
import "./tts";
import { getActiveTtsEngineId, onActiveTtsEngineChange, } from "./tts";
import { resolveBackendUrl } from "./lib/backendUrl";
import { getModelSettings, getPresetDescription, detectArchitecture, getArchitectureLabel, } from "./modelPresets";
// ── Agentic Status sub-component (Phase 1, additive) ─────────────────────────
function AgenticStatus({ backendUrl, apiKey }) {
    const [status, setStatus] = React.useState(null);
    const [adminUrl, setAdminUrl] = React.useState(null);
    ...
    if (err) {
        ...
        <button type="button" onClick={fetchStatus} className="text-[11px] px-3 py-1 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-white/60 hover:text-white/80 transition-all">
          Retry
        </button>
      </div>);
    }
    if (!status)
        return null;
    const dot = (ok) => (<span className={`inline-block w-2 h-2 rounded-full ${ok ? "bg-green-400" : "bg-white/20"}`}/>);
    return (<div className="space-y-2">
      ...
      <div className="flex items-center gap-2">
        {adminUrl && (<a href={adminUrl} target="_blank" rel="noopener noreferrer" className="text-[11px] px-3 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-white/70 hover:text-white transition-all">
            Open Tool &amp; Agent Manager
          </a>)}
        <button type="button" onClick={fetchStatus} className="text-[11px] px-3 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-white/70 hover:text-white transition-all">
          {loading ? "Refreshing..." : "Refresh"}
        </button>
      </div>
    </div>);
}
export default function SettingsPanel({ value, onChangeDraft, onSave, onClose, }) {
    ...
    const [ttsEngineId, setTtsEngineId] = useState(() => getActiveTtsEngineId());
    ...
    async function fetchModelsFor(providerKey, baseUrlOverride, modelType) {
    ...
    function detectVideoModelType(modelName) {
        if (!modelName)
            return "ltx"; // Default to LTX
        ...
        if (lower.includes("mochi"))
            return "mochi";
        if (lower.includes("cog"))
            return "cogvideo";
        return "ltx"; // Default
    }
    // Fetch video presets from backend for a specific model
    async function fetchVideoPresets(modelName) {
        ...
        try {
            ...
            await Promise.all(presetLevels.map(async (preset) => {
        ...
        if ('speechSynthesis' in window) {
            ...
            if (voices.length > 0) {
                ...
                if (!voicesInitialized && !value.selectedVoice && voices.length > 0) {
                    ...
                    const preferred = voices.find((v) => v.name.toLowerCase().includes('google') &&
    ...
    useEffect(() => {
        if (autoFetchedRef.current || Object.keys(providers).length === 0)
            return;
        ...
        if (value.providerImages) {
            ...
            const modelType = value.providerImages === 'comfyui' ? 'image' : undefined;
        ...
        if (value.providerVideo) {
            ...
            const modelType = value.providerVideo === 'comfyui' ? 'video' : undefined;
    ...
    useEffect(() => {
        ...
        const updates = {};
        if (!value.modelChat && models[chatKey]?.length)
            updates.modelChat = models[chatKey][0];
        if (!value.modelImages && models[imgKey]?.length)
            updates.modelImages = models[imgKey][0];
        if (!value.modelVideo && models[vidKey]?.length)
            updates.modelVideo = models[vidKey][0];
        if (!value.modelMultimodal && mmKey && models[mmKey]?.length)
            updates.modelMultimodal = models[mmKey][0];
    ...
    useEffect(() => {
        if (value.preset !== "custom") {
            ...
            const needsImageUpdate = value.imgWidth !== imgSettings.width ||
                value.imgHeight !== imgSettings.height ||
                value.imgSteps !== imgSettings.steps ||
                value.imgCfg !== imgSettings.cfg;
            const needsVideoUpdate = value.vidWidth !== vidSettings.width ||
    ...
    const ollamaSelected = value.providerChat === "ollama" ||
        value.providerImages === "ollama" ||
        value.providerVideo === "ollama";
    const ollamaDown = ollamaSelected && health?.providers?.ollama && health.providers.ollama.ok === false;
    const showOllamaBanner = !!ollamaDown && !dismissOllamaBanner;
    function providerSelectRow(label, provider, setProvider) {
        return (<div>
        <div className="text-[11px] uppercase tracking-wider text-white/40 mb-2 font-semibold">{label}</div>
        <select className="w-full bg-black border border-white/10 rounded-xl px-3 py-2 text-xs text-white" value={provider} onChange={(e) => setProvider(e.target.value)}>
          {providerOptions.map(([k, info]) => (<option key={k} value={k}>
              {info.label}
            </option>))}
        </select>
      </div>);
    }
    function baseUrlRow(label, providerKey, valueUrl, setUrl) {
        const info = providers?.[providerKey];
        const hint = info?.base_url || '';
        return (<div className="border-t border-white/5 pt-3">
        <div className="text-[11px] uppercase tracking-wider text-white/40 mb-2 font-semibold">{label}</div>
        <input className="w-full bg-black border border-white/10 rounded-xl px-3 py-2 text-xs text-white" value={valueUrl ?? ''} placeholder={hint ? `Default: ${hint}` : 'Optional'} onChange={(e) => setUrl(e.target.value)}/>
        <div className="mt-1 text-[10px] text-white/35">
          Leave blank to use the backend default. Useful when running services locally (e.g. Ollama on http://localhost:11434).
        </div>
      </div>);
    }
    function modelSelectRow(label, providerKey, modelValue, setModel, baseUrlOverride, modelType) {
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <button type="button" onClick={() => fetchModelsFor(providerKey, baseUrlOverride, modelType)} className="px-3 py-1.5 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-xs text-white font-semibold disabled:opacity-50" disabled={loading || !providerKey}>
            {loading ? "Fetching..." : "Fetch models"}
          </button>
        </div>

        <select className="w-full bg-black border border-white/10 rounded-xl px-3 py-2 text-xs text-white" value={modelValue} onChange={(e) => setModel(e.target.value)}>
    ...
    return (<div className="absolute bottom-16 left-0 right-0 mx-2 max-h-[70vh] bg-[#181818] border border-white/15 rounded-2xl shadow-2xl z-50 flex flex-col overflow-hidden">
      ...
      <div className="sticky top-0 z-10 bg-[#181818]/95 backdrop-blur-sm border-b border-white/[0.06] px-4 py-3 flex items-center justify-between shrink-0">
        ...
        <button type="button" onClick={onClose} className="text-white/50 hover:text-white w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/5 transition-colors" aria-label="Close settings">
          ✕
        </button>
      </div>
      {/* Scrollable body */}
      ...
      <OllamaHealthBanner show={showOllamaBanner} onDismiss={() => setDismissOllamaBanner(true)} onSwitchToVllm={() => {
      ...
      <div className="space-y-3">
        <div>
          ...
          <input className="w-full bg-black border border-white/10 rounded-xl px-3 py-2 text-xs text-white" value={value.backendUrl} onChange={(e) => onChangeDraft({ ...value, backendUrl: e.target.value })}/>
        </div>

        <div>
          <div className="text-[11px] uppercase tracking-wider text-white/40 mb-2 font-semibold">API Key (optional)</div>
          <input type="password" className="w-full bg-black border border-white/10 rounded-xl px-3 py-2 text-xs text-white" value={value.apiKey} onChange={(e) => onChangeDraft({ ...value, apiKey: e.target.value })}/>
        </div>

        <div className="border-t border-white/5 pt-3">
          <div className="flex items-center justify-between">
            <div className="text-[11px] uppercase tracking-wider text-white/40 font-semibold">Providers</div>
            <button type="button" onClick={fetchProviders} className="px-3 py-1.5 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-xs text-white font-semibold disabled:opacity-50" disabled={loadingProviders}>
        ...
        {(value.providerImages === 'comfyui' || value.providerVideo === 'comfyui') && (<div className="border-t border-white/5 pt-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <select aria-label="ComfyUI VRAM mode" value={value.comfyVramMode ?? 'high'} onChange={(e) => onChangeDraft({
                ...value,
                comfyVramMode: (e.target.value || 'high'),
        ...
        <div className="border-t border-white/5 pt-3">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => onChangeDraft({ ...value, nsfwMode: !value.nsfwMode })} className={[
          ...
          {value.nsfwMode && (<div className="mt-2 text-[10px] text-red-400/80 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">
        ...
        <div className="border-t border-white/5 pt-3">
          ...
          <select className="w-full bg-black border border-white/10 rounded-xl px-3 py-2 text-sm text-white outline-none focus:border-white/20" value={value.memoryEngine || 'v2'} onChange={(e) => onChangeDraft({ ...value, memoryEngine: e.target.value })}>
        ...
        <div className="border-t border-white/5 pt-3">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => onChangeDraft({ ...value, experimentalCivitai: !value.experimentalCivitai })} className={[
            "relative inline-flex h-6 w-11 items-center rounded-full transition-colors",
          ...
          {value.experimentalCivitai && (<div className="mt-2 text-[10px] text-blue-400/80 bg-blue-500/10 border border-blue-500/20 rounded-lg px-3 py-2">
          ...
          {value.experimentalCivitai && value.nsfwMode && (<div className="mt-3 p-3 bg-purple-500/5 border border-purple-500/20 rounded-lg">
              ...
              <input type="password" value={value.civitaiApiKey || ""} onChange={(e) => onChangeDraft({ ...value, civitaiApiKey: e.target.value })} placeholder="Optional - only needed for adult content" className="w-full bg-black border border-purple-500/30 rounded-lg px-3 py-2 text-xs text-white placeholder:text-white/30 focus:border-purple-500/50 outline-none transition-colors"/>
        ...
        <div className="border-t border-white/5 pt-3">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => onChangeDraft({ ...value, promptRefinement: !value.promptRefinement })} className={[
            "relative inline-flex h-6 w-11 items-center rounded-full transition-colors",
          ...
          {value.promptRefinement && (<div className="mt-2 text-[10px] text-green-400/80 bg-green-500/10 border border-green-500/20 rounded-lg px-3 py-2">
          ...
          {!value.promptRefinement && (<div className="mt-2 text-[10px] text-white/50 bg-white/5 border border-white/10 rounded-lg px-3 py-2">
        ...
        <div className="border-t border-white/5 pt-3">
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {["low", "med", "high", "ultra", "custom"].map((p) => (<button key={p} type="button" onClick={() => onChangeDraft({ ...value, preset: p })} className={[
          ...
          {value.preset !== "custom" && (<div className="text-[10px] text-green-400/70 mt-1">
              {videoPresetsLoading ? (<span className="text-white/40">Loading video presets...</span>) : (<>
          ...
          {value.modelImages && (<div className="mt-2 text-[10px] text-blue-400/70 bg-blue-500/10 border border-blue-500/20 rounded-lg px-3 py-1.5">
        ...
        {value.preset === "custom" && (<div className="border-t border-white/5 pt-3 space-y-3">
            {/* Text Generation */}
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input type="number" className="w-full bg-black border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white" value={value.textTemperature ?? 0.7} onChange={(e) => onChangeDraft({ ...value, textTemperature: parseFloat(e.target.value) })} step="0.1" min="0" max="2"/>
                </div>
                <div>
                  <label className="text-[10px] text-white/50">Max Tokens</label>
                  <input type="number" className="w-full bg-black border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white" value={value.textMaxTokens ?? 2048} onChange={(e) => onChangeDraft({ ...value, textMaxTokens: parseInt(e.target.value) })} step="256" min="256" max="8192"/>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input type="number" className="w-full bg-black border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white" value={value.imgWidth ?? currentModelSettings.width} onChange={(e) => onChangeDraft({ ...value, imgWidth: parseInt(e.target.value) })} step="64" min="256" max="2048"/>
                </div>
                <div>
                  <label className="text-[10px] text-white/50">
                    Height <span className="text-white/30">(rec: {currentModelSettings.height})</span>
                  </label>
                  <input type="number" className="w-full bg-black border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white" value={value.imgHeight ?? currentModelSettings.height} onChange={(e) => onChangeDraft({ ...value, imgHeight: parseInt(e.target.value) })} step="64" min="256" max="2048"/>
                </div>
                <div>
                  <label className="text-[10px] text-white/50">
                    Steps <span className="text-white/30">(rec: {currentModelSettings.steps})</span>
                  </label>
                  <input type="number" className="w-full bg-black border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white" value={value.imgSteps ?? currentModelSettings.steps} onChange={(e) => onChangeDraft({ ...value, imgSteps: parseInt(e.target.value) })} step="1" min="1" max="100"/>
                </div>
                <div>
                  <label className="text-[10px] text-white/50">
                    CFG Scale <span className="text-white/30">(rec: {currentModelSettings.cfg})</span>
                  </label>
                  <input type="number" className="w-full bg-black border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white" value={value.imgCfg ?? currentModelSettings.cfg} onChange={(e) => onChangeDraft({ ...value, imgCfg: parseFloat(e.target.value) })} step="0.5" min="1" max="20"/>
                </div>
                <div className="col-span-2">
                  <label className="text-[10px] text-white/50">Seed (0 = random)</label>
                  <input type="number" className="w-full bg-black border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white" value={value.imgSeed ?? 0} onChange={(e) => onChangeDraft({ ...value, imgSeed: parseInt(e.target.value) })} step="1" min="0"/>
                </div>
              </div>
              {currentArchitecture === "sd15" && (value.imgWidth ?? 0) > 768 && (<div className="mt-2 text-[10px] text-yellow-400/80 bg-yellow-500/10 border border-yellow-500/20 rounded-lg px-3 py-1.5">
                  ⚠️ SD 1.5 models work best at max 768px. Higher resolutions may cause duplicate subjects.
                </div>)}
              {currentArchitecture === "flux_schnell" && (value.imgSteps ?? 0) > 6 && (<div className="mt-2 text-[10px] text-yellow-400/80 bg-yellow-500/10 border border-yellow-500/20 rounded-lg px-3 py-1.5">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input type="number" className="w-full bg-black border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white" value={value.vidSeconds ?? 5} onChange={(e) => onChangeDraft({ ...value, vidSeconds: parseInt(e.target.value) })} step="1" min="1" max="30"/>
                </div>
                <div>
                  <label className="text-[10px] text-white/50">FPS</label>
                  <input type="number" className="w-full bg-black border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white" value={value.vidFps ?? 24} onChange={(e) => onChangeDraft({ ...value, vidFps: parseInt(e.target.value) })} step="1" min="8" max="60"/>
                </div>
                <div className="col-span-2">
                  <label className="text-[10px] text-white/50">Motion Bucket</label>
                  <input type="text" className="w-full bg-black border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white" value={value.vidMotion ?? "127"} onChange={(e) => onChangeDraft({ ...value, vidMotion: e.target.value })} placeholder="127"/>
                </div>
              </div>
            </div>
          </div>)}
        ...
        <div className="border-t border-white/5 pt-3">
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={value.ttsEnabled ?? true} onChange={(e) => onChangeDraft({ ...value, ttsEnabled: e.target.checked })} className="w-4 h-4 rounded"/>
            ...
            {ttsEngineId === 'web-speech-api' && (<div>
                <label className="block text-[10px] text-white/50 mb-2">Assistant Voice</label>
                <select className="w-full bg-black border border-white/10 rounded-lg px-3 py-2 text-xs text-white" value={value.selectedVoice ?? ''} onChange={(e) => onChangeDraft({ ...value, selectedVoice: e.target.value })}>
                  <option value="">System Default</option>
                  {availableVoices.map((voice) => (<option key={voice.voiceURI} value={voice.name}>
        ...
        <div className="border-t border-white/5 pt-3">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => onChangeDraft({ ...value, multimodalAuto: !(value.multimodalAuto ?? true) })} className={[
            "relative inline-flex h-6 w-11 items-center rounded-full transition-colors",
            ...
            <div>
              ...
              <select value={value.multimodalTopology || 'smart'} onChange={(e) => onChangeDraft({ ...value, multimodalTopology: e.target.value })} className="w-full bg-black border border-white/10 rounded-lg px-3 py-2 text-xs text-white/80 focus:outline-none focus:border-purple-500/50">
          ...
          <div className="space-y-2">
            <div>
              ...
              <div className="flex items-center gap-3">
                <input type="range" min={1} max={3} step={1} value={value.teamsConcurrentCalls ?? 1} onChange={(e) => onChangeDraft({ ...value, teamsConcurrentCalls: parseInt(e.target.value) })} className="flex-1 h-1.5 rounded-full appearance-none bg-white/10 accent-cyan-500"/>
        ...
        <div className="border-t border-white/5 pt-3">
          ...
          <div className="space-y-2.5">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => onChangeDraft({ ...value, matrixHubEnabled: !value.matrixHubEnabled })} className={`w-9 h-5 rounded-full transition-colors relative ${value.matrixHubEnabled ? 'bg-violet-500' : 'bg-white/20'}`}>
                <span className={`block w-3.5 h-3.5 rounded-full bg-white shadow transition-transform absolute top-[3px] ${value.matrixHubEnabled ? 'left-[19px]' : 'left-[3px]'}`}/>
              </button>
            </div>

            {value.matrixHubEnabled && (<div className="space-y-1.5">
                <label className="text-[10px] text-white/50 block">MatrixHub URL</label>
                <input type="text" value={value.matrixHubUrl || ''} onChange={(e) => onChangeDraft({ ...value, matrixHubUrl: e.target.value })} placeholder="http://localhost:8080" className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-xs text-white placeholder-white/25 focus:outline-none focus:border-violet-500/50 transition-colors font-mono"/>
        ...
        <div className="border-t border-white/5 pt-3">
          ...
          <div className="space-y-2.5">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => onChangeDraft({ ...value, ollaBridgeEnabled: !value.ollaBridgeEnabled })} className={`w-9 h-5 rounded-full transition-colors relative ${value.ollaBridgeEnabled ? 'bg-emerald-500' : 'bg-white/20'}`}>
                <span className={`block w-3.5 h-3.5 rounded-full bg-white shadow transition-transform absolute top-[3px] ${value.ollaBridgeEnabled ? 'left-[19px]' : 'left-[3px]'}`}/>
              </button>
            </div>

            {value.ollaBridgeEnabled && (<div className="space-y-2">
                <div className="space-y-1.5">
                  <label className="text-[10px] text-white/50 block">API Key</label>
                  <input type="text" value={value.ollaBridgeApiKey || 'my-secret'} onChange={(e) => onChangeDraft({ ...value, ollaBridgeApiKey: e.target.value })} placeholder="my-secret" className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-xs text-white placeholder-white/25 focus:outline-none focus:border-emerald-500/50 transition-colors font-mono"/>
                  <div className="text-[10px] text-white/30">
        ...
        <div className="border-t border-white/5 pt-4 mt-2 space-y-3">
          ...
          <div className="flex items-center justify-between gap-4">
            <button type="button" onClick={() => setShowProfileSettings(true)} className="text-[11px] px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-white/70 hover:text-white transition-all">
              Profile &amp; Integrations
            </button>
            <button type="button" onClick={onSave} className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold transition-colors">
              Save Settings
            </button>
          </div>
        </div>
      ...
      {/* Profile & Integrations modal (additive) */}
      {showProfileSettings ? (<ProfileSettingsModal backendUrl={value.backendUrl} apiKey={value.apiKey} nsfwMode={!!value.nsfwMode} onClose={() => setShowProfileSettings(false)}/>) : null}
