...
export const TVModePlayer = ({ scene, isTransitioning, transitionDuration, isImageLoading = false, isPrefetching = false, }) => {
    ...
    const prevSceneRef = useRef(undefined);
    ...
    if (!displayedScene) {
        return (<div className="tv-player-empty">
    ...
    return (<div className="tv-player">
      ...
      <div className={`scene-image-container ${isAnimating ? "transitioning" : ""}`}>
        {imageUrl ? (<img src={imageUrl} alt={`Scene ${displayedScene.idx + 1}`} className={`scene-image ${imageLoaded ? "loaded" : ""}`} onLoad={handleImageLoad}/>) : isImageLoading ? (<div className="scene-loading">
            <div className="image-loading-spinner"/>
            <p className="loading-text">Generating image...</p>
            <p className="loading-subtext">The story will continue when ready</p>
          </div>) : (<div className="scene-placeholder">
      ...
      <div className={`narration-container ${settings.narrationPosition === "top" ? "narration-top" : "narration-bottom"}`}>
        <div className={`narration-text ${getNarrationSizeClass()} ${isAnimating ? "" : "visible"}`}>
          {settings.showSceneNumber && (<span className="scene-indicator">
      ...
      {isLoadingNextChapter && (<div className="chapter-loading-overlay">
