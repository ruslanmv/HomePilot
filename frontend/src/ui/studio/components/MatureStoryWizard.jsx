 ...
 */
export function MatureStoryWizard({ videoId, onPromptReady }) {
    const [step, setStep] = useState(1);
    const [guide, setGuide] = useState(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    // Form state
    const [tone, setTone] = useState("sensual");
    const [explicitness, setExplicitness] = useState("suggestive");
    ...
    const renderStep1 = () => (<div className="space-y-4">
      ...
      {guide ? (<div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-3 border rounded bg-green-500/5">
              ...
              <ul className="text-xs mt-2 space-y-1">
                {guide.allowed_elements.map((item, i) => (<li key={i} className="flex items-start gap-1">
            ...
            <div className="p-3 border rounded bg-red-500/5">
              ...
              <ul className="text-xs mt-2 space-y-1">
                {guide.blocked_elements.map((item, i) => (<li key={i} className="flex items-start gap-1">
        ...
        </div>) : (<div className="text-sm opacity-70">Loading guidelines...</div>)}

      <div className="flex justify-end">
        <button className="px-4 py-2 rounded border bg-primary text-primary-foreground" onClick={() => setStep(2)}>
    ...
    const renderStep2 = () => (<div className="space-y-4">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-2">
          {TONE_OPTIONS.map((opt) => (<label key={opt.value} className={`flex items-start gap-2 p-3 border rounded cursor-pointer ${tone === opt.value ? "border-primary bg-primary/5" : ""}`}>
              <input type="radio" name="tone" value={opt.value} checked={tone === opt.value} onChange={(e) => setTone(e.target.value)} className="mt-1"/>
              <div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-2">
          {EXPLICITNESS_OPTIONS.map((opt) => (<label key={opt.value} className={`flex items-start gap-2 p-3 border rounded cursor-pointer ${explicitness === opt.value ? "border-primary bg-primary/5" : ""}`}>
              <input type="radio" name="explicitness" value={opt.value} checked={explicitness === opt.value} onChange={(e) => setExplicitness(e.target.value)} className="mt-1"/>
              <div>
                <div className="font-medium text-sm">{opt.label}</div>
                <div className="text-xs opacity-70">{opt.description}</div>
              </div>
      ...
      <div className="flex justify-between">
        ...
        <button className="px-4 py-2 rounded border bg-primary text-primary-foreground" onClick={() => setStep(3)}>
          Continue
        </button>
      </div>
    </div>);
    ...
    const renderStep3 = () => (<div className="space-y-4">
      ...
      <div>
        ...
        <input className="w-full border rounded px-3 py-2 text-sm" placeholder="e.g., Candlelit room in a Parisian apartment, evening" value={setting} onChange={(e) => setSetting(e.target.value)}/>
      </div>

      <div>
        <label className="text-sm font-medium block mb-1">Characters</label>
        <input className="w-full border rounded px-3 py-2 text-sm" placeholder="e.g., two consenting adults who reunite after years apart" value={characters} onChange={(e) => setCharacters(e.target.value)}/>
      ...
      <div>
        ...
        <textarea className="w-full border rounded px-3 py-2 text-sm resize-none" rows={4} placeholder="Describe the scene, emotional context, and what you want to happen. Focus on feelings and atmosphere..." value={prompt} onChange={(e) => setPrompt(e.target.value)}/>
        <div className="text-xs opacity-60 mt-1">
          Tip: Focus on emotional connection and atmosphere, not physical actions
        </div>
      </div>

      {guide && (<div className="p-3 border rounded bg-muted/30">
          <div className="text-xs font-medium mb-1">Tips:</div>
          <ul className="text-xs opacity-70 space-y-1">
            {guide.tips.slice(0, 3).map((tip, i) => (<li key={i}>• {tip}</li>))}
          </ul>
        </div>)}

      {error && (<div className="p-3 border rounded bg-red-500/10 border-red-500/30 text-sm">
          {error}
        </div>)}
      ...
      <div className="flex justify-between">
        ...
        <button className="px-4 py-2 rounded border bg-primary text-primary-foreground disabled:opacity-50" onClick={preparePrompt} disabled={!prompt.trim() || loading}>
          {loading ? "Checking..." : "Prepare Prompt"}
        </button>
      </div>
    </div>);
    // Step 4: Review prepared prompt
    const renderStep4 = () => (<div className="space-y-4">
      <div className="text-lg font-semibold">Step 4: Review & Generate</div>

      {policyResult?.ok && (<div className="p-3 border rounded bg-green-500/10 border-green-500/30">
          <div className="font-medium text-sm text-green-600 dark:text-green-400">
            ✓ Policy Check Passed
          </div>
      ...
      {preparedPrompt && (<div className="space-y-3">
          <div className="p-3 border rounded">
      ...
      <div className="flex justify-between">
        ...
        <button className="px-4 py-2 rounded border bg-primary text-primary-foreground" onClick={() => {
            if (preparedPrompt) {
    ...
    return (<div className="max-w-2xl mx-auto p-4">
      ...
      <div className="flex items-center gap-2 mb-6">
        {[1, 2, 3, 4].map((s) => (<React.Fragment key={s}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${step >= s
