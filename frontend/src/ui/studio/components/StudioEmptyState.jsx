 ...
 */
export function StudioEmptyState({ title = 'No scenes yet', description = 'Generate your first scene to start bringing your story to life.', isGenerating = false, generatingLabel = 'Generating first scene...', onGenerateFirstScene, buttonLabel = 'Generate First Scene', className = '', }) {
    return (<div className={`flex-1 flex items-center justify-center ${className}`}>
      <div className="text-center p-8 max-w-md">
        ...
        <div className="flex flex-col items-center">
            ...
            {onGenerateFirstScene && (<button onClick={onGenerateFirstScene} className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 hover:opacity-90 rounded-full text-white font-semibold transition-all hover:scale-[1.02] active:scale-[0.98]" type="button">
                <Plus size={18}/>
                {buttonLabel}
              </button>)}
          </div>)}
