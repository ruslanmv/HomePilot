...
export function ToolCard({ item, onClick }) {
    ...
    return (<div onClick={onClick} className="flex flex-col gap-3 p-5 rounded-2xl bg-white/5 hover:bg-white/10 transition-all duration-200 cursor-pointer border border-white/10 hover:border-white/20 h-full group">
      ...
      <div className="flex justify-between items-start gap-3">
        ...
        {/* Type chip */}
        <span className={`shrink-0 text-[10px] px-2 py-0.5 rounded-full font-semibold uppercase tracking-wide ${isA2A
      ...
      <div className="flex flex-wrap gap-1.5">
        {badges.map((b) => (<span key={b.label} className={`text-xs px-2 py-0.5 rounded-full font-medium ${b.bg} ${b.color}`}>
            {b.label}
          </span>))}
      </div>
    </div>);
