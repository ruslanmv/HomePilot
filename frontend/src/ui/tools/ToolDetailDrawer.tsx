...
import type { CapabilityItem, CatalogA2AAgent } from '../../agentic/types'
...
export function ToolDetailDrawer({ item, onClose }: Props) {
  const { kind, data } = item
  const isA2A = kind === 'a2a_agent'
  const a2aData = isA2A ? (data as CatalogA2AAgent) : null

  const badges = computeBadges(data.name, data.description || '', isA2A ? 'A2A' : undefined)
      ...
      >
        ...
        <div className="px-6 py-6 space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {badges.map((b) => (
              <span key={b.label} className={`text-xs px-2.5 py-1 rounded-full font-medium ${b.bg} ${b.color}`}>
                {b.label}
              </span>
            ))}
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-white/70">
              {data.enabled === true && (
                isA2A
                  ? 'This A2A agent is active and available for use in personas and agent projects.'
                  : 'This tool is active and available for use in agent projects.'
              )}
              {data.enabled === false && (
                isA2A
                  ? 'This A2A agent is registered but currently disabled.'
                  : 'This tool is registered but currently disabled.'
              )}
              {data.enabled == null && 'Status information is not available from the registry.'}
            </p>
          </div>
        </div>
      </div>
