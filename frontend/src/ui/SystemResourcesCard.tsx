...
import { fetchSystemResources, type SystemResourcesResponse } from './systemResourcesApi'

/* ── status helpers ────────────────────────────────── */

function pillTone(status?: string) {
  if (status === 'critical' || status === 'tight') return 'bg-red-500/8 text-red-300/80 border-red-400/15'
  if (status === 'warning') return 'bg-amber-500/8 text-amber-300/80 border-amber-400/15'
  if (status === 'unavailable') return 'bg-white/[0.04] text-white/35 border-white/[0.06]'
  return 'bg-emerald-500/8 text-emerald-300/80 border-emerald-500/15'
}

function barColor(status?: string) {
  if (status === 'critical' || status === 'tight') return 'bg-red-400/80'
...
export default function SystemResourcesCard({
  ...
  return (
    <div
      className="rounded-3xl border border-white/[0.07] bg-white/[0.02] p-6 mb-7"
      style={{ animation: 'statusCardIn 350ms ease-out 240ms both' }}
    >
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <ResourceItem
          icon={<Gauge size={15} />}
          title="GPU VRAM"
          subtitle={data.gpu.available ? (data.gpu.name || 'NVIDIA GPU') : 'GPU unavailable'}
          value={
            data.gpu.available
              ? `${((data.gpu.vram_used_mb ?? 0) / 1024).toFixed(1)} / ${((data.gpu.vram_total_mb ?? 0) / 1024).toFixed(1)} GB`
              : '\u2014'
          }
          percent={data.gpu.used_percent ?? 0}
          status={data.gpu.status}
          extra={data.gpu.available ? `${data.gpu.utilization_percent ?? 0}% GPU load` : 'No nvidia-smi detected'}
        />
        ...
        <ResourceItem
          ...
          value={`${(data.ram.used_mb / 1024).toFixed(1)} / ${(data.ram.total_mb / 1024).toFixed(1)} GB`}
          percent={data.ram.percent}
          status={data.ram.status}
          extra={`${(data.ram.available_mb / 1024).toFixed(1)} GB available`}
        />
        ...
        <ResourceItem
          icon={<Cpu size={15} />}
          title="CPU"
          subtitle={data.cpu.name || 'Processor'}
          value={`${data.cpu.percent}% load`}
          percent={data.cpu.percent}
          status={data.cpu.status}
          extra={`${data.cpu.logical_cores} logical cores`}
        />
        ...
        <ResourceItem
          ...
          title="Disk"
          subtitle={data.disk.path}
          value={`${data.disk.free_gb} GB free`}
          percent={data.disk.percent}
          status={data.disk.status}
          extra={`${data.disk.used_gb} / ${data.disk.total_gb} GB used`}
        />
      </div>
    </div>
  )
