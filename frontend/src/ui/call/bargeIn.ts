...
export interface UseBargeInDetectorArgs {
  /** Shared ref mutated at rAF cadence by the parent — typically
   *  ``voice.audioLevel`` mirrored into a ref so we avoid stale
   *  reads without thrashing React.  0..1 range. */
  audioLevelRef: React.MutableRefObject<number>
  /** Only fire barge-ins when this is true. Wire this to
   *  ``tts.isSpeaking && session.streamingNegotiated``. */
  enabled: boolean
  /** Callback fired once per speech-start episode. Debounced via
   *  ``minRefractoryMs`` to avoid double-firing on sustained speech. */
  onBargeIn: () => void
  /** Audio level (0..1) above which the tap considers the user to
   *  be speaking. Default 0.08 — see header comment on TTS bleed. */
  threshold?: number
  /** How long the level must stay above ``threshold`` before we
   *  declare speech-start. 80 ms matches the minimum voiced-phoneme
   *  duration, so a single keyboard clack doesn't trigger. */
  minSustainMs?: number
  /** After firing, ignore further triggers for this long. Prevents
   *  a single barge-in from firing twice while speech continues. */
  minRefractoryMs?: number
}
 ...
 */
export function useBargeInDetector({
  audioLevelRef,
  enabled,
  onBargeIn,
  threshold = 0.08,
...
export function detectBargeIn(
  samples: Array<{ tMs: number; level: number }>,
  opts: {
    threshold: number
    minSustainMs: number
    minRefractoryMs: number
  ...
  for (const { tMs, level } of samples) {
    if (level >= opts.threshold) {
      if (aboveSince === null) aboveSince = tMs
      else if (
        tMs >= refractoryUntil &&
