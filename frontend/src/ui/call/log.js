...
export function isCallFullDuplexEnabled() {
    ...
    const envVal = import.meta.env?.VITE_CALL_FULL_DUPLEX_ENABLED;
 ...
 */
export function speakOwned(source, text, hooks) {
    if (typeof window === 'undefined')
        return false;
    const t = (text ?? '').trim();
    if (!t)
