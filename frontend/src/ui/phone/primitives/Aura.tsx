...
export type AuraMood = 'calm' | 'warm' | 'alert'

export interface AuraProps {
  /** Identity. Same string → same hue across renders. */
  seed: string
  size?: number           // default 160 (px)
  /** When true AND the user allows motion, the hue drifts ±6° over
   *  5 s. Pure polish — the identity is already conveyed by the
   *  seed, so disabling this is lossless. */
  animated?: boolean
  /** Hue offset: calm +0, warm +15, alert -30. */
  mood?: AuraMood
  /** When set, the inner disc is filled with this image and the
   *  seeded gradient shows only as a rim. Use for personas with a
   *  real portrait. */
...
function moodOffset(mood: AuraMood | undefined): number {
  switch (mood) {
    case 'warm':
      return 15
    case 'alert':
...
const Aura: React.FC<AuraProps> = ({
  ...
  const hue = useMemo(
    () => (hashHue(seed) + moodOffset(mood) + 360) % 360,
    [seed, mood],
  )
  ...
  // repeatedly.
  React.useEffect(() => {
    if (motionOK) ensureAuraKeyframes()
  }, [motionOK])
      ...
      {photoUrl ? (
        ...
        <div
          ...
          style={{
            ...
            inset: 0,
            background:
              'radial-gradient(circle at 40% 25%, rgba(255,255,255,0.35) 0%, transparent 45%)',
          }}
        />
      )}
    </div>
...
// the style block.
function ensureAuraKeyframes(): void {
  if (typeof document === 'undefined') return
  const flag = '__hp_aura_keyframes__'
  const w = window as unknown as { [k: string]: boolean | undefined }
  if (w[flag]) return
  w[flag] = true
