...
export interface AmbientAuraProps {
  /** Hue in degrees (0–359). Derive from persona seed via
   *  Aura's hashHue for consistency, or pass a fixed value for
   *  feature/section backdrops. */
  hue: number
  /** Glow strength. 0 = invisible; 1 = saturated. Default 0.55 —
   *  matches what the old inline backdrop used. */
  intensity?: number
  /** Default true. Disabled under prefers-reduced-motion. */
  animated?: boolean
}

const AmbientAura: React.FC<AmbientAuraProps> = ({
  hue,
  intensity = 0.55,
  animated = true,
}) => {
  ...
  if (motionOK) ensureAmbientAuraKeyframes()
...
function ensureAmbientAuraKeyframes(): void {
