...
export type ControlBtnTone = 'neutral' | 'active' | 'danger'
export type ControlBtnSize = 'sm' | 'md' | 'lg'

export interface ControlBtnProps {
  icon: React.ReactNode
  /** Accessible + visible caption. Mandatory because every action
   *  button must have a screen-reader label — there's no path to
   *  an unlabelled control in this component. */
  label: string
  onClick?: () => void
  tone?: ControlBtnTone
  size?: ControlBtnSize
  disabled?: boolean
  /** Defaults to ``true``. Set false in layouts where a caption
   *  would wrap / collide with neighbours (e.g. the tight 3-up
   *  row in CallModal). The aria-label still reflects ``label``. */
  showLabel?: boolean
  /** Defaults to ``false``. Reflects toggle state to ATs; useful
   *  for mic / speaker. Independent of ``tone`` since 'active'
   *  tone is optional — a neutral-toned toggle can still report
   *  aria-pressed. */
  pressed?: boolean
  className?: string
}

const SIZES: Record<ControlBtnSize, number> = {
  sm: 48,
  md: 60,
  lg: 72,
}

function toneStyles(tone: ControlBtnTone): {
  background: string
  color: string
  border: string
  shadow: string
    ...
    >
      <button
        ...
        style={{
          ...
          transition:
            'transform 100ms ease, background 120ms ease, box-shadow 120ms ease',
