...
function getLayer(container: Element, idx: number): HTMLElement {
  const root = container.firstElementChild as HTMLElement
  return root.children[idx] as HTMLElement
}
...
describe('AmbientAura', () => {
  ...
  it('intensity scales the primary layer alpha', () => {
    ...
    expect(getLayer(lo.container, 0).style.background).not.toBe(
      getLayer(hi.container, 0).style.background,
    )
  })
  ...
  it('hue normalizes negatives (-20 → 340)', () => {
    ...
    expect(getLayer(a.container, 0).style.background).toBe(
      getLayer(b.container, 0).style.background,
    )
  })

  it('animated=false sets no animationName', () => {
    const { container } = render(
      <AmbientAura hue={340} animated={false} />,
    )
    const a = getLayer(container, 0)
    const b = getLayer(container, 1)
  ...
  it('reduced-motion disables animation even when animated=true', () => {
    ...
    const { container } = render(
      <AmbientAura hue={340} animated={true} />,
    )
