...
const Icon: React.FC = () => <svg data-testid="icon" width="10" height="10" />
...
describe('ControlBtn', () => {
  ...
  it('suppresses onClick when disabled', () => {
    const onClick = vi.fn()
    render(
      <ControlBtn icon={<Icon />} label="End" onClick={onClick} disabled />,
    )
  ...
  it('renders each tone with a distinct background', () => {
    const { rerender } = render(
      <ControlBtn icon={<Icon />} label="x" tone="neutral" />,
    )
    const bg = (
      name: string,
  ...
  it('sizes map to 48 / 60 / 72 px', () => {
    const { rerender } = render(
      <ControlBtn icon={<Icon />} label="x" size="sm" />,
    )
    const read = () =>
      (screen.getByRole('button', { name: 'x' }) as HTMLElement).style.width
  ...
  it('reflects pressed state via aria-pressed when provided', () => {
    const { rerender } = render(
      <ControlBtn icon={<Icon />} label="Mic" pressed={false} />,
    )
