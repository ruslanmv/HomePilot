...
function getRootStyle(container: Element): CSSStyleDeclaration {
...
describe('Aura', () => {
  ...
  it('same seed → identical gradient string across renders', () => {
    ...
    expect(getRootStyle(a.container).background).toBe(
      getRootStyle(b.container).background,
    )
  })
  ...
  it('different seeds → different gradient strings', () => {
    ...
    expect(getRootStyle(a.container).background).not.toBe(
      getRootStyle(b.container).background,
    )
  })
  ...
  it('mood shifts the gradient vs the default (calm)', () => {
    ...
    expect(getRootStyle(calm.container).background).not.toBe(
      getRootStyle(warm.container).background,
    )
    expect(getRootStyle(calm.container).background).not.toBe(
      getRootStyle(alert.container).background,
    )
    expect(getRootStyle(warm.container).background).not.toBe(
      getRootStyle(alert.container).background,
    )
  })
  ...
  it('photoUrl path renders an <img> and drops the gradient', () => {
    const { container } = render(
      <Aura seed="vesper" photoUrl="https://example.test/avatar.png" />,
    )
  ...
  it('animated=false disables the hue-drift animation', () => {
    const { container } = render(
      <Aura seed="vesper" animated={false} />,
    )
    const root = container.firstElementChild as HTMLElement
