...
export type WaveformMode = 'listening' | 'speaking' | 'muted' | 'idle'

export interface WaveformProps {
  mode: WaveformMode
  /** Ref to a 0..1 intensity value, mutated each frame by the
   *  parent (mic RMS or synthesised speech envelope). Omit for a
   *  low-ambient "breathing" waveform. */
  intensityRef?: React.MutableRefObject<number>
  bars?: number          // default 26
  height?: number        // default 24 (px)
  /** Used to seed per-bar amplitude + phase so two waveforms with
   *  different personas look subtly different. Same seed = same
   *  shape, frame-after-frame, which helps with screenshot tests. */
  seed?: string
  /** Decorative by default — no keyboard/SR surface. Pass false
   *  only when the waveform is the primary indicator of something
   *  (rare; we usually pair it with text). */
  ariaHidden?: boolean
}

function modeColor(mode: WaveformMode): string {
  switch (mode) {
    case 'speaking':
    case 'listening':
      return CALL.rose
