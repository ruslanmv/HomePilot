...
describe('PostCallCard — expand variant (default)', () => {
    ...
    it('shows the transcript button when a transcript is supplied', () => {
        render(<PostCallCard durationSec={42} personaName="Vesper" transcript={[{ who: 'user', text: 'hello' }]}/>);
    ...
    it('expands the transcript inline (NOT a modal) when clicked', () => {
        render(<PostCallCard durationSec={42} personaName="Vesper" transcript={[
                { who: 'user', text: 'hello' },
    ...
    it('collapses the transcript on a second click', () => {
        render(<PostCallCard durationSec={42} personaName="Vesper" transcript={[{ who: 'assistant', text: 'hey there' }]}/>);
    ...
    it('fires onResume when Resume call is clicked', () => {
        ...
        render(<PostCallCard durationSec={42} personaName="Vesper" onResume={onResume}/>);
...
describe('PostCallCard — highlights variant', () => {
    it('shows the summary line and a "Full transcript" button when handler supplied', () => {
        const onOpen = vi.fn();
        render(<PostCallCard durationSec={600} personaName="Vesper" variant="highlights" summary="You talked about Lisbon and made plans for Friday." onOpenFullTranscript={onOpen}/>);
        expect(screen.getByText(/Vesper remembers/i)).toBeInTheDocument();
        expect(screen.getByText(/You talked about Lisbon and made plans for Friday/i)).toBeInTheDocument();
    ...
    it('omits the "Full transcript" button when no handler is supplied', () => {
        render(<PostCallCard durationSec={600} personaName="Vesper" variant="highlights" summary="…"/>);
        expect(screen.queryByRole('button', { name: /open full transcript/i })).not.toBeInTheDocument();
    });
});
describe('PostCallCard — missed variant', () => {
    it('swaps Resume → Call back and omits secondary CTA', () => {
        const onCallBack = vi.fn();
        render(<PostCallCard durationSec={0} personaName="Vesper" missed onCallBack={onCallBack}/>);
    ...
    it('falls back to onResume when onCallBack is omitted', () => {
        const onResume = vi.fn();
        render(<PostCallCard durationSec={0} personaName="Vesper" missed onResume={onResume}/>);
        fireEvent.click(screen.getByRole('button', { name: /call back/i }));
        expect(onResume).toHaveBeenCalledTimes(1);
    });
});
