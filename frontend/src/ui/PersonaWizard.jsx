...
// Style presets — SFW
// ---------------------------------------------------------------------------
const STYLE_PRESETS = ['Executive', 'Elegant', 'Romantic', 'Casual'];
const STYLE_HINTS = {
...
// ---------------------------------------------------------------------------
const NSFW_STYLE_PRESETS = ['Seductive', 'Lingerie', 'Pin-Up', 'Fantasy'];
const NSFW_STYLE_HINTS = {
...
const BODY_TYPES = ['Slim', 'Athletic', 'Curvy', 'Voluptuous', 'Petite'];
const OUTFIT_HINTS = {
...
function autoDescription(g, stylePreset, current) {
    const looksDefault = !current ||
...
// Component
// ---------------------------------------------------------------------------
export function PersonaWizard({ backendUrl, apiKey, onClose, onCreated, initialDraft }) {
    ...
    const [nsfwStylePreset, setNsfwStylePreset] = useState('Seductive');
    const [bodyType, setBodyType] = useState('Curvy');
    const [outfit, setOutfit] = useState('None specified');
    ...
    const canNext = useMemo(() => {
        if (step === 0)
            return draft.persona_class !== undefined;
        if (step === 1)
            return (draft.persona_agent?.label ?? '').trim().length > 0;
        if (step === 2)
            return !!draft.persona_appearance.selected?.image_id;
        if (step === 3)
            return true; // Voice is always optional
        return true;
    }, [step, draft]);
    // -------------------------------------------------------------------------
    // Step 0: Select class
    // -------------------------------------------------------------------------
    function onSelectClass(bp) {
        ...
        const stylePreset = bp.defaults.style_preset;
        setDraft({
            persona_class: bp.id,
        ...
        if (isNsfw && NSFW_STYLE_PRESETS.includes(stylePreset)) {
            setNsfwStylePreset(stylePreset);
        }
        setCharacterDesc('');
        setStep(1);
    }
    ...
    function buildPrompt() {
        ...
        const nameAndRole = role && role.toLowerCase() !== personaName.toLowerCase()
            ? `${personaName}, ${role}`
            : personaName;
    ...
    async function onGenerateSet() {
        ...
        try {
            ...
            const identityRef = generationMode === 'identity' && selectedImageUrl ? selectedImageUrl : undefined;
            ...
            const selected = draft.persona_appearance.selected ?? (images[0] ? { set_id, image_id: images[0].id } : undefined);
    ...
    async function onCreate() {
        ...
        try {
            ...
            const appearance = (project?.persona_appearance ?? {});
    ...
    const selectedImageUrl = useMemo(() => {
        ...
        for (const s of draft.persona_appearance.sets) {
            for (const img of s.images) {
                if (img.id === sel.image_id && img.set_id === sel.set_id)
                    return img.url;
            }
        }
        return null;
    ...
    // Step labels
    // -------------------------------------------------------------------------
    const STEP_LABELS = ['Choose Class', 'Identity & Skills', 'Appearance', 'Voice', 'Review'];
    // -------------------------------------------------------------------------
    // Render
    // -------------------------------------------------------------------------
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-4xl bg-[#1a1a2e] rounded-2xl border border-white/10 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              {STEP_LABELS.map((label, i) => (<React.Fragment key={i}>
                  {i > 0 && <span className="text-white/20 text-xs">&rsaquo;</span>}
                  <span className={`text-xs ${i === step
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
          ...
          {step === 0 && (<div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                ...
                return (<button key={bp.id} type="button" onClick={() => onSelectClass(bp)} className={`relative flex flex-col items-start p-4 rounded-xl border-2 transition-all text-left group hover:scale-[1.02] ${isSelected
                      ...
                      {bp.defaults.capabilities.length > 0 && (<div className="flex items-center gap-1 mt-2">
                      ...
                      {/* Category tag */}
                      {bp.category === 'nsfw' && (<span className="absolute top-2 right-2 text-[9px] px-1.5 py-0.5 rounded-full bg-orange-500/20 border border-orange-500/30 text-orange-300 font-medium">
                          18+
                        </span>)}
                    </button>);
            })}
          ...
          {step === 1 && (<div className="space-y-5">
              {/* Class badge */}
              {currentBlueprint && currentBlueprint.id !== 'custom' && (<div className="flex items-center gap-2 mb-2">
                  <span className="text-lg">{currentBlueprint.icon}</span>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${CLASS_COLORS[currentBlueprint.color]?.bg ?? 'bg-white/10'} ${CLASS_COLORS[currentBlueprint.color]?.text ?? 'text-white/60'} border ${CLASS_COLORS[currentBlueprint.color]?.border ?? 'border-white/10'}`}>
              ...
              <div className="space-y-3">
                ...
                <input type="text" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all" value={draft.persona_agent.label} onChange={(e) => setDraft({
                ...draft,
              ...
              <div className="space-y-3">
                ...
                <input type="text" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all" value={draft.persona_agent.role ?? ''} onChange={(e) => setDraft({
                ...draft,
              ...
              <div className="space-y-3">
                ...
                <textarea className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all min-h-[100px] resize-y" value={draft.persona_agent.system_prompt ?? ''} onChange={(e) => setDraft({
                ...draft,
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {['warm', 'professional', 'playful', 'assertive', ...(isSpicy ? ['flirty'] : [])].map((tone) => (<button key={tone} type="button" onClick={() => setDraft({
                    ...draft,
                    persona_agent: {
                        ...draft.persona_agent,
                        response_style: { ...draft.persona_agent.response_style, tone },
                    },
                })} className={`px-4 py-2 rounded-full border text-sm capitalize transition-all ${draft.persona_agent.response_style?.tone === tone
              ...
              <div className="space-y-3">
                ...
                <input type="text" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all text-sm" value={draft.agentic.goal} onChange={(e) => setDraft({
                ...draft,
              ...
              <div className="space-y-3">
                ...
                return (<button key={cap.id} type="button" onClick={() => {
                        const next = active
                        ...
                        <div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors ${active ? 'bg-emerald-500 border-emerald-500' : 'border-white/20'}`}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {GENDERS.map((g) => (<button key={g.value} type="button" onClick={() => {
                    const newDesc = autoDescription(g.value, draft.persona_appearance.style_preset, characterDesc);
                    setDraft({
                        ...draft,
                        persona_appearance: { ...draft.persona_appearance, gender: g.value },
                    });
                    setCharacterDesc(newDesc);
                }} className={`px-4 py-2 rounded-full border text-sm transition-all ${(draft.persona_appearance.gender ?? 'female') === g.value
                    ? 'bg-purple-500/20 border-purple-500/50 text-purple-300'
                    : 'bg-white/5 border-white/10 text-white/60 hover:bg-white/10 hover:text-white/80'}`}>
                          {g.label}
                        </button>))}
                  ...
                  {isSpicy ? (<>
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {NSFW_STYLE_PRESETS.map((p) => (<button key={p} type="button" onClick={() => setNsfwStylePreset(p)} className={`px-4 py-2 rounded-full border text-sm transition-all ${nsfwStylePreset === p
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {BODY_TYPES.map((bt) => (<button key={bt} type="button" onClick={() => setBodyType(bt)} className={`px-4 py-2 rounded-full border text-sm transition-all ${bodyType === bt
                        ? 'bg-pink-500/20 border-pink-500/50 text-pink-300'
                        : 'bg-white/5 border-white/10 text-white/60 hover:bg-white/10 hover:text-white/80'}`}>
                              {bt}
                            </button>))}
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {Object.keys(OUTFIT_HINTS).map((o) => (<button key={o} type="button" onClick={() => setOutfit(o)} className={`px-4 py-2 rounded-full border text-sm transition-all ${outfit === o
                        ? 'bg-pink-500/20 border-pink-500/50 text-pink-300'
                        : 'bg-white/5 border-white/10 text-white/60 hover:bg-white/10 hover:text-white/80'}`}>
                              {o}
                            </button>))}
                      ...
                      <div className="space-y-3">
                        ...
                        <input type="text" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 transition-all text-sm" value={customPromptExtra} onChange={(e) => setCustomPromptExtra(e.target.value)} placeholder="e.g., long red hair, green eyes, tattoo on shoulder"/>
                      </div>
                    </>) : (<div className="space-y-3">
                      <label className="block text-sm font-medium text-white/80">Style preset</label>
                      <div className="flex flex-wrap gap-2">
                        {STYLE_PRESETS.map((p) => (<button key={p} type="button" onClick={() => setDraft({
                        ...draft,
                        persona_appearance: { ...draft.persona_appearance, style_preset: p },
                    })} className={`px-4 py-2 rounded-full border text-sm transition-all ${draft.persona_appearance.style_preset === p
                        ? 'bg-pink-500/20 border-pink-500/50 text-pink-300'
                        : 'bg-white/5 border-white/10 text-white/60 hover:bg-white/10 hover:text-white/80'}`}>
                            {p}
                          </button>))}
                  ...
                  <div className="space-y-3">
                    ...
                    <textarea className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all min-h-[60px] resize-y text-sm" value={characterDesc} onChange={(e) => setCharacterDesc(e.target.value)} placeholder="e.g., long brown hair, green eyes, fair skin, elegant face..."/>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {['low', 'med', 'high'].map((q) => (<button key={q} type="button" onClick={() => setDraft({
                    ...draft,
                    persona_appearance: { ...draft.persona_appearance, img_preset: q },
                })} className={`px-4 py-2 rounded-full border text-sm capitalize transition-all ${draft.persona_appearance.img_preset === q
                    ? 'bg-purple-500/20 border-purple-500/50 text-purple-300'
                    : 'bg-white/5 border-white/10 text-white/60 hover:bg-white/10 hover:text-white/80'}`}>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <button type="button" onClick={() => setGenerationMode('standard')} className={`flex-1 px-4 py-2.5 rounded-xl border text-sm transition-all text-left ${generationMode === 'standard'
                ...
                : 'Install Avatar & Identity Models (Add-ons tab) to enable'} className={`flex-1 px-4 py-2.5 rounded-xl border text-sm transition-all text-left ${generationMode === 'identity'
                ...
                : 'Requires Avatar Models'}
                    ...
                    {generationMode === 'identity' && (<p className="text-[10px] text-emerald-300/50 px-1">
                        {selectedImageUrl
                  ...
                  <div className="relative flex items-center gap-0">
                    <button type="button" onClick={onGenerateSet} disabled={generating} className="flex-1 px-6 py-3 bg-purple-500 hover:bg-purple-600 disabled:opacity-60 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-l-xl transition-all flex items-center justify-center gap-2">
                      {generating ? (<>
                    ...
                    <button type="button" onClick={() => setShowFaceCountPicker(!showFaceCountPicker)} disabled={generating} className="px-3 py-3 bg-purple-600 hover:bg-purple-700 disabled:opacity-60 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-r-xl border-l border-purple-400/30 transition-all" title="Change number of faces to generate">
                      ({faceCount})
                    </button>
                    {showFaceCountPicker && (<div className="absolute right-0 bottom-full mb-1 bg-gray-800 border border-white/10 rounded-lg shadow-xl z-50 flex flex-col overflow-hidden">
                        {[1, 2, 3, 4].map((n) => (<button key={n} type="button" onClick={() => { setFaceCount(n); setShowFaceCountPicker(false); }} className={`px-4 py-2 text-sm text-white hover:bg-purple-500/40 transition-colors ${faceCount === n ? 'bg-purple-500/30 font-semibold' : ''}`}>
                            {n}
                          </button>))}
                      </div>)}
                  </div>

                  {genError && (<div className="text-sm text-red-400 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3">
                      {genError}
                    </div>)}

                  {allImages.length > 0 && (<div className="text-xs text-white/40">
                ...
                <div className="space-y-3">
                  ...
                  {allImages.length === 0 ? (<div className="flex flex-col items-center justify-center h-64 bg-white/5 border border-white/10 rounded-xl text-white/40 text-sm">
                      <Sparkles size={32} className="mb-3 opacity-40"/>
                      Generate a set to view options
                    </div>) : (<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 max-h-[440px] overflow-y-auto custom-scrollbar pr-1">
                      {allImages.map((img) => {
                    const isSel = draft.persona_appearance.selected?.set_id === img.set_id &&
                        draft.persona_appearance.selected?.image_id === img.id;
                    return (<button key={img.id} type="button" onClick={() => setDraft({
                            ...draft,
                            persona_appearance: {
                                ...draft.persona_appearance,
                                selected: { set_id: img.set_id, image_id: img.id },
                            },
                        })} className={`relative overflow-hidden rounded-xl border-2 transition-all ${isSel
                            ...
                            {isSel && (<div className="absolute top-2 right-2 text-xs bg-purple-500 px-2 py-1 rounded-full font-medium shadow-lg">
                                Selected
                              </div>)}
                          </button>);
                })}
          ...
          {/* ===== STEP 3: Voice ===== */}
          {step === 3 && (<PersonaVoiceStep gender={draft.persona_appearance.gender ?? 'female'} personaName={draft.persona_agent?.label || 'Persona'} value={draft.persona_voice} onChange={(v) => setDraft((d) => ({ ...d, persona_voice: v }))}/>)}
          ...
          {step === 4 && (<div className="space-y-5">
              <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    {currentBlueprint && currentBlueprint.id !== 'custom' && (<span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${CLASS_COLORS[currentBlueprint.color]?.bg ?? 'bg-white/10'} ${CLASS_COLORS[currentBlueprint.color]?.text ?? 'text-white/60'} border ${CLASS_COLORS[currentBlueprint.color]?.border ?? 'border-white/10'}`}>
                        {currentBlueprint.label} Class
                      </span>)}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Left: avatar preview */}
                  <div>
                    {selectedImageUrl ? (<img src={resolveFileUrl(selectedImageUrl)} className="w-full max-w-[280px] rounded-xl border border-white/10" alt="Selected avatar"/>) : (<div className="w-full h-64 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-white/40 text-sm">
                        No avatar selected
                      </div>)}
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    return (<span key={capId} className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/15 border border-emerald-500/20 text-emerald-300">
                                {cap?.label ?? capId}
                              </span>);
                })}
                        </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        {['adaptive', 'basic'].map((mode) => (<button key={mode} type="button" onClick={() => setDraft({ ...draft, memory_mode: mode })} className={`text-xs px-3 py-1.5 rounded-full border transition-all ${draft.memory_mode === mode
                    ? mode === 'adaptive'
                ...
                : 'Only remembers what is explicitly saved. Best for deterministic enterprise workflows.'}
                    ...
                    {draft.persona_agent.system_prompt && (<div>
                    ...
                    <div className="text-xs text-white/40 mt-2">
                      ...
                      Mode: {generationMode === 'identity' ? (<span className="text-emerald-300/70">Same Person</span>) : 'Standard'}
                    </div>
                ...
                const count3d = outfits.filter((o) => o.view_pack && Object.keys(o.view_pack).length >= 2).length;
                return (<div className="mt-2 rounded-lg bg-white/[0.03] border border-white/[0.06] p-2.5 space-y-1.5">
                          <div className="flex items-center gap-1.5 text-xs text-purple-300/80">
                            <Shirt size={12}/>
                            <span className="font-medium">
                              {outfits.length} outfit{outfits.length !== 1 ? 's' : ''} included
                            </span>
                            {count3d > 0 && (<span className="inline-flex items-center gap-0.5 ml-1 px-1.5 py-0 rounded-full bg-cyan-500/15 border border-cyan-500/20 text-cyan-300/80 text-[9px] font-medium">
                        ...
                        return (<span key={o.id} className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-white/50">
                                  {o.label || 'Outfit'}
                                  {has3d && (<span className="inline-flex items-center gap-0.5 text-cyan-300/70">
                    ...
                    {draft.persona_appearance.avatar_settings && (<div className="text-[10px] text-white/30 bg-white/[0.03] border border-white/5 rounded-lg p-2">
              ...
              {/* Voice chip */}
              {draft.persona_voice?.voiceURI && (<div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <div className="text-xs text-white/40 mb-2">Voice</div>
                  <PersonaVoiceChip personaName={draft.persona_agent?.label || 'Persona'} voice={draft.persona_voice}/>
                </div>)}
        ...
        <div className="p-6 border-t border-white/10 bg-[#1a1a2e] flex justify-between">
          <button onClick={() => (step === 0 ? onClose() : setStep((step - 1)))} className="px-4 py-2 text-sm font-medium text-white/60 hover:text-white transition-colors flex items-center gap-1">
            {step === 0 ? ('Cancel') : (<>
                <ChevronLeft size={16}/>
                Back
              </>)}
          ...
          <div className="flex gap-2">
            {step > 0 && step < 4 && (<button disabled={!canNext} onClick={() => setStep((step + 1))} className="px-6 py-2 bg-purple-500 hover:bg-purple-600 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-full transition-colors flex items-center gap-2">
                Next <ChevronRight size={16}/>
              </button>)}

            {step === 4 && (<button disabled={saving || !draft.persona_appearance.selected?.image_id} onClick={onCreate} className="px-6 py-2 bg-purple-500 hover:bg-purple-600 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-full transition-colors">
