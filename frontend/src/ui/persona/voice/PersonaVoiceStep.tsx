...
import type { PersonaVoiceConfig, PersonaGender } from '../../personaTypes'
...
export function PersonaVoiceStep({
  gender,
  personaName,
  value,
  onChange,
  ...
  personaName: string
  value?: PersonaVoiceConfig
  onChange: (v: PersonaVoiceConfig) => void
}) {
  ...
  const voices: SpeechSynthesisVoice[] = useMemo(() => {
    return (window as any).SpeechService?.getVoices?.() || []
  ...
  const pickSuggestedVoiceURI = useCallback((): string => {
  ...
  const testSpeak = useCallback(async () => {
    ...
    await svc.speakWithConfig(
      testText,
      { voiceURI: currentURI, rate: currentRate, pitch: currentPitch, volume: currentVolume },
  ...
  const update = useCallback(
    (partial: Partial<PersonaVoiceConfig>) => {
  ...
  const selectedVoice = useMemo(
    () => voices.find((v) => v.voiceURI === currentURI),
    [voices, currentURI],
  )
      ...
      <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="px-3 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-xs text-white/70 hover:bg-white/[0.08] transition-colors flex items-center gap-2"
          >
            <RefreshCcw size={13} />
            Suggest
          </button>
      ...
      <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm text-white/80 focus:outline-none focus:border-purple-500/40 transition-colors"
          />
          <button
            type="button"
            onClick={testSpeak}
  ...
  return (
    <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
      ...
      <input
        ...
        className="w-full h-1 mt-1 rounded-full bg-white/[0.06] appearance-none cursor-pointer accent-purple-400 [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:bg-purple-400 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:appearance-none"
      />
    </div>
  )
}
