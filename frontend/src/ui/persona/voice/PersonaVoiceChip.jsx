...
export function PersonaVoiceChip({ personaName, voice, size = 'md', }) {
    ...
    const handlePreview = useCallback(async () => {
        ...
        await svc.speakWithConfig(text, {
    ...
    return (<div className={`inline-flex items-center gap-1.5 rounded-full border bg-white/[0.03] border-white/[0.08] ${isSmall ? 'px-2 py-0.5 text-[10px]' : 'px-3 py-1.5 text-xs'} text-white/50`}>
      <span className="truncate max-w-[140px]">{voiceLabel}</span>

      <button type="button" onClick={handlePreview} disabled={speaking} className={`p-0.5 rounded-full transition-colors ${speaking
            ? 'text-purple-300'
            : 'text-white/35 hover:text-purple-300'}`} title="Preview voice">
