...
import { Play, Plus, Settings2, X, Tv2, Film, Trash2, ChevronLeft, BookOpen, Users, MapPin, Maximize2, } from 'lucide-react';
...
async function deleteJson(baseUrl, path, apiKey) {
    const url = `${baseUrl.replace(/\/+$/, '')}${path.startsWith('/') ? path : `/${path}`}`;
    const res = await fetch(url, {
        method: 'DELETE',
        headers: {
...
export default function StudioView(props) {
    ...
    const getDefaultStoryMode = () => {
        try {
            return localStorage.getItem('hp_default_story_mode');
        }
        catch {
            return null;
        }
    };
    const setDefaultStoryMode = (mode) => {
        try {
    ...
    const unifiedProjects = useMemo(() => {
        const playProjects = sessions.map((s) => ({
            id: s.id,
            title: s.title,
            description: s.premise,
            type: 'play',
            status: 'draft', // Play stories are always "in progress"
            updatedAt: s.updated_at * 1000,
            raw: s,
        }));
        const creatorProjectsMapped = creatorProjects.map((p) => ({
            id: p.id,
            title: p.title,
            description: p.logline,
            type: 'creator',
            status: p.status === 'approved' ? 'finished' : 'draft',
    ...
    const imageQueueRef = useRef(new Map());
    const MAX_IMAGE_RETRIES = 3;
    const imageInFlightRef = useRef(null);
    ...
    const loadSessions = async () => {
        ...
        try {
            ...
            const [storyData, creatorData] = await Promise.allSettled([
                fetchJson(props.backendUrl, '/story/sessions/list', authKey),
                fetchJson(props.backendUrl, '/studio/videos', authKey),
            ]);
    ...
    const createStory = async () => {
        if (!premise.trim() || isCreating)
            return;
        setIsCreating(true);
        try {
            // Step 1: Create story bible
            const storyData = await postJson(props.backendUrl, '/story/start', {
            ...
            const sceneData = await postJson(props.backendUrl, '/story/next', {
            ...
            const fullStory = await fetchJson(props.backendUrl, `/story/${storyData.session_id}`, authKey);
    ...
    const loadStory = async (sessionId) => {
        try {
            const data = await fetchJson(props.backendUrl, `/story/${sessionId}`, authKey);
    ...
    const deleteStory = async (sessionId) => {
    ...
    const deleteCreatorProject = async (projectId) => {
        if (!confirm('Delete this project and all its scenes?'))
            return;
        try {
    ...
    const deleteScene = async (sceneIdx) => {
        if (!currentStory) {
        ...
        try {
            const result = await deleteJson(props.backendUrl, `/story/${currentStory.session_id}/scene/${sceneIdx}`, authKey);
    ...
    const generateNextScene = async () => {
        if (!currentStory || isGeneratingScene)
            return;
        setIsGeneratingScene(true);
        try {
            const data = await postJson(props.backendUrl, '/story/next', {
            ...
            if (data.story_complete) {
                ...
                try {
                    const chapterData = await postJson(props.backendUrl, '/story/continue', {
                    ...
                    const firstSceneData = await postJson(props.backendUrl, '/story/next', {
                    ...
                    const fullStory = await fetchJson(props.backendUrl, `/story/${chapterData.session_id}`, authKey);
                    setCurrentStory(fullStory);
                    setCurrentSceneIndex(0);
                    setStoryComplete(false);
    ...
    const processImageQueue = useCallback(async () => {
        // Already processing?
        if (imageInFlightRef.current !== null)
            return;
        ...
        const [nextIdx, sceneData] = nextEntry;
        ...
        try {
            ...
            const data = await postJson(props.backendUrl, '/chat', {
            ...
            if (imageUrl) {
                ...
                try {
                    await postJson(props.backendUrl, '/story/scene/image', {
    ...
    const generateImageForScene = useCallback((scene, sessionId, force = false) => {
        // Already has image? Skip unless force regenerate
        const hasImage = Boolean(scene.image_url || scene.image_url || scene.image);
        if (hasImage && !force)
            return;
    ...
    const handleEnterTVMode = useCallback(() => {
        if (!currentStory || currentStory.scenes.length === 0)
            return;
    ...
    const continueAsNextChapter = useCallback(async () => {
        if (!currentStory)
            return null;
        try {
            const data = await postJson(props.backendUrl, '/story/continue', {
                previous_session_id: currentStory.session_id,
            }, authKey);
            console.log(`[TV Mode] Started chapter ${data.chapter_number}: ${data.title}`);
            // Load the new chapter's story data
            const newStoryData = await fetchJson(props.backendUrl, `/story/${data.session_id}`, authKey);
    ...
    const continueChapterInStudio = useCallback(async () => {
        if (!currentStory || isCreatingChapter)
            return;
    ...
    if (view === 'list') {
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-white/10">
          ...
          <button onClick={handleNewStoryClick} className="flex items-center gap-2 bg-purple-500 hover:bg-purple-600 px-4 py-2 rounded-full text-sm font-semibold transition-all" type="button">
            <Plus size={16}/>
            Create Story
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loadingSessions ? (<div className="flex items-center justify-center h-64">
              <div className="animate-spin w-8 h-8 border-2 border-purple-500 border-t-transparent rounded-full"/>
            </div>) : unifiedProjects.length === 0 ? (<div className="flex flex-col items-center justify-center h-64 text-white/50">
              ...
              {unifiedProjects.map((project) => (<div key={`${project.type}-${project.id}`} className={`bg-white/5 border rounded-2xl p-5 hover:border-white/20 transition-colors group cursor-pointer ${project.type === 'creator' ? 'border-blue-500/30' : 'border-white/10'}`} onClick={() => {
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${project.type === 'creator'
                        ? 'bg-blue-500/20 text-blue-300'
                        : 'bg-purple-500/20 text-purple-300'}`}>
                      {project.type === 'creator' ? 'Creator Studio' : 'Play Story'}
                    </span>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${project.status === 'finished'
                  ...
                  <div className="flex items-start justify-between mb-3">
                    ...
                    }} className="opacity-0 group-hover:opacity-100 p-1 text-red-400 hover:text-red-300 transition-all" type="button" title="Delete project">
                      <Trash2 size={16}/>
                    </button>
                  </div>
        ...
        {/* Mode Chooser Modal */}
        {showModeChooser && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
            <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-8 max-w-lg w-full mx-4 shadow-2xl">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => setShowModeChooser(false)} className="p-2 text-white/50 hover:text-white hover:bg-white/10 rounded-full transition-colors">
                  <X size={20}/>
                </button>
              </div>
              ...
              <div className="space-y-4">
                {/* Play Studio Option */}
                <button onClick={handleChoosePlay} className="w-full p-6 bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/30 rounded-xl text-left hover:border-purple-500/60 hover:bg-purple-500/20 transition-all group">
                ...
                {props.onOpenCreatorStudio && (<button onClick={handleChooseCreator} className="w-full p-6 bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/30 rounded-xl text-left hover:border-blue-500/60 hover:bg-blue-500/20 transition-all group">
                    <div className="flex items-start gap-4">
    ...
    if (view === 'create') {
        ...
        <div className="flex items-center px-6 py-4 border-b border-white/10">
          <button onClick={() => setView('list')} className="mr-4 p-2 text-white/50 hover:text-white hover:bg-white/5 rounded-full transition-colors" type="button">
            <ChevronLeft size={20}/>
          </button>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-purple-500/20 to-pink-500/20 border border-white/10 flex items-center justify-center">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="max-w-2xl mx-auto space-y-6">
            <div>
              ...
              <textarea value={premise} onChange={(e) => setPremise(e.target.value)} placeholder="A cyberpunk detective in Naples solves surreal crimes in the neon rain..." className="w-full h-40 bg-white/5 border border-white/10 rounded-xl p-4 text-white placeholder-white/30 focus:border-purple-500/50 focus:outline-none resize-none"/>
            ...
            <div>
              ...
              <input type="text" value={titleHint} onChange={(e) => setTitleHint(e.target.value)} placeholder="Neon Tide" className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white placeholder-white/30 focus:border-purple-500/50 focus:outline-none"/>
            </div>

            <button onClick={createStory} disabled={!premise.trim() || isCreating} className={`w-full py-4 rounded-xl font-semibold text-lg transition-all flex items-center justify-center gap-2 ${premise.trim() && !isCreating
    ...
    // Render: Player
    // -----------------------------------------------------------------------------
    return (<div ref={playerRef} className="h-full w-full bg-black text-white font-sans overflow-hidden flex flex-col">
      {/* Header */}
      {!isFullscreen && (<div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
          <div className="flex items-center gap-4">
            <button onClick={() => {
                setView('list');
                setIsPlaying(false);
                loadSessions();
            }} className="p-2 text-white/50 hover:text-white hover:bg-white/5 rounded-full transition-colors" type="button">
              <ChevronLeft size={20}/>
            </button>
            <div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setShowBible(true)} className="p-2 text-white/50 hover:text-white hover:bg-white/5 rounded-full transition-colors" type="button" title="Story Bible">
              <BookOpen size={20}/>
            </button>
            <button onClick={toggleFullscreen} className="p-2 text-white/50 hover:text-white hover:bg-white/5 rounded-full transition-colors" type="button" title="Fullscreen">
              <Maximize2 size={20}/>
            </button>
          </div>
        </div>)}
      ...
      {/* Scene Chips Rail */}
      {currentStory && currentStory.scenes.length > 0 && (<SceneChips scenes={currentStory.scenes.map((scene) => ({
                idx: scene.idx,
                status: scene.image_url ? 'ready' : (isGeneratingImage && scene.idx === currentScene?.idx ? 'generating' : 'pending'),
                thumbnailUrl: scene.image_url,
            }))} activeIndex={currentScene?.idx ?? 0} onSelect={(sceneIdx) => {
                // Convert scene.idx to array index
                const arrayIndex = currentStory.scenes.findIndex(s => s.idx === sceneIdx);
                if (arrayIndex >= 0)
                    setCurrentSceneIndex(arrayIndex);
            }} onDelete={deleteScene} className="border-b border-white/10"/>)}

      {/* Main Preview Area */}
      {currentStory && currentStory.scenes.length > 0 ? (<StudioPreviewPanel imageUrl={currentScene?.image_url} isGenerating={isGeneratingImage} narration={currentScene?.narration} prompt={currentScene?.image_prompt} onRegenerateImage={currentScene ? () => generateImageForScene(currentScene, undefined, true) : undefined}/>) : (<StudioEmptyState isGenerating={isGeneratingScene} generatingLabel="Creating your first scene..." onGenerateFirstScene={generateNextScene}/>)}

      {/* Action Bar */}
      <StudioActions isPlaying={isPlaying} onTogglePlay={() => setIsPlaying(!isPlaying)} canGoBack={currentSceneIndex > 0} canGoForward={currentStory ? currentSceneIndex < currentStory.scenes.length - 1 : false} onPrevScene={() => setCurrentSceneIndex((prev) => Math.max(0, prev - 1))} onNextScene={() => setCurrentSceneIndex((prev) => Math.min((currentStory?.scenes.length || 1) - 1, prev + 1))} isGeneratingScene={isGeneratingScene} onGenerateNextScene={generateNextScene} isStoryComplete={isStoryComplete} onContinueChapter={continueChapterInStudio} isCreatingChapter={isCreatingChapter} onShowChapterSettings={() => setShowChapterSettings(true)} currentIndex={currentSceneIndex} totalScenes={currentStory?.scenes.length || 0} onSelectScene={setCurrentSceneIndex} isMuted={isMuted} onToggleMute={() => setIsMuted(!isMuted)} onFullscreen={toggleFullscreen} onEnterTVMode={handleEnterTVMode} tvModeDisabled={!currentStory || currentStory.scenes.length === 0}/>

      {/* Story Bible Modal */}
      {showBible && currentStory && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4 backdrop-blur-md" onClick={() => setShowBible(false)}>
          <div className="max-w-3xl w-full max-h-[80vh] bg-[#121212] border border-white/10 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-white/10 flex items-center justify-between">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <BookOpen size={20} className="text-purple-400"/>
                Story Bible
              </h3>
              <button type="button" onClick={() => setShowBible(false)} className="text-white/50 hover:text-white">
                <X size={20}/>
              </button>
            </div>
            ...
            <div className="overflow-y-auto max-h-[60vh] p-6 space-y-6">
              ...
              {currentStory.bible.recurring_characters.length > 0 && (<div>
                  <h4 className="text-sm font-semibold text-white/50 uppercase tracking-wider mb-2 flex items-center gap-2">
                    <Users size={14}/>
                    Characters
                  </h4>
                  <div className="space-y-2">
                    {currentStory.bible.recurring_characters.map((char, i) => (<div key={i} className="bg-white/5 rounded-lg p-3">
              ...
              {currentStory.bible.recurring_locations.length > 0 && (<div>
                  <h4 className="text-sm font-semibold text-white/50 uppercase tracking-wider mb-2 flex items-center gap-2">
                    <MapPin size={14}/>
                    Locations
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {currentStory.bible.recurring_locations.map((loc, i) => (<span key={i} className="px-3 py-1 bg-white/5 rounded-full text-sm text-white/70">
                        {loc}
                      </span>))}
                  </div>
                </div>)}

              {currentStory.bible.visual_style_rules.length > 0 && (<div>
                  <h4 className="text-sm font-semibold text-white/50 uppercase tracking-wider mb-2">
                    Visual Style Rules
                  </h4>
                  <ul className="space-y-1">
                    {currentStory.bible.visual_style_rules.map((rule, i) => (<li key={i} className="text-sm text-white/70">
                        • {rule}
                      </li>))}
                  </ul>
                </div>)}

              {currentStory.bible.do_not_change.length > 0 && (<div>
                  <h4 className="text-sm font-semibold text-white/50 uppercase tracking-wider mb-2">
                    Consistency Rules
                  </h4>
                  <ul className="space-y-1">
                    {currentStory.bible.do_not_change.map((rule, i) => (<li key={i} className="text-sm text-white/70">
                        • {rule}
                      </li>))}
                  </ul>
                </div>)}
      ...
      {showChapterSettings && isStoryComplete && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4 backdrop-blur-md" onClick={() => setShowChapterSettings(false)}>
          <div className="max-w-lg w-full bg-[#121212] border border-white/10 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-white/10 flex items-center justify-between">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <BookOpen size={20} className="text-purple-400"/>
                New Chapter Settings
              </h3>
              <button type="button" onClick={() => setShowChapterSettings(false)} className="text-white/50 hover:text-white">
                <X size={20}/>
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea value={chapterHint} onChange={(e) => setChapterHint(e.target.value)} placeholder="e.g., 'Focus on the mysterious stranger from scene 3' or 'Add a plot twist involving...'" className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-purple-500/50 resize-none" rows={3}/>
              </div>

              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowChapterSettings(false)} className="flex-1 px-4 py-3 bg-white/5 hover:bg-white/10 text-white rounded-xl transition-colors">
                  Cancel
                </button>
                <button onClick={continueChapterInStudio} disabled={isCreatingChapter} className="flex-1 px-4 py-3 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-medium rounded-xl transition-colors disabled:opacity-50">
      ...
      {/* TV Mode Overlay */}
      {tvModeActive && (<TVModeContainer onGenerateNext={generateNextForTVMode} onEnsureImage={generateImageForScene} onContinueChapter={continueAsNextChapter}/>)}
    </div>);
}
