 ...
 * `status === 'ready'` job becomes the current scene.
 *
 * Production fixes in this version:
 *  - Persona idle portrait is centered and does not crop the face.
 *  - Portrait-oriented scene stills render with contain + blurred fill.
 *  - Avatar chip uses object-top so face thumbnails crop better.
 */
...
function InteractivePlayerBody({
  ...
  useEffect(() => {
    ...
    async function tick() {
      if (cancelled) return;
      try {
        const res = await api.pending(sessionId, {
          since_id: cursorRef.current || undefined,
        });
        if (cancelled) return;
