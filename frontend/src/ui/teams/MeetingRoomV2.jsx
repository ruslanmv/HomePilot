...
import { ArrowLeft, Send, User, Users, Plus, X, MessageSquare, FileText, ListChecks, Zap, BarChart3, Settings as SettingsIcon, } from 'lucide-react';
...
// Component
// ---------------------------------------------------------------------------
export function MeetingRoomV2({ room, personas, backendUrl, apiKey, onBack, onSendMessage, onAddParticipant, onRemoveParticipant, }) {
    ...
    const inMeeting = useMemo(() => room.participant_ids.map((id) => personaMap.get(id)).filter(Boolean), [room.participant_ids, personaMap]);
    const available = useMemo(() => personas.filter((p) => p.project_type === 'persona' && !room.participant_ids.includes(p.id)), [personas, room.participant_ids]);
    ...
    const TabBtn = ({ id, label, icon }) => {
        ...
        return (<button onClick={() => setRightTab(id)} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-medium border transition-colors ${active
    ...
    return (<div className="h-full w-full bg-black text-white overflow-hidden flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setSettingsOpen(true)} className="p-2 rounded-lg bg-white/[0.04] border border-white/[0.06] text-white/45 hover:text-white/70 hover:bg-white/[0.06] transition-colors" title="Room Settings">
            <SettingsIcon size={16}/>
          </button>
          <button onClick={() => setLeftCollapsed((v) => !v)} className="px-3 py-1.5 rounded-lg text-xs font-medium bg-white/[0.04] border border-white/[0.06] text-white/45 hover:bg-white/[0.06] hover:text-white/70 transition-colors">
      ...
      <div className="flex-1 min-h-0 flex">
        ...
        {/* ── LEFT RAIL (People) ── */}
        <div className={`min-h-0 border-r border-white/[0.06] bg-white/[0.02] transition-all overflow-hidden ${leftCollapsed ? 'w-0 opacity-0 pointer-events-none' : 'w-72 opacity-100'}`}>
          <div className="h-full flex flex-col min-h-0">
            ...
            return (<div key={p.id} className={`flex items-center justify-between gap-2 rounded-xl border px-3 py-2 transition-colors ${isSpeaking
                    ? 'border-emerald-500/25 bg-emerald-500/10'
                    : 'border-white/[0.06] bg-white/[0.02]'}`}>
                        <div className="flex items-center gap-2 min-w-0">
                          <div className="w-8 h-8 rounded-full overflow-hidden border border-white/10 bg-white/5">
                            {avatarUrl ? (<img src={avatarUrl} className="w-full h-full object-cover" alt={p.name}/>) : (<div className="w-full h-full flex items-center justify-center text-[10px] text-white/30 font-bold">
                        ...
                        <button onClick={() => onRemoveParticipant(p.id)} className="p-1.5 rounded-lg bg-white/[0.03] border border-white/[0.06] hover:bg-red-500/10 hover:border-red-500/20 text-white/30 hover:text-red-300 transition-colors" title={`Remove ${p.name}`}>
                          <X size={12}/>
                        </button>
                      </div>);
        })}
              ...
              <div>
                ...
                {available.length === 0 ? (<div className="text-xs text-white/25 italic border border-white/[0.06] rounded-xl p-3 bg-white/[0.02]">
                    No available personas.
                  </div>) : (<div className="space-y-2">
                    {available.map((p) => {
                const avatarUrl = resolveAvatarUrl(p, backendUrl);
                return (<button key={p.id} onClick={() => onAddParticipant(p.id)} className="w-full flex items-center justify-between gap-2 rounded-xl border border-white/[0.06] bg-white/[0.02] hover:bg-cyan-500/[0.05] hover:border-cyan-500/20 px-3 py-2 transition-colors">
                          <div className="flex items-center gap-2 min-w-0">
                            <div className="w-8 h-8 rounded-full overflow-hidden border border-white/10 bg-white/5">
                              {avatarUrl ? (<img src={avatarUrl} className="w-full h-full object-cover" alt={p.name}/>) : (<div className="w-full h-full flex items-center justify-center text-[10px] text-white/30 font-bold">
                                  {p.name[0]?.toUpperCase()}
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          ...
          <div className="flex-1 min-h-0 p-4">
            <div className="h-full rounded-2xl border border-white/[0.06] bg-white/[0.02] relative overflow-hidden">
              ...
              {/* Human seat */}
              {seats.length > 0 && (<div className="absolute -translate-x-1/2 -translate-y-1/2" style={{ left: `${seats[0].x}%`, top: `${seats[0].y}%` }}>
            ...
            const avatarUrl = resolveAvatarUrl(p, backendUrl);
            const isSpeaking = sending && lastMsg?.sender_id === p.id && lastMsg?.role === 'assistant';
            return (<div key={p.id} className="absolute -translate-x-1/2 -translate-y-1/2" style={{ left: `${pos.x}%`, top: `${pos.y}%` }}>
                    <div className="flex flex-col items-center gap-1">
                      <div className={`${v2AvatarSize} rounded-full border-2 overflow-hidden flex items-center justify-center transition-all ${isSpeaking
                    ? 'border-emerald-400 shadow-lg shadow-emerald-500/20'
                    : 'border-white/15 hover:border-white/25'}`}>
                        {avatarUrl ? (<img src={avatarUrl} alt={p.name} className="w-full h-full object-cover"/>) : (<div className="w-full h-full bg-white/5 flex items-center justify-center text-sm text-white/30 font-bold">
                            {p.name[0]?.toUpperCase()}
                          </div>)}
                      </div>
                      <span className={`text-[10px] font-medium truncate max-w-[80px] ${isSpeaking ? 'text-emerald-300/80' : 'text-white/35'}`}>
                        {p.name}
                      </span>
          ...
          <div className="px-4 pb-4">
            <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-3">
              <div className="flex items-end gap-2">
                <textarea value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={handleKeyDown} placeholder="Type a message..." rows={1} className="flex-1 px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-cyan-500/40 transition-colors resize-none"/>
                <button onClick={handleSend} disabled={!message.trim() || sending} className={`p-2.5 rounded-xl transition-all ${message.trim() && !sending
        ...
        <div className="w-[380px] max-w-[42vw] min-h-0 border-l border-white/[0.06] bg-white/[0.02] flex flex-col">
          ...
          <div className="flex-1 min-h-0 overflow-hidden">
            {/* ═══ TRANSCRIPT ═══ */}
            {rightTab === 'transcript' && (<div ref={transcriptRef} className="h-full min-h-0 overflow-y-auto px-3 py-3 space-y-3 scrollbar-hide">
                {(!room.messages || room.messages.length === 0) ? (<div className="text-center py-8">
                ...
                return (<div key={msg.id} className={`flex gap-3 ${isHuman ? 'flex-row-reverse' : ''}`}>
                        ...
                        <div className={`max-w-[78%] ${isHuman ? 'items-end' : ''}`}>
                          ...
                          <div className={`px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed ${isHuman
            ...
            {/* ═══ DOCUMENTS ═══ */}
            {rightTab === 'documents' && (<div className="h-full p-3 overflow-hidden">
                <DocumentsPanel backendUrl={backendUrl} apiKey={apiKey} roomId={room.id} initialDocuments={room.documents || []}/>
              </div>)}

            {/* ═══ AGENDA ═══ */}
            {rightTab === 'agenda' && (<div className="h-full overflow-y-auto p-4 scrollbar-hide">
                <div className="text-xs text-white/70 font-medium mb-2">Agenda</div>
                {room.agenda && room.agenda.length > 0 ? (<div className="space-y-2">
                    {room.agenda.map((item, i) => (<div key={i} className="rounded-lg bg-white/[0.03] border border-white/[0.06] p-3 text-sm text-white/70">
                        <span className="text-white/40 text-xs mr-2">{i + 1}.</span>
                        {item}
                      </div>))}
                  </div>) : (<div className="text-xs text-white/25 italic border border-white/[0.06] rounded-xl p-3 bg-white/[0.02]">
                    No agenda items set. Add agenda items when creating a session.
                  </div>)}
              </div>)}

            {/* ═══ ACTIONS ═══ */}
            {rightTab === 'actions' && (<div className="h-full overflow-y-auto p-4 scrollbar-hide">
            ...
            {/* ═══ STATS ═══ */}
            {rightTab === 'stats' && (<div className="h-full overflow-y-auto p-4 scrollbar-hide">
      ...
      {/* ═══ SETTINGS DRAWER ═══ */}
      <TeamsSettingsDrawer room={room} open={settingsOpen} onClose={() => setSettingsOpen(false)}/>
