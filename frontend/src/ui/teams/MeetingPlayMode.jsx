...
import { Gamepad2, Play, X, Zap, MessageSquare, Swords, Circle, Drama, FlaskConical, } from 'lucide-react';
// ---------------------------------------------------------------------------
// Style presets
// ---------------------------------------------------------------------------
const STYLE_OPTIONS = [
...
// Component
// ---------------------------------------------------------------------------
export function MeetingPlayMode({ playMode, participantCount, turnMode = 'reactive', onStart, onStop, onPause, onResume, }) {
    const [panelOpen, setPanelOpen] = useState(false);
    const [style, setStyle] = useState(playMode?.style || 'discussion');
    ...
    return (<div className="relative">
      {/* ── Toggle Button ── */}
      <button onClick={() => setPanelOpen(!panelOpen)} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold transition-all border ${isActive
            ? 'bg-purple-500/20 border-purple-500/50 text-purple-300'
            : panelOpen
                ? 'bg-purple-500/10 border-purple-500/30 text-purple-300/80'
                : 'bg-white/[0.03] hover:bg-white/[0.06] border-white/[0.06] hover:border-white/12 text-white/40 hover:text-white/60'}`} title="Play Mode — autonomous AI conversation">
        <Gamepad2 size={14}/>
        {isActive && (<span className="w-1.5 h-1.5 rounded-full bg-purple-400 animate-glow-pulse"/>)}
      </button>

      {/* ── Config Panel (only when NOT active) ── */}
      {panelOpen && !isActive && (<div className="absolute top-full right-0 mt-2 w-80 rounded-2xl bg-black/95 backdrop-blur-xl border border-white/10 shadow-2xl z-50 p-5 animate-msg-slide-in">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setPanelOpen(false)} className="p-1 rounded-lg hover:bg-white/5 text-white/30 hover:text-white/60 transition-colors">
              <X size={14}/>
            </button>
          </div>
          ...
          <div className="space-y-4">
            ...
            {turnMode !== 'round-robin' && (<div>
              ...
              <div className="space-y-1.5">
                {STYLE_OPTIONS.map((opt) => (<button key={opt.value} onClick={() => setStyle(opt.value)} className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-left transition-all ${style === opt.value
            ...
            {turnMode === 'round-robin' && (<div className="px-2.5 py-2 rounded-lg bg-purple-500/5 border border-purple-500/10">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                {INTERVAL_PRESETS.map((p) => (<button key={p.value} onClick={() => setIntervalMs(p.value)} className={`flex-1 px-2 py-1.5 rounded-lg text-xs font-medium transition-all ${intervalMs === p.value
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setInfinite(!infinite)} className={`text-[10px] px-1.5 py-0.5 rounded-md font-medium transition-all ${infinite
                ? 'bg-purple-500/20 text-purple-300 border border-purple-500/30'
              ...
              {!infinite && (<input type="range" min={5} max={200} step={5} value={maxRounds} onChange={(e) => setMaxRounds(Number(e.target.value))} className="w-full h-1 rounded-full appearance-none bg-white/[0.06] accent-purple-500 cursor-pointer
                    [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3
                    [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-purple-400 [&::-webkit-slider-thumb]:shadow-md"/>)}
            </div>

            {/* Start button */}
            <button onClick={handleStart} disabled={!canStart || starting} className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${canStart && !starting
            ...
            {!canStart && (<p className="text-[10px] text-red-300/50 text-center">
