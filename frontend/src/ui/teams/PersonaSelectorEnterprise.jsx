...
function inferRoleTag(p) {
    const txt = `${p.name ?? ''} ${p.description ?? ''} ${p.persona_agent?.persona_class ?? ''}`.toLowerCase();
    if (txt.includes('secret') || txt.includes('assistant') || txt.includes('calendar') || txt.includes('minutes'))
        return 'secretary';
    if (txt.includes('analyst') || txt.includes('data') || txt.includes('kpi') || txt.includes('metrics'))
        return 'analyst';
    if (txt.includes('creative') || txt.includes('design') || txt.includes('brand') || txt.includes('copy'))
        return 'creative';
    if (txt.includes('engineer') || txt.includes('devops') || txt.includes('api') || txt.includes('infra'))
        return 'engineer';
    if (txt.includes('research') || txt.includes('study') || txt.includes('evidence'))
        return 'research';
    if (txt.includes('product') || txt.includes('roadmap') || txt.includes('requirements'))
        return 'product';
    return 'all';
}
const ROLE_LABELS = {
...
// Component
// ---------------------------------------------------------------------------
export function PersonaSelectorEnterprise({ personas, backendUrl, selectedIds, onToggle, onSetSelected, bundles = [], }) {
    const [query, setQuery] = useState('');
    const [role, setRole] = useState('all');
    const [sort, setSort] = useState('recent');
    const personaProjects = useMemo(() => personas.filter((p) => p.project_type === 'persona'), [personas]);
    ...
    return (<div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-xs text-white/35">
          ...
          {selectedIds.size > 0 && (<span className="text-cyan-300 ml-1">{selectedIds.size} selected</span>)}
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <div className="flex-1 flex items-center gap-2 rounded-xl bg-white/[0.03] border border-white/[0.06] px-3 py-2 focus-within:border-cyan-500/30 transition-colors">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search personas..." className="flex-1 bg-transparent text-sm text-white placeholder:text-white/20 focus:outline-none"/>
          {query && (<button onClick={() => setQuery('')} className="p-1 rounded-lg hover:bg-white/5 text-white/40 hover:text-white/70 transition-colors" type="button" aria-label="Clear search">
              <X size={14}/>
            </button>)}
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-2 rounded-xl bg-white/[0.03] border border-white/[0.06] px-3 py-2">
            ...
            <select value={role} onChange={(e) => setRole(e.target.value)} className="bg-transparent text-sm text-white/70 focus:outline-none cursor-pointer">
              {Object.keys(ROLE_LABELS).map((r) => (<option key={r} value={r}>Role: {ROLE_LABELS[r]}</option>))}
            </select>
          </div>

          <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] px-3 py-2">
            <select value={sort} onChange={(e) => setSort(e.target.value)} className="bg-transparent text-sm text-white/70 focus:outline-none cursor-pointer">
              <option value="az">Sort: A-Z</option>
              <option value="recent">Sort: Recent</option>
            </select>
          </div>

          {bundles.length > 0 && (<div className="rounded-xl bg-white/[0.03] border border-white/[0.06] px-3 py-2">
              <select value="" onChange={(e) => {
                if (e.target.value)
                    applyBundle(e.target.value);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-white/[0.06] bg-white/[0.02] p-3">
          ...
          {filtered.length === 0 ? (<div className="text-center py-10 text-sm text-white/35">
            ...
            </div>) : (<div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[340px] overflow-y-auto scrollbar-thin">
              {filtered.map((p) => {
                const selected = selectedIds.has(p.id);
                const avatarUrl = resolveAvatarUrl(p, backendUrl);
                const roleTag = inferRoleTag(p);
                return (<button key={p.id} type="button" onClick={() => onToggle(p.id)} className={`flex items-center gap-3 p-3 rounded-xl border transition-all duration-200 text-left ${selected
                    ...
                    <div className="w-10 h-10 rounded-full border border-white/10 bg-white/5 flex-shrink-0 overflow-hidden">
                      {avatarUrl ? (<img src={avatarUrl} alt={p.name} className="w-full h-full object-cover"/>) : (<div className="w-full h-full flex items-center justify-center text-sm text-white/30 font-bold">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {roleTag !== 'all' && (<div className="mt-1 inline-flex text-[10px] px-2 py-0.5 rounded-full bg-white/[0.03] border border-white/[0.06] text-white/45">
                          {ROLE_LABELS[roleTag]}
                        </div>)}
                    </div>
                    <div className={`w-7 h-7 rounded-xl border flex items-center justify-center transition-all duration-200 ${selected
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-3 flex flex-col">
          ...
          {/* Selected personas */}
          {selectedList.length === 0 ? (<div className="text-sm text-white/30 py-6 text-center flex-1 flex items-center justify-center">
              Pick 3-6 personas for best results.
            </div>) : (<div className="space-y-1.5 flex-1 max-h-[220px] overflow-y-auto scrollbar-thin">
              {selectedList.map((p) => {
                const avatarUrl = resolveAvatarUrl(p, backendUrl);
                return (<div key={p.id} className="flex items-center gap-2 p-2 rounded-xl border border-white/[0.06] bg-white/[0.02] group/item">
                    <div className="w-6 h-6 rounded-full border border-white/10 bg-white/5 flex-shrink-0 overflow-hidden">
                      {avatarUrl ? (<img src={avatarUrl} alt={p.name} className="w-full h-full object-cover"/>) : (<div className="w-full h-full flex items-center justify-center text-[8px] text-white/30 font-bold">
                          {(p.name || 'P').charAt(0).toUpperCase()}
                        </div>)}
                    </div>
                    <div className="text-xs text-white font-medium truncate flex-1">{p.name}</div>
                    <button type="button" onClick={() => onToggle(p.id)} className="text-[10px] px-2 py-1 rounded-lg bg-white/[0.03] border border-white/[0.06] text-white/40 hover:text-red-300 hover:bg-red-500/10 hover:border-red-500/20 transition-all duration-150 opacity-0 group-hover/item:opacity-100">
                      Remove
                    </button>
                  </div>);
            })}
