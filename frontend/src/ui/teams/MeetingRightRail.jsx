...
import { X, ListChecks, Zap, BarChart3, Clock, MessageSquare, Users, CheckCircle2, Circle, FileText, Plus, Pencil, Trash2, Check, Target, } from 'lucide-react';
...
// Component
// ---------------------------------------------------------------------------
export function MeetingRightRail({ room, personas, backendUrl, apiKey, onClose, onUpdateAgenda, onUpdateTopic }) {
    const [activeTab, setActiveTab] = useState('agenda');
    const [checkedAgenda, setCheckedAgenda] = useState(new Set());
    ...
    return (<div className="flex-shrink-0 w-64 border-l border-white/[0.04] bg-white/[0.01] flex flex-col animate-rail-slide-right overflow-hidden">
      {/* Tabs */}
      <div className="flex items-center border-b border-white/[0.04]">
        {TABS.map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex-1 flex items-center justify-center gap-1 py-2.5 text-[13px] font-medium transition-colors relative ${activeTab === tab.id
                ? 'text-cyan-300'
                : 'text-white/35 hover:text-white/50'}`}>
            {tab.icon}
            {tab.label}
            {activeTab === tab.id && (<span className="absolute bottom-0 left-1/4 right-1/4 h-[1.5px] bg-cyan-400/60 rounded-full animate-tab-underline"/>)}
          </button>))}
        <button onClick={onClose} className="p-1.5 mx-1 rounded hover:bg-white/5 text-white/20 hover:text-white/40 transition-colors" title="Close panel">
          <X size={12}/>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-3 py-3">
        {/* ═══ AGENDA ═══ */}
        {activeTab === 'agenda' && (<div className="space-y-3">
            {/* ── Main Topic ── */}
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                {onUpdateTopic && !editingTopic && (<button onClick={() => { setEditingTopic(true); setTopicText(room.topic || ''); }} className="p-0.5 rounded hover:bg-white/5 text-white/20 hover:text-white/40 transition-colors" title="Edit topic">
                    <Pencil size={10}/>
                  </button>)}
              </div>
              {editingTopic && onUpdateTopic ? (<div className="flex items-center gap-1.5">
                  <input type="text" value={topicText} onChange={(e) => setTopicText(e.target.value)} onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                    ...
                    if (e.key === 'Escape')
                        setEditingTopic(false);
                }} autoFocus placeholder="What is this discussion about?" className="flex-1 bg-white/[0.04] border border-white/10 rounded-lg px-2.5 py-1.5 text-xs text-white/70 placeholder:text-white/20 focus:outline-none focus:border-cyan-500/30"/>
                  <button onClick={() => { onUpdateTopic(topicText.trim()); setEditingTopic(false); }} className="p-1 rounded hover:bg-cyan-500/10 text-cyan-400/60 hover:text-cyan-300 transition-colors">
                    <Check size={12}/>
                  </button>
                </div>) : (<div className={`px-2.5 py-2 rounded-lg text-xs leading-relaxed ${room.topic
                    ? 'bg-cyan-500/[0.04] border border-cyan-500/10 text-white/55'
                    : 'bg-white/[0.02] border border-white/[0.04] text-white/20 italic'}`} onClick={onUpdateTopic ? () => { setEditingTopic(true); setTopicText(room.topic || ''); } : undefined} style={onUpdateTopic ? { cursor: 'pointer' } : undefined}>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                {onUpdateAgenda && (<button onClick={() => { setAddingNew(true); setNewItemText(''); }} className="p-0.5 rounded hover:bg-white/5 text-white/20 hover:text-white/40 transition-colors" title="Add agenda item">
                    <Plus size={12}/>
                  </button>)}
              </div>

              {/* Add new item input */}
              {addingNew && onUpdateAgenda && (<div className="flex items-center gap-1.5 animate-msg-slide-in">
                  <input type="text" value={newItemText} onChange={(e) => setNewItemText(e.target.value)} onKeyDown={(e) => {
                    ...
                    if (e.key === 'Escape')
                        setAddingNew(false);
                }} autoFocus placeholder="New agenda item..." className="flex-1 bg-white/[0.04] border border-white/10 rounded-lg px-2.5 py-1.5 text-xs text-white/70 placeholder:text-white/20 focus:outline-none focus:border-cyan-500/30"/>
                  <button onClick={() => {
                    if (newItemText.trim()) {
                        onUpdateAgenda([...(room.agenda || []), newItemText.trim()]);
                        setNewItemText('');
              ...
              {(!room.agenda || room.agenda.length === 0) && !addingNew ? (<div className="text-center py-4">
                  <ListChecks size={18} className="mx-auto text-white/10 mb-1.5"/>
                  <p className="text-[11px] text-white/25">No agenda items</p>
                  {onUpdateAgenda && (<button onClick={() => { setAddingNew(true); setNewItemText(''); }} className="mt-2 text-[10px] text-cyan-400/50 hover:text-cyan-300/70 transition-colors">
                      + Add first item
                    </button>)}
                </div>) : ((room.agenda || []).map((item, i) => {
                const done = checkedAgenda.has(i);
                const isEditing = editingIdx === i;
                if (isEditing && onUpdateAgenda) {
                    return (<div key={i} className="flex items-center gap-1.5 animate-msg-slide-in">
                        <input type="text" value={editText} onChange={(e) => setEditText(e.target.value)} onKeyDown={(e) => {
                            ...
                            if (e.key === 'Escape')
                                setEditingIdx(null);
                        }} autoFocus className="flex-1 bg-white/[0.04] border border-white/10 rounded-lg px-2.5 py-1.5 text-xs text-white/70 focus:outline-none focus:border-cyan-500/30"/>
                        <button onClick={() => {
                            if (editText.trim()) {
                                const updated = [...(room.agenda || [])];
                                updated[i] = editText.trim();
                ...
                return (<div key={i} className={`group w-full flex items-start gap-2 px-2.5 py-2 rounded-lg text-left transition-all ${done
                      ...
                      <button onClick={() => toggleAgenda(i)} className="flex-shrink-0 mt-0.5">
                        {done ? (<CheckCircle2 size={13} className="text-emerald-400/70 animate-agenda-check"/>) : (<Circle size={13} className="text-white/15"/>)}
                      </button>
                      <span className={`flex-1 text-[13px] leading-relaxed ${done ? 'text-white/30 line-through' : 'text-white/55'}`}>
                        {item}
                      </span>
                      {/* Edit/Remove controls */}
                      {onUpdateAgenda && (<div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => { setEditingIdx(i); setEditText(item); }} className="p-0.5 rounded hover:bg-white/5 text-white/15 hover:text-white/40 transition-colors" title="Edit">
                            <Pencil size={10}/>
                          </button>
                          <button onClick={() => {
                            const updated = (room.agenda || []).filter((_, idx) => idx !== i);
                            onUpdateAgenda(updated);
                        }} className="p-0.5 rounded hover:bg-red-500/10 text-white/15 hover:text-red-300/60 transition-colors" title="Remove">
                            <Trash2 size={10}/>
                          </button>
                        </div>)}
                    </div>);
            }))}
              {room.agenda && room.agenda.length > 0 && (<div className="text-[10px] text-white/20 text-center mt-2">
        ...
        {/* ═══ DOCUMENTS ═══ */}
        {activeTab === 'documents' && (<DocumentsPanel backendUrl={backendUrl} apiKey={apiKey} roomId={room.id} initialDocuments={room.documents || []}/>)}

        {/* ═══ ACTIONS ═══ */}
        {activeTab === 'actions' && (<div className="space-y-2">
            {actions.length === 0 ? (<div className="text-center py-6">
              ...
              </div>) : (actions.map((action, i) => (<div key={i} className="px-2.5 py-2 rounded-lg bg-white/[0.02] border border-white/[0.04]">
        ...
        {activeTab === 'stats' && (<div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                return (<div key={name} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 h-1.5 rounded-full bg-white/[0.04] overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-500 ${isDominant ? 'bg-amber-400/50' : 'bg-cyan-400/40'}`} style={{ width: `${Math.round((count / maxCount) * 100)}%` }}/>
                      </div>
                      <span className="text-[9px] text-white/25 w-4 text-right">{count}</span>
                      {isDominant && (<span className="text-[8px] text-amber-400/60" title="Dominance suppression active (>40% of messages)">!</span>)}
                    </div>);
            })}
              </div>
            </div>
            ...
            {room.policy && (<div>
                ...
                ].filter(item => item.value !== undefined).map((item) => (<div key={item.label} className="flex items-center justify-between px-2 py-1 rounded bg-white/[0.02]">
                      <span className="text-[9px] text-white/25">{item.label}</span>
                      <span className="text-[9px] text-white/45 font-mono">{item.value}</span>
                    </div>))}
                </div>
                {room.round !== undefined && room.round > 0 && (<div className="mt-2 text-center text-[9px] text-white/20">
                    Round {room.round}
                  </div>)}
              </div>)}
          </div>)}
