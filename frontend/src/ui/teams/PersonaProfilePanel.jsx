...
import { X, Shield, Sparkles, Wrench, Palette, Calendar, Zap, Brain, Swords, Image, Shirt, Target, Volume2, Crown, ChevronRight, Users, Bot, RotateCcw, } from 'lucide-react';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function resolveAvatarUrl(p, backendUrl) {
...
function getEquippedOutfit(persona) {
...
function getOutfitAvailableViews(outfit) {
    const vp = outfit?.view_pack || {};
    return ['front', 'left', 'right', 'back'].filter((a) => !!vp[a]);
}
...
function computeLevel(p) {
    let xp = 1;
    if (p.persona_agent?.system_prompt && p.persona_agent.system_prompt.length > 100)
        xp++;
    if (p.persona_agent?.role)
        xp++;
    if (p.persona_agent?.response_style?.tone)
        xp++;
    if (p.persona_agent?.key_techniques?.length)
        xp++;
    if (p.persona_appearance?.selected_filename)
        xp++;
    if ((p.persona_appearance?.sets?.length || 0) > 0)
        xp++;
    if ((p.persona_appearance?.outfits?.length || 0) > 0)
        xp++;
    if ((p.agentic?.capabilities?.length || 0) > 0)
        xp++;
    if ((p.persona_agent?.allowed_tools?.length || 0) > 2)
        xp++;
...
// Component
// ---------------------------------------------------------------------------
export function PersonaProfilePanel({ persona, backendUrl, onClose, status, onInspectOutfit }) {
    ...
    return (<div className="fixed inset-0 z-50 flex justify-end bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-sm h-full bg-[#0a0a0a] border-l border-white/[0.08] overflow-y-auto animate-rail-slide-right" onClick={(e) => e.stopPropagation()}>
        {/* Close button */}
        ...
        <div className="px-6 pb-5 text-center">
          ...
          <div className="relative inline-block">
            <div className={`w-36 h-36 rounded-full overflow-hidden border-[3px] mx-auto ${status === 'speaking'
            ...
            : status === 'wants-to-speak'
                ...
                : 'border-white/15'}`}>
              {avatarUrl ? (<img src={avatarUrl} alt={persona.name} className="w-full h-full object-cover"/>) : (<div className="w-full h-full bg-white/5 flex items-center justify-center text-4xl text-white/25 font-bold">
          ...
          <div className="flex items-center justify-center gap-2 mt-2">
            ...
            {status && (<span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium bg-white/[0.04] border border-white/[0.08]">
          ...
          {(equippedOutfit || outfitViews.length > 0) && (<div className="flex items-center justify-center gap-2 mt-2.5 flex-wrap">
              {equippedOutfit && (<span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-white/[0.04] border border-white/[0.08] text-white/40">
              ...
              {outfitViews.length > 0 && (<span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-violet-500/10 border border-violet-500/20 text-violet-300/70">
                  <RotateCcw size={9}/>
                  360 preview
                </span>)}
            </div>)}
          ...
          {/* Role subtitle */}
          {agent?.role && (<p className="mt-2 text-xs text-white/40">{agent.role}</p>)}

          {/* Description */}
          {persona.description && (<p className="mt-3 text-[13px] text-white/50 leading-relaxed max-w-xs mx-auto">
              {persona.description}
            </p>)}
        </div>
          ...
          <div className="space-y-2.5">
            {stats.map((stat) => (<div key={stat.label} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-2 rounded-full bg-white/[0.06] overflow-hidden">
                  <div className={`h-full rounded-full ${stat.color} transition-all duration-500`} style={{ width: `${stat.value}%` }}/>
                </div>
        ...
        {/* ═══════════ CURRENT LOOK ═══════════ */}
        {equippedOutfit && (<div className="px-6 py-4 border-t border-white/[0.06]">
            ...
            <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.06]">
              ...
              <div className="min-w-0 flex-1">
                ...
                {outfitViews.length > 0 && (<div className="text-[10px] text-white/30 mt-0.5">
                    360 preview &middot; {outfitViews.length} views
                  </div>)}
              </div>
              {onInspectOutfit && (<button onClick={() => onInspectOutfit(equippedOutfit.id || '', equippedOutfit.hero_view || 'front')} className="px-2.5 py-1 text-[10px] font-medium text-white/50 bg-white/[0.04] border border-white/[0.08] rounded-lg hover:bg-white/[0.08] hover:text-white/70 transition-colors shrink-0">
                  Inspect
                </button>)}
            </div>
            {/* View angle chips */}
            {outfitViews.length > 0 && (<div className="flex gap-1.5 mt-2 px-1">
                {outfitViews.map((angle) => (<button key={angle} onClick={() => onInspectOutfit?.(equippedOutfit.id || '', angle)} className="px-2.5 py-1 text-[10px] font-medium text-white/40 bg-white/[0.03] border border-white/[0.06] rounded-full hover:bg-white/[0.08] hover:text-white/60 transition-colors capitalize">
                    {angle}
                  </button>))}
              </div>)}
            {/* Discoverability hint */}
            {outfitViews.length > 0 && (<p className="text-[10px] text-white/20 mt-2.5 px-1 italic">
        ...
        <div className="px-6 py-4 border-t border-white/[0.06]">
          ...
          <div className="grid grid-cols-2 gap-2">
            {/* Tone */}
            {agent?.response_style?.tone && (<InfoTile icon={<Volume2 size={13}/>} label="Tone" value={agent.response_style.tone}/>)}
            {/* Style */}
            {appearance?.style_preset && (<InfoTile icon={<Palette size={13}/>} label="Style" value={appearance.style_preset}/>)}
            {/* Portraits */}
            <InfoTile icon={<Image size={13}/>} label="Portraits" value={`${portraitCount}`}/>
            {/* Wardrobe */}
            <InfoTile icon={<Shirt size={13}/>} label="Wardrobe" value={`${outfitCount} outfit${outfitCount !== 1 ? 's' : ''}`}/>
            {/* Stance */}
            <InfoTile icon={stance.icon} label="Stance" value={stance.label}/>
            {/* Age */}
            <InfoTile icon={<Calendar size={13}/>} label="Age" value={formatAge(persona.created_at)}/>
          </div>
        </div>

        {/* ═══════════ SKILLS ═══════════ */}
        {skillCount > 0 && (<div className="px-6 py-4 border-t border-white/[0.06]">
            <div className="text-[10px] font-semibold text-white/30 uppercase tracking-wider mb-3">
              Skills ({skillCount})
            </div>
            <div className="space-y-1.5">
              {(agentic?.capabilities || []).map((cap) => (<div key={cap} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/[0.02] border border-white/[0.06]">
        ...
        <div className="px-6 py-4 border-t border-white/[0.06]">
          ...
          {toolCount > 0 ? (<div className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/[0.06]">
            ...
            </div>) : (<div className="text-xs text-white/25 italic">No tools configured</div>)}
          {/* Tool details if available */}
          {agentic?.tool_details && agentic.tool_details.length > 0 && (<div className="mt-2 space-y-1">
              {agentic.tool_details.slice(0, 6).map((t, i) => (<div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-white/[0.01]">
                  <Swords size={10} className="text-white/20"/>
                  <span className="text-[11px] text-white/40 truncate">{t.name}</span>
                </div>))}
              {agentic.tool_details.length > 6 && (<div className="text-[10px] text-white/20 text-center mt-1">
        ...
        {agentic?.agent_details && agentic.agent_details.length > 0 && (<div className="px-6 py-4 border-t border-white/[0.06]">
            <div className="text-[10px] font-semibold text-white/30 uppercase tracking-wider mb-3">
              Party Members ({agentic.agent_details.length})
            </div>
            <div className="space-y-1.5">
              {agentic.agent_details.map((a, i) => (<div key={i} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/[0.02] border border-white/[0.06]">
        ...
        {/* ═══════════ QUEST OBJECTIVE ═══════════ */}
        {agentic?.goal && (<div className="px-6 py-4 border-t border-white/[0.06]">
...
function InfoTile({ icon, label, value }) {
    return (<div className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.06]">
...
function SummaryRow({ label, value, icon }) {
    return (<div className="flex items-center justify-between py-1 px-1">
