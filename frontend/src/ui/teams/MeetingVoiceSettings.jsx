...
// Component
// ---------------------------------------------------------------------------
export function MeetingVoiceSettings({ open, onClose, participants, backendUrl, meetingTtsEnabled, onToggleTts, getPersonaVoice, setPersonaVoice, }) {
    const [advancedOpen, setAdvancedOpen] = useState(new Set());
    const toggleAdvanced = useCallback((personaId) => {
      ...
      <div className="relative w-80 max-w-[90vw] h-full bg-[#0a0a0a] border-l border-white/[0.06] flex flex-col animate-rail-slide-right">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide px-4 py-3 space-y-4">
          ...
          <div className="flex items-center justify-between py-2">
            ...
            <button onClick={() => onToggleTts(!meetingTtsEnabled)} className={`relative w-9 h-5 rounded-full transition-colors ${meetingTtsEnabled
            ? 'bg-emerald-500/25 border border-emerald-500/30'
            : 'bg-white/[0.06] border border-white/[0.08]'}`}>
              <span className={`absolute top-0.5 w-4 h-4 rounded-full transition-all ${meetingTtsEnabled
          ...
          {/* ── Status pill ── */}
          <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-[10px] ${meetingTtsEnabled
          ...
          {meetingTtsEnabled && participants.length > 0 && (<div className="space-y-1">
              ...
              {participants.map((p) => {
                ...
                return (<div key={p.id} className="rounded-lg border border-white/[0.04] overflow-hidden">
                    {/* Voice picker row */}
                    <div className="px-3">
                      <PersonaVoicePicker personaId={p.id} label={p.name} value={cfg} onChange={(newCfg) => setPersonaVoice(p.id, newCfg)}/>
                    </div>

                    {/* Advanced toggle */}
                    <button onClick={() => toggleAdvanced(p.id)} className="w-full flex items-center gap-1 px-3 py-1 text-[9px] text-white/20 hover:text-white/35 transition-colors">
                      {isAdvOpen ? <ChevronDown size={9}/> : <ChevronRight size={9}/>}
                      Rate / Pitch
                    </button>

                    {/* Advanced sliders */}
                    {isAdvOpen && (<div className="px-3 pb-2 space-y-2">
                        {/* Rate */}
                        <div>
                          ...
                          <input type="range" min={0.5} max={2.0} step={0.05} value={cfg?.rate ?? 1.0} onChange={(e) => setPersonaVoice(p.id, { ...(cfg || { voiceURI: '' }), rate: parseFloat(e.target.value) })} className="w-full h-1 rounded-full bg-white/[0.06] appearance-none cursor-pointer accent-cyan-500 [&::-webkit-slider-thumb]:w-2.5 [&::-webkit-slider-thumb]:h-2.5 [&::-webkit-slider-thumb]:bg-cyan-400 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:appearance-none"/>
                        </div>
                        {/* Pitch */}
                        <div>
                          <div className="flex items-center justify-between mb-0.5">
                          ...
                          <input type="range" min={0.5} max={2.0} step={0.05} value={cfg?.pitch ?? 1.0} onChange={(e) => setPersonaVoice(p.id, { ...(cfg || { voiceURI: '' }), pitch: parseFloat(e.target.value) })} className="w-full h-1 rounded-full bg-white/[0.06] appearance-none cursor-pointer accent-cyan-500 [&::-webkit-slider-thumb]:w-2.5 [&::-webkit-slider-thumb]:h-2.5 [&::-webkit-slider-thumb]:bg-cyan-400 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:appearance-none"/>
                        </div>
                      </div>)}
                  </div>);
            })}
        ...
        <div className="flex-shrink-0 px-4 py-3 border-t border-white/[0.06]">
          <button onClick={onClose} className="w-full px-3 py-2 rounded-lg bg-white/[0.04] hover:bg-white/[0.06] text-white/40 text-xs transition-colors">
            Done
          </button>
        </div>
      </div>
