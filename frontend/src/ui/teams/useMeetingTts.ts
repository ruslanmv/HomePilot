...
export function useMeetingTts(
  ...
  // ── Tracking refs ──
  const processedRef = useRef<Set<string>>(new Set(messages.map((m) => m.id)))
  const initialisedRef = useRef(false)
  const queueRef = useRef<MeetingMessage[]>([])
    ...
    while (queueRef.current.length > 0) {
      ...
      if (queueRef.current.length > MAX_QUEUE) {
        ...
        setRevealedIds((prev) => {
          ...
          for (const m of overflow) next.add(m.id)
          return next
        })
      }
  ...
  const stop = useCallback(() => {
    ...
    if (queueRef.current.length > 0) {
      setRevealedIds((prev) => {
        const next = new Set(prev)
        for (const m of queueRef.current) next.add(m.id)
        return next
      })
      queueRef.current = []
    }
