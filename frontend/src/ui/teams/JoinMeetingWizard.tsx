...
export interface JoinMeetingWizardProps {
...
export function JoinMeetingWizard({ personas, backendUrl, onCancel, onJoin }: JoinMeetingWizardProps) {
  ...
  const canNext = (): boolean => {
    if (step === 1) return urlValid && meetingName.trim().length > 0
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (!healthRes || !healthRes.ok) {
        throw new Error(
          `Teams MCP server is not reachable at ${mcpBaseUrl}. ` +
      ...
      if (!healthData.available) {
        throw new Error(
          `Teams MCP server at ${mcpBaseUrl} is not responding. ` +
      ...
      <div className="relative w-full max-w-2xl mx-4 bg-[#1a1a2e] border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-blue-500/30 transition-all"
                />
              </div>
                  ...
                  return (
                    ...
                    >
                      {avatarUrl ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500/30 to-purple-500/30 flex items-center justify-center text-sm font-semibold text-white/70">
                          {p.name.charAt(0)}
                        </div>
                      )}
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white text-sm placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-blue-500/30"
                />
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-white/10 bg-white/[0.02]">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 text-sm text-white/60 hover:text-white/80 transition-colors"
          >
            <ChevronLeft size={16} />
            {step > 1 ? 'Back' : 'Cancel'}
          </button>
          ...
          {step < 3 ? (
            <button
              ...
              disabled={!canNext()}
              className="flex items-center gap-1.5 px-5 py-2.5 text-sm font-medium rounded-xl transition-all disabled:opacity-30 disabled:cursor-not-allowed bg-gradient-to-r from-blue-500 to-purple-500 text-white hover:brightness-110"
            >
              Next
            ...
            <button
              ...
              disabled={submitting}
              className="flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-xl transition-all disabled:opacity-50 bg-gradient-to-r from-green-500 to-emerald-500 text-white hover:brightness-110"
            >
              {submitting ? (
                <>
