// frontend/src/ui/teams/teamBundles.ts
import type { PersonaBundle } from './PersonaSelectorEnterprise'
import type { PersonaSummary } from './types'

const has = (p: PersonaSummary, ...keys: string[]) => {
  const txt = `${p.name ?? ''} ${p.description ?? ''} ${p.persona_agent?.persona_class ?? ''}`.toLowerCase()
  return keys.some((k) => txt.includes(k))
}

export const TEAM_BUNDLES: PersonaBundle[] = [
  {
    id: 'brainstorm',
