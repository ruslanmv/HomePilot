...
export function usePersonaVoices() {
  const [map, setMap] = useState<Record<string, PersonaVoiceConfig>>(() =>
  ...
  const setPersonaVoice = useCallback((personaId: string, cfg: PersonaVoiceConfig) => {
    setMap((m) => ({ ...m, [personaId]: cfg }))
  }, [])

  const clearPersonaVoice = useCallback((personaId: string) => {
    setMap((m) => {
      const copy = { ...m }
  ...
  const getPersonaVoice = useCallback(
    (personaId: string): PersonaVoiceConfig | undefined => map[personaId],
    [map],
  )
