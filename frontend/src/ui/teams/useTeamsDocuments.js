 ...
 */
export function useTeamsDocuments(backendUrl, apiKey) {
    const headers = {};
    if (apiKey)
    ...
    async function upload(roomId, file, uploadedBy = 'You') {
    ...
    async function addUrl(roomId, url, title = '') {
    ...
    async function preview(roomId, docId) {
        const res = await fetch(`${backendUrl}/v1/teams/rooms/${roomId}/documents/${docId}/preview`, { headers });
        if (!res.ok)
            throw new Error(`Failed to preview: ${res.status}`);
        const data = await res.json();
        return data.preview;
    }
    async function download(roomId, doc) {
        const res = await fetch(`${backendUrl}/v1/teams/rooms/${roomId}/documents/${doc.id}/download`, { headers });
        if (!res.ok)
    ...
    async function deleteDoc(roomId, docId) {
