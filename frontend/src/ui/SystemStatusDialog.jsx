...
import { fetchSystemOverview } from './systemApi';
...
function statusTone(ok) {
    if (ok)
        return 'text-emerald-300/90 border-emerald-500/15 bg-emerald-500/8';
...
/* ── main component ────────────────────────────────────── */
export default function SystemStatusDialog({ backendUrl, apiKey, onClose, }) {
    ...
    const donut = useMemo(() => {
        if (!data)
            return 'conic-gradient(#374151 0% 100%)';
    ...
    const handleBackdrop = (e) => { if (e.target === e.currentTarget)
        onClose(); };
    return (<div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={handleBackdrop} style={{ animation: 'statusFadeIn 200ms ease-out' }}>
      <div className="relative w-[960px] max-w-[96vw] max-h-[92vh] overflow-hidden rounded-3xl border border-white/[0.07] bg-[#0c0c18] shadow-2xl" style={{ animation: 'statusSlideUp 250ms ease-out' }}>
        {/* Close — smaller, subtler, brighter on hover */}
        <button type="button" onClick={onClose} className="absolute top-4 right-4 z-10 h-7 w-7 rounded-full grid place-items-center text-white/25 hover:text-white/80 hover:bg-white/8 transition-colors">
          <X size={14}/>
        </button>
        ...
        <div className="px-8 pt-7 pb-8 overflow-y-auto max-h-[calc(92vh-4px)]">
          ...
          <div className="flex items-start justify-between gap-6 mb-7">
            ...
            {data && (<div className="px-3 py-1 rounded-full text-xs font-semibold border text-emerald-300/80 border-emerald-500/15 bg-emerald-500/8">
                v{data.overview.version}
              </div>)}
          </div>

          {loading ? (<div className="text-white/50 text-sm py-12 text-center">Loading system status...</div>) : error ? (<div className="text-red-300/80 text-sm py-12 text-center">{error}</div>) : data ? (<>
              ...
              <div className="grid grid-cols-[280px_1fr] gap-6 mb-7">
                ...
                <div className="rounded-3xl border border-white/[0.07] bg-white/[0.02] px-6 pt-5 pb-6 flex flex-col items-center">
                  ...
                  <div className="relative h-40 w-40 mb-1">
                    ...
                    <div className="absolute inset-[-4px] rounded-full blur-md opacity-30 donut-ring" style={{ background: `conic-gradient(rgba(52,211,153,0.5) 0% ${healthyPct}%, transparent ${healthyPct}% 100%)` }}/>
                    {/* Donut track */}
                ...
                <div className="rounded-3xl border border-white/[0.07] bg-white/[0.02] p-6">
                  ...
                  <div className="grid grid-cols-4 gap-5 text-sm">
                    <FlowCard title="Inputs" icon={<PlugZap size={14}/>} items={[`${data.architecture.inputs.virtual_servers_active}/${data.architecture.inputs.virtual_servers_total} virtual servers`]}/>
                    <FlowCard title="Gateway" icon={<Activity size={14}/>} items={[`ContextForge ${data.architecture.gateway.contextforge_ok ? 'online' : 'offline'}`]}/>
                    <FlowCard title="Infrastructure" icon={<Database size={14}/>} items={[data.architecture.infrastructure.database, data.architecture.infrastructure.memory_mode]}/>
                    <FlowCard title="Outputs" icon={<Bot size={14}/>} items={[
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="rounded-3xl border border-white/[0.07] bg-white/[0.02] p-6">
                  ...
                  <div className="space-y-2.5">
                    {Object.entries(data.services).map(([key, svc]) => (<div key={key} className="status-card flex items-center justify-between rounded-2xl border border-white/[0.04] bg-black/20 px-4 py-3">
...
function MetricCard({ label, value, delay = 0 }) {
    return (<div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] px-4 py-4" style={{ animation: `statusCardIn 350ms ease-out ${delay}ms both` }}>
      <div className="text-[10px] uppercase tracking-wider text-white/25 mb-2">{label}</div>
      <div className="text-2xl font-bold text-white">{value}</div>
    </div>);
}
function SmallCount({ label, value, sub }) {
    return (<div className="rounded-2xl border border-white/[0.04] bg-black/20 px-4 py-4">
      <div className="text-[10px] uppercase tracking-wider text-white/25 mb-2">{label}</div>
      <div className="text-2xl font-bold text-white">{value}</div>
...
function Legend({ color, label }) {
    return (<div className="flex items-center gap-1.5 text-white/35">
      <div className={`h-2 w-2 rounded-full ${color}`}/>
      <span>{label}</span>
    </div>);
}
function FlowCard({ title, icon, items }) {
    return (<div className="rounded-2xl border border-white/[0.04] bg-black/20 p-4">
      ...
      <div className="space-y-2">
        {items.map((item) => (<div key={item} className="text-[11px] text-white/35">{item}</div>))}
      </div>
    </div>);
}
