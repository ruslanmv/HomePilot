...
export function AvatarStageQuickTools({ previews, loadingAngles, activeAngle, busy = false, orbitMode = false, onToggleOrbit, onGenerateAngle, onOpenAngle, onGenerateMissing, }) {
    ...
    return (<div className="flex items-center gap-1 rounded-lg border border-white/[0.08] bg-black/50 backdrop-blur-md px-2 py-1.5 shadow-lg">
      {VIEW_ANGLE_OPTIONS.map((angle) => {
            ...
            return (<button key={angle.id} onClick={() => (available ? onOpenAngle(angle.id) : onGenerateAngle(angle.id))} disabled={loading} className={[
                ...
                ].join(' ')} title={available ? `View ${angle.label}` : `Generate ${angle.label}`}>
      ...
      {/* 360° orbit toggle */}
      {canOrbit && onToggleOrbit && (<button onClick={onToggleOrbit} className={[
            ...
            ].join(' ')} title={orbitMode ? 'Exit 360° mode' : 'Enter 360° mode'}>
          <RotateCw size={11}/>
        </button>)}

      {missingCount > 0 && (<button onClick={onGenerateMissing} disabled={busy} className={[
