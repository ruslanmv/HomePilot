...
import { buildAnglePrompt, getViewAngleOption, VIEW_ANGLE_OPTIONS } from './viewPack'
...
export function useViewPackGeneration(backendUrl: string, apiKey?: string, cacheKey?: string) {
  ...
  const generateAngle = useCallback(async (params: GenerateViewParams) => {
    ...
    // For back / rear-quarter angles with NSFW outfits, rear-emphasis tokens are
    // injected by buildAnglePrompt so the model shows the garment from behind.
    const outfitPrompt = buildAnglePrompt(params.angle, outfitDesc)
