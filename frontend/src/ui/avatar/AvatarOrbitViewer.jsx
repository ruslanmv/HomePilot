...
export function AvatarOrbitViewer({ previews, activeAngle, onAngleChange, onOpenLightbox, }) {
    ...
    if (!currentUrl) {
        ...
        if (!fallbackUrl) {
            return (<div className="relative h-full rounded-2xl border border-white/[0.08] bg-black/40 flex items-center justify-center">
    ...
    return (<div className="relative group h-full">
      ...
      <div ref={containerRef} className="relative h-full rounded-2xl overflow-hidden border border-cyan-500/20 bg-black/40 flex items-center justify-center select-none" style={{ touchAction: 'pan-y' }} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp}>
        {/* Previous image — fades out during crossfade */}
        {fading && prevUrl && prevUrl !== currentUrl && (<img src={prevUrl} alt={prevAngle} className="absolute inset-0 w-full h-full object-contain pointer-events-none" style={{
                opacity: 0,
                transition: `opacity ${CROSSFADE_MS}ms ease-out`,
            }}/>)}

        {/* Current image — fades in */}
        {currentUrl && (<img src={currentUrl} alt={currentAngle} className="max-w-full max-h-full object-contain pointer-events-none" style={{
                opacity: fading ? 0 : 1,
        ...
        {hasEnoughAngles && (<div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-black/50 backdrop-blur-sm border border-white/10 text-[10px] text-white/40 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
            <span>← → drag to rotate</span>
          </div>)}

        {/* Lightbox overlay on hover */}
        {currentUrl && (<div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-pointer" onClick={handleClick}>
        ...
        <div className="absolute bottom-3 right-3 flex items-center gap-1.5">
          {ORBIT_ORDER.map((angle, idx) => {
            ...
            return (<button key={angle} onClick={(e) => {
                    e.stopPropagation();
                    if (!hasImage)
                        return;
                    transitionTo(idx);
                ...
                ].join(' ')} title={`${ANGLE_LABELS[angle] || angle}${hasImage ? '' : ' (not generated)'}`}/>);
        })}
        </div>
      </div>
