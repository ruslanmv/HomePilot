...
function getFramingTokens(framingType) {
 ...
 */
export function extractVisualDescriptors(prompt) {
    const found = new Map();
    for (const { re, category } of VISUAL_DESCRIPTOR_PATTERNS) {
        if (found.has(category))
            continue; // first match wins per category
        const m = prompt.match(re);
        if (m)
            found.set(category, m[0].trim());
    }
 ...
 */
export function buildVisualDescriptorsFromMeta(meta) {
    if (!meta)
        return '';
    const parts = [];
    if (meta.hairColor)
        parts.push(`${meta.hairColor} hair`);
    if (meta.hairType)
        parts.push(`${meta.hairType} hair texture`);
    if (meta.skinTone)
        parts.push(`${meta.skinTone} skin tone`);
    if (meta.eyeColor)
        parts.push(`${meta.eyeColor} eyes`);
    return parts.join(', ');
}
/**
 ...
 */
export function stripDescriptorDuplicates(basePrompt, descriptors) {
    if (!descriptors)
        return basePrompt;
    ...
    const kept = segments.filter((seg) => {
        ...
        for (const dt of descriptorTokens) {
            if (lower.includes(dt) || dt.includes(lower))
                return false;
        }
...
// ---------------------------------------------------------------------------
function buildLeftPrompt(ft, w, autoMirror) {
    if (autoMirror) {
...
function buildRightPrompt(ft, w) {
    return `(right profile view:${w.toFixed(1)}), (full body side view:1.3), (body turned right:1.3), shoulders angled right, torso rotated right, hips turned right, facing right, ${ft.bodyRange}, solo, consistent lighting`;
}
function buildBackPrompt(ft, w) {
...
export function buildIdentityLockSuffix(_framingType) {
...
export function buildAnglePrompt(angle, framingType, settings) {
...
export const ANGLE_TUNING_DEFAULTS = {
...
export function resolveAngleTuning(angle, settings) {
...
export function getAvailableViewAngles(viewPack) {
    if (!viewPack)
        return [];
    return ['front', 'left', 'right', 'back'].filter((a) => !!viewPack[a]);
}
export function getViewAngleOption(angle) {
...
const FRONT_POSE_DESC_PATTERNS = [
...
const IDENTITY_BOILERPLATE_PATTERNS = [
...
const PROFESSIONAL_IDENTITY_PATTERNS = [
...
const BACKEND_QUALITY_PATTERNS = [
 ...
 */
function cleanCommaArtifacts(text) {
 ...
 */
export function sanitiseBasePromptForAngle(basePrompt, angle) {
    if (angle === 'front')
        return cleanCommaArtifacts(basePrompt);
...
const LINGERIE_OUTFIT_KEYWORDS = [
...
function isLingerieOutfit(outfitPrompt) {
    return LINGERIE_OUTFIT_KEYWORDS.some((re) => re.test(outfitPrompt));
}
const GARMENT_EMPHASIS_RULES = [
 ...
 */
export function getGarmentEmphasis(angle, outfitPrompt) {
    if (angle === 'front')
        return '';
    if (!isLingerieOutfit(outfitPrompt))
        return '';
 ...
 */
export function compressOutfitForAngle(outfitPrompt, angle) {
    if (angle === 'front')
        return outfitPrompt;
    if (!isLingerieOutfit(outfitPrompt))
        return outfitPrompt;
...
export function extractViewAngle(metadata) {
    const value = metadata?.view_angle;
    if (typeof value !== 'string')
        return null;
    return VIEW_ANGLE_OPTIONS.some((item) => item.id === value) ? value : null;
}
