...
describe('fetchAvatarPacks', () => {
    ...
    it('calls the correct endpoint', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/v1/avatars/packs', { headers: {} });
    });
    it('strips trailing slash from backendUrl', async () => {
        ...
        await fetchAvatarPacks('http://localhost:8000/');
        expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/v1/avatars/packs', expect.anything());
    });
    it('sends API key header when provided', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith(expect.any(String), { headers: { 'x-api-key': 'secret-key' } });
    });
    ...
    it('throws on HTTP error', async () => {
        ...
        await expect(fetchAvatarPacks('http://localhost:8000')).rejects.toThrow('Internal Server Error');
    });
});
describe('generateAvatars', () => {
    ...
    it('sends POST with correct body', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/v1/avatars/generate', expect.objectContaining({
            method: 'POST',
    ...
    it('sends API key when provided', async () => {
        ...
        await generateAvatars('http://localhost:8000', { mode: 'studio_random' }, 'my-api-key');
    ...
    it('throws on HTTP error', async () => {
        ...
        await expect(generateAvatars('http://localhost:8000', { mode: 'studio_random' })).rejects.toThrow('Service Unavailable');
    });
    it('includes reference_image_url for reference mode', async () => {
        ...
        expect(body.reference_image_url).toBe('http://localhost:8000/uploads/face.png');
    });
});
