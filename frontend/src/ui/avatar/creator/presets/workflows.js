...
export function presetsForStep(step, includeAdvanced = false) {
    return Object.values(WORKFLOW_PRESETS).filter((p) => p.step === step && (includeAdvanced || !p.advanced));
}
...
export function pickWizardWorkflow(opts) {
    ...
    if (step === 'FACE') {
        if (hasStyleGAN)
            return 'wf01_face_stylegan';
        return 'wf02_face_diffusion';
    }
    if (step === 'BODY') {
        if (wantsPose && capabilities?.openpose)
            return 'wf05_body_pose';
        if (wantsSdxl && capabilities?.sdxl)
            return 'wf06_body_sdxl';
        return 'wf04_body_anchor';
    }
    if (step === 'OUTFIT') {
        if (wantsSdxl && capabilities?.sdxl)
            return 'wf09_outfit_sdxl';
        if (wantsPhotoMaker && capabilities?.photomaker)
            return 'wf08_outfit_photomaker';
        return 'wf07_outfit_default';
    }
    if (step === 'PORTRAIT')
        return 'wf16_portrait';
    if (step === 'REPAIR_FACE')
        return 'wf11_identity_reproject';
    if (step === 'EDIT_OUTFIT')
        return 'wf12_inpaint_outfit';
    if (step === 'EDIT_BG')
        return 'wf13_bg_replace';
    if (step === 'EDIT_EXPRESSION')
        return 'wf14_expression_change';
    if (step === 'EDIT_POSE')
        return 'wf15_pose_adjust';
    return 'wf07_outfit_default';
}
