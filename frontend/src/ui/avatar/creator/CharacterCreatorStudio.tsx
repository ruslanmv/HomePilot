...
import type { GalleryItem, WizardMeta, CharacterGender, OutfitScenarioTag, FramingType } from '../galleryTypes'
import {
...
export interface CharacterCreatorStudioProps {
  backendUrl: string
  apiKey?: string
  globalModelImages?: string
  enabledModes?: string[]
  /** Shared gallery instance from the parent so items persist across view changes. */
  gallery: ReturnType<typeof useAvatarGallery>
  onClose: () => void
  ...
  /** Navigate to the Character Sheet (AvatarViewer) for a specific avatar project. */
...
type CreatorPhase = 'identity' | 'body' | 'outfit' | 'finalize'
...
export function CharacterCreatorStudio({
  backendUrl,
  apiKey,
  globalModelImages,
  enabledModes = [],
  ...
  const faceGenMode: AvatarMode = avatarSettings.useStyleGAN
  ...
  const [savedIdentity, setSavedIdentity] = useState<GalleryItem | null>(null)

  // Track identities created during this wizard session (for scoped display)
  const [sessionIdentityIds, setSessionIdentityIds] = useState<Set<string>>(new Set())
  // Track imported identity IDs (brought from library, not created in session)
  const [importedIdentityIds, setImportedIdentityIds] = useState<Set<string>>(new Set())
  ...
  const [bodyFraming, setBodyFraming] = useState<FramingType>('half_body')
  const [bodyType, setBodyType] = useState<string>('average')
  const [bodyPosture, setBodyPosture] = useState<string>('standing')
  const [bodyCount, setBodyCount] = useState(1)
  const [selectedBodyIndex, setSelectedBodyIndex] = useState<number | null>(null)
  const [savedBody, setSavedBody] = useState<GalleryItem | null>(null)
  ...
  // ── Profession ──
  const [selectedProfession, setSelectedProfession] = useState<string | null>('executive_secretary')

  // ── Phase 2: Outfit ──
  const [selectedOutfitPreset, setSelectedOutfitPreset] = useState<string | null>(null)
  ...
  const [allOutfits, setAllOutfits] = useState<AvatarResult[]>([])
  ...
  const [nudityLevel, setNudityLevel] = useState<NudityLevel | null>(null)
  const [sensualPose, setSensualPose] = useState<SensualPose | null>(null)
  const [powerDynamic, setPowerDynamic] = useState<PowerDynamic | null>(null)
  const [fantasyTone, setFantasyTone] = useState<FantasyTone | null>(null)
  ...
  const pendingIdentitySeedRef = useRef<{ seed: number | undefined; url: string } | null>(null)
  ...
  const outfitPresets = OUTFIT_PRESETS.filter(
    (p) => {
  ...
  const identityPrompt = useMemo(() => {
    ...
    if (prof && prof.id !== 'custom') {
      base = `${base}, ${prof.label.toLowerCase()} professional`
    }
    if (showGenetics) {
      const frag = buildGeneticsPromptFragment(geneticsPrefs)
      base = `${base}, ${frag}`
    }
    // Only append framing keywords for half-body (needs specific composition guidance).
    // Headshot relies on the high-quality preset prompt + portrait dimensions.
    if (bodyFraming === 'half_body') {
  ...
  const bodyPrompt = useMemo(() => {
    ...
    return `Half body portrait photograph of a single real ${gender}, ${bodyType} build, ${pose}, ` +
      'neutral plain clothing, clean seamless background, professional studio lighting, visible from head to waist, upper body focus, ' +
  ...
  const effectiveOutfitPrompt = useMemo(() => {
    ...
    if (nudityLevel) {
      ...
      if (nl) parts.push(nl.prompt)
    }
    if (sensualPose) {
      const sp = SENSUAL_POSES.find((p) => p.id === sensualPose)
      if (sp) parts.push(sp.prompt)
    }
    if (powerDynamic) {
      const pd = POWER_DYNAMICS.find((d) => d.id === powerDynamic)
      if (pd) parts.push(pd.prompt)
    }
    if (fantasyTone) {
      const ft = FANTASY_TONES.find((t) => t.id === fantasyTone)
      if (ft) parts.push(ft.prompt)
    }
    if (sceneSetting) {
      const ss = SCENE_SETTINGS.find((s) => s.id === sceneSetting)
      if (ss) parts.push(ss.prompt)
    }
    if (selectedAccessories.length > 0) {
      const accPrompts = selectedAccessories
        .map((id) => ACCESSORY_OPTIONS.find((a) => a.id === id)?.prompt)
        .filter(Boolean)
      if (accPrompts.length) parts.push(accPrompts.join(', '))
    }
    // Add color context
    if (outfitPrimaryColor) parts.push(`${outfitPrimaryColor} primary color`)
    if (outfitSecondaryColor) parts.push(`${outfitSecondaryColor} accent color`)
    // Add profession context to outfit
    const prof = selectedProfession ? PROFESSIONS.find((p) => p.id === selectedProfession) : null
    if (prof && prof.id !== 'custom') parts.push(`${prof.label.toLowerCase()} role`)
    return parts.join(', ')
  ...
  const handleGenerateFaces = useCallback(async () => {
    ...
    try {
      ...
      if (result?.results?.length) {
        ...
        if (isPlaceholder) {
          showToast(
            'No AI generator available — showing placeholder faces. Start ComfyUI for real AI faces.',
            'info',
          )
  ...
  const handleSaveIdentity = useCallback(() => {
    if (!gen.result?.results?.length || selectedFaceIndex === null) return
    ...
    gallery.addAnchorWithPortraits(
      chosen, portraits, faceGenMode,
      identityPrompt || undefined,
      undefined,
      { vibeTag: selectedStyle || undefined, nsfw: isSpicy || undefined },
    )
  ...
  }, [gen, selectedFaceIndex, gallery, identityPrompt, selectedStyle, showToast, needsBodyStep])
  ...
  useEffect(() => {
    ...
    if (!pending) return
    const found = gallery.items.find(
      (i) => i.role === 'anchor' && !i.parentId && i.url === pending.url,
    )
    if (found) {
      ...
      if (result?.results?.length) {
        ...
        showToast(
          result.results.length === 1
            ...
            : `${result.results.length} body variations generated — pick the best one`,
          'success',
        )
      }
    } catch (err: any) {
  ...
  const handleSaveBody = useCallback(() => {
    if (!bodyGen.result?.results?.length || selectedBodyIndex === null || !savedIdentity) return
    const chosen = bodyGen.result.results[selectedBodyIndex]

    // Save body anchor to gallery linked to face identity
    gallery.addBatch(
      [{ url: chosen.url, seed: chosen.seed, metadata: chosen.metadata }],
      'studio_reference' as AvatarMode,
      bodyPrompt,
      savedIdentity.url,
      undefined,
    ...
    const bodyItem: GalleryItem = {
      id: crypto.randomUUID ? crypto.randomUUID() : `body-${Date.now()}`,
      url: chosen.url,
      seed: chosen.seed,
      prompt: bodyPrompt,
      mode: 'studio_reference' as AvatarMode,
      createdAt: Date.now(),
      role: 'anchor',
      parentId: savedIdentity.id,
    }
  ...
  const handleGenerateOutfits = useCallback(async () => {
    if (!savedIdentity || !effectiveOutfitPrompt) return
    ...
    try {
      ...
      if (result?.results?.length) {
        ...
        const scenarioTag: OutfitScenarioTag = (selectedOutfitPreset as OutfitScenarioTag) || 'custom'
        // Save to gallery linked to identity
        gallery.addBatch(
          result.results,
          'studio_reference' as AvatarMode,
          effectiveOutfitPrompt,
          savedIdentity.url,
          scenarioTag,
          { parentId: savedIdentity.id },
        )
  ...
  const handleSelectIdentity = useCallback((item: GalleryItem) => {
  ...
  const handleDeleteFace = useCallback((index: number) => {
    gen.removeResult(index)
    setSelectedFaceIndex((prev) => {
      if (prev === null) return null
      if (prev === index) return gen.result?.results && gen.result.results.length > 1 ? Math.max(0, index - 1) : null
      if (prev > index) return prev - 1
      return prev
    })
  }, [gen])

  const handleDeleteBody = useCallback((index: number) => {
    bodyGen.removeResult(index)
    setSelectedBodyIndex((prev) => {
      if (prev === null) return null
      if (prev === index) return bodyGen.result?.results && bodyGen.result.results.length > 1 ? Math.max(0, index - 1) : null
      if (prev > index) return prev - 1
      return prev
    })
  }, [bodyGen])

  const handleDeleteOutfit = useCallback((index: number) => {
    // Remove from accumulated list
    setAllOutfits((prev) => prev.filter((_, i) => i !== index))
    setSelectedOutfitIndex((prev) => {
      if (prev === null) return null
      if (prev === index) return allOutfits.length > 1 ? Math.max(0, index - 1) : null
      if (prev > index) return prev - 1
      return prev
    })
  }, [allOutfits])
  ...
  const handleDeleteOutfitFromGallery = useCallback((item: GalleryItem) => {
    gallery.removeItem(item.id)
  }, [gallery])

  // ── Select outfit from library → show on stage ──
  const handleSelectOutfitFromLibrary = useCallback((item: GalleryItem) => {
  ...
  // ── Resolve URL helper ──
  const resolveUrlFn = useCallback(
    (url: string) => resolveUrl(url, backendUrl),
    [backendUrl],
  )
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') {
        e.preventDefault()
        if (phase === 'identity' && !gen.loading) handleGenerateFaces()
        if (phase === 'body' && !bodyGen.loading) handleGenerateBody()
        if (phase === 'outfit' && !outfitGen.loading && effectiveOutfitPrompt) handleGenerateOutfits()
      }
    }
  ...
  const stageContent: StageContent = (() => {
    ...
    const outfitStage = (): StageContent | null => {
    ...
    if (phase === 'finalize') {
      ...
      if (baseAnchor) return { kind: 'face', url: resolveUrlFn(baseAnchor.url), seed: baseAnchor.seed }
      return { kind: 'empty' }
    }

    if (phase === 'outfit') {
      if (outfitGen.loading) return { kind: 'generating' }
      const outfit = outfitStage()
      if (outfit) return outfit
    ...
    // Phase 1.5: body
    if (phase === 'body') {
      if (bodyGen.loading) return { kind: 'generating' }
    ...
    // Phase 1: identity
    if (gen.loading) return { kind: 'generating' }
    ...
    <div className="flex flex-col h-full bg-[#0a0a0f] text-white overflow-hidden">
                ...
                <React.Fragment key={s.key}>
                  {i > 0 && (
                    <div className={`w-5 h-px mx-0.5 ${
                      s.done || phase === s.key ? 'bg-purple-500/40' : 'bg-white/10'
                    }`} />
                  )}
                  <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium transition-all ${
                    ...
                    {s.done ? (
                      <CheckCircle2 size={10} className="text-green-400/70" />
                    ) : (
                      <span className={`w-3.5 h-3.5 rounded-full text-[8px] flex items-center justify-center font-bold ${
                        phase === s.key ? 'bg-purple-500/30 text-purple-200' : 'bg-white/10 text-white/30'
                      }`}>{s.num}</span>
                    )}
                    {s.label}
                  </div>
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 md:grid-cols-[180px_1fr_320px] overflow-hidden">
        ...
        <div className="hidden md:flex flex-col border-r border-white/[0.06] py-4 px-3 overflow-y-auto gap-4">
          ...
          <IdentityLibrary
            items={gallery.items}
            sessionIds={sessionIdentityIds}
            importedIds={importedIdentityIds}
            activeIdentityId={savedIdentity?.id ?? null}
            onSelectIdentity={handleSelectIdentity}
            onNewIdentity={handleBackToIdentity}
            onImportIdentity={handleSelectIdentity}
          ...
          <OutfitLibrary
            items={gallery.items}
            activeIdentityId={savedIdentity?.id ?? null}
            activeOutfitUrl={selectedOutfitIndex !== null && selectedOutfitIndex < allOutfits.length ? allOutfits[selectedOutfitIndex].url : null}
            onSelectOutfit={handleSelectOutfitFromLibrary}
            onDeleteOutfit={handleDeleteOutfitFromGallery}
            resolveUrl={resolveUrlFn}
          />
          ...
          <div className="space-y-1">
            ...
            >
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold flex-shrink-0 ${
                phase === 'identity' ? 'bg-purple-500/25 text-purple-300'
            ...
            {needsBodyStep && (
              <button
                onClick={() => savedIdentity && setPhase('body')}
                disabled={!savedIdentity}
                className={[
                  'w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs font-medium transition-all',
                  phase === 'body'
              ...
              >
                <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold flex-shrink-0 ${
                  phase === 'body' ? 'bg-pink-500/25 text-pink-300'
            ...
            <button
              onClick={() => {
                if (needsBodyStep ? savedBody : savedIdentity) setPhase('outfit')
              }}
              disabled={needsBodyStep ? !savedBody : !savedIdentity}
              className={[
                'w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs font-medium transition-all',
                phase === 'outfit'
            ...
            >
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold flex-shrink-0 ${
                phase === 'outfit' ? 'bg-cyan-500/25 text-cyan-300' : 'bg-white/[0.05] text-white/20'
              }`}>
                {needsBodyStep ? '3' : '2'}
              </span>
              Style & Outfit
            ...
            <button
              onClick={() => {
                if (savedBody || savedIdentity) setPhase('finalize')
              }}
              disabled={!(savedBody || savedIdentity)}
              className={[
                'w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs font-medium transition-all',
                phase === 'finalize'
            ...
            >
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold flex-shrink-0 ${
                phase === 'finalize' ? 'bg-green-500/25 text-green-300' : 'bg-white/[0.05] text-white/20'
              }`}>
                {needsBodyStep ? '4' : '3'}
              </span>
              Finalize
          ...
          {phase === 'identity' && (
            <button
              onClick={() => {
                setSelectedGender(['female', 'male', 'neutral'][Math.floor(Math.random() * 3)] as CharacterGender)
                const styles = charStyles
                if (styles.length) setSelectedStyle(styles[Math.floor(Math.random() * styles.length)].id)
              }}
        ...
        <div className="flex flex-col min-h-0 px-4 py-4 gap-3 overflow-hidden">
          ...
          <div className="flex-1 min-h-0">
            <CharacterStage
              ...
              faceLocked={phase === 'outfit' && !!(savedBody || savedIdentity)}
              lockedFaceUrl={(savedBody || savedIdentity) ? resolveUrlFn((savedBody || savedIdentity)!.url) : undefined}
              onOpenLightbox={onOpenLightbox}
              onDelete={
                phase === 'identity' && gen.result?.results?.length && selectedFaceIndex !== null
          ...
          {phase === 'identity' && gen.result?.results && gen.result.results.length > 1 && (
            <FaceFilmstrip
              results={gen.result.results}
              selectedIndex={selectedFaceIndex ?? 0}
          ...
          {phase === 'body' && bodyGen.result?.results && bodyGen.result.results.length > 1 && (
            <FaceFilmstrip
              results={bodyGen.result.results}
              selectedIndex={selectedBodyIndex ?? 0}
              onSelect={setSelectedBodyIndex}
          ...
          {(phase === 'outfit' || phase === 'finalize') && allOutfits.length > 0 && (
            <FaceFilmstrip
              results={allOutfits}
              selectedIndex={selectedOutfitIndex ?? 0}
              onSelect={setSelectedOutfitIndex}
              onDelete={phase === 'outfit' ? handleDeleteOutfit : undefined}
              resolveUrl={resolveUrlFn}
              accent="cyan"
            />
          )}
            ...
            {phase === 'identity' ? (
              ...
              <>
                      ...
                      <div>
                        ...
                        <div className="flex gap-1">
                          {FACE_BASE_OPTIONS.map((o) => (
                            <OptionPill key={o.key}
                              label={o.label}
                              active={geneticsPrefs.faceBase === o.key}
                              onClick={() => setGeneticsPrefs((p) => ({ ...p, faceBase: o.key as FaceBase }))}
                            />
                          ))}
                        </div>
                      </div>
                ...
                <div className="border-t border-white/[0.06] pt-4">
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    <MiniLabel className="mb-0">Faces:</MiniLabel>
                    <div className="flex gap-1">
                      {[1, 4, 8].map((n) => (
                        <button key={n} onClick={() => setFaceCount(n)}
                          className={[
                            'w-9 h-9 rounded-lg text-xs font-medium border flex items-center justify-center transition-all',
                            faceCount === n
                              ? 'border-purple-500/30 bg-purple-500/15 text-purple-300 shadow-[0_0_8px_rgba(168,85,247,0.15)]'
              ...
              <>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {POSTURE_OPTIONS.map((p) => (
                      <OptionPill key={p.id}
                        label={p.label}
                        active={bodyPosture === p.id}
                        onClick={() => setBodyPosture(p.id)}
                        accent="purple"
                      />
                    ))}
                  </div>
                ...
                <div className="border-t border-white/[0.06] pt-4">
                  <SectionLabel>Generation</SectionLabel>
                  <div className="flex items-center gap-3 mb-3">
                    <MiniLabel className="mb-0">Bodies:</MiniLabel>
                    <div className="flex gap-1">
                      {[1, 2, 4].map((n) => (
                        <button key={n} onClick={() => setBodyCount(n)}
                          className={[
                            'w-9 h-9 rounded-lg text-xs font-medium border flex items-center justify-center transition-all',
                            bodyCount === n
                              ? 'border-pink-500/30 bg-pink-500/15 text-pink-300 shadow-[0_0_8px_rgba(236,72,153,0.15)]'
                              : 'border-white/[0.06] bg-white/[0.03] text-white/40 hover:bg-white/[0.06] hover:text-white/60',
                          ].join(' ')}
                        >
                          {n}
              ...
              <>
                ...
                {(savedBody || savedIdentity) && (() => {
                  ...
                  return (
                    <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                      ...
                      <button
                        onClick={isBody ? handleBackToBody : handleBackToIdentity}
                        className="text-[10px] text-white/25 hover:text-white/50 transition-colors"
                        title={isBody ? 'Change body' : 'Change identity'}
                      >
                        Change
                      </button>
                    </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1 p-1 rounded-xl bg-white/[0.04] border border-white/[0.08] mb-3">
                    <button onClick={() => { setOutfitTab('standard'); setSelectedOutfitPreset(null) }}
                      className={[
                    ...
                    {nsfwMode && (
                      <>
                        <button onClick={() => { setOutfitTab('romance'); setSelectedOutfitPreset(null) }}
                          className={[
                            'flex-1 flex items-center justify-center gap-1 px-2 py-2 rounded-lg text-[10px] font-medium transition-all',
                            outfitTab === 'romance'
                        ...
                        <button onClick={() => { setOutfitTab('18+'); setSelectedOutfitPreset(null) }}
                          className={[
                            'flex-1 flex items-center justify-center gap-1 px-2 py-2 rounded-lg text-[10px] font-medium transition-all',
                            outfitTab === '18+'
                      ...
                      return (
                        <button key={p.id}
                          onClick={() => {
                            ...
                            if (!active) setCustomOutfitPrompt('')
                          }}
                          className={[
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {['black', 'white', 'navy', 'red', 'burgundy', 'grey', 'blush', 'emerald'].map((c) => (
                        <button key={c} onClick={() => setOutfitPrimaryColor(outfitPrimaryColor === c ? null : c)}
                          className={[
                            'w-6 h-6 rounded-full border-2 transition-all',
                            outfitPrimaryColor === c ? 'border-cyan-400 scale-110 ring-2 ring-cyan-400/30' : 'border-white/10 hover:border-white/30',
                          ].join(' ')}
                          style={{ backgroundColor: { black: '#1a1a1a', white: '#f0f0f0', navy: '#1e3a5f', red: '#dc2626', burgundy: '#7f1d1d', grey: '#6b7280', blush: '#fbcfe8', emerald: '#059669' }[c] }}
                          title={c}
                        />
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {['black', 'white', 'navy', 'red', 'burgundy', 'grey', 'blush', 'emerald'].map((c) => (
                        <button key={c} onClick={() => setOutfitSecondaryColor(outfitSecondaryColor === c ? null : c)}
                          className={[
                            'w-6 h-6 rounded-full border-2 transition-all',
                ...
                {nsfwMode && (outfitTab === 'romance' || outfitTab === '18+') && (
                  <div className="border-t border-rose-500/20 pt-3">
                    <button onClick={() => setShowNsfwAdvanced(!showNsfwAdvanced)}
                      className="flex items-center gap-2 text-[10px] text-rose-400 font-bold uppercase tracking-wider w-full"
                    >
                    ...
                    {showNsfwAdvanced && (
                      <div className="mt-3 space-y-4 animate-fadeSlideIn">
                        {/* Nudity / Exposure Level */}
                        <div>
                          <MiniLabel>Nudity / Exposure Level</MiniLabel>
                          <div className="grid grid-cols-2 gap-1.5">
                            {NUDITY_LEVELS.map((nl) => (
                              <button key={nl.id} onClick={() => setNudityLevel(nudityLevel === nl.id ? null : nl.id)}
                                className={[
                        ...
                        <div>
                          ...
                          <input type="range" min={0} max={10} value={explicitnessIntensity}
                            onChange={(e) => setExplicitnessIntensity(Number(e.target.value))}
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-3 gap-1.5">
                            {SENSUAL_POSES.map((sp) => (
                              <button key={sp.id} onClick={() => setSensualPose(sensualPose === sp.id ? null : sp.id)}
                                className={[
                        ...
                        <div>
                          ...
                          <div className="flex gap-1.5">
                            {POWER_DYNAMICS.map((pd) => (
                              <button key={pd.id} onClick={() => setPowerDynamic(powerDynamic === pd.id ? null : pd.id)}
                                className={[
                        ...
                        <div>
                          ...
                          <div className="flex gap-1.5">
                            {FANTASY_TONES.map((ft) => (
                              <button key={ft.id} onClick={() => setFantasyTone(fantasyTone === ft.id ? null : ft.id)}
                                className={[
                                  'flex-1 px-2 py-2 rounded-lg text-[10px] font-medium border transition-all text-center',
                                  fantasyTone === ft.id
                                    ? 'border-rose-500/40 bg-rose-500/15 text-rose-200'
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-1.5">
                            {SCENE_SETTINGS.map((ss) => (
                              <button key={ss.id} onClick={() => setSceneSetting(sceneSetting === ss.id ? null : ss.id)}
                                className={[
                                  'px-2.5 py-2 rounded-lg text-[10px] font-medium border transition-all text-center',
                                  sceneSetting === ss.id
                                    ? 'border-rose-500/40 bg-rose-500/15 text-rose-200'
                  ...
                  <div className={[
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.value.trim()) setSelectedOutfitPreset(null)
                      }}
                ...
                <div>
                  <div className="flex items-center gap-3">
                    <MiniLabel className="mb-0">Qty:</MiniLabel>
                    <div className="flex gap-1">
                      {[1, 4, 8].map((n) => (
                        <button key={n} onClick={() => setOutfitCount(n)}
                          className={[
                            'w-8 h-8 rounded-lg text-xs font-medium border flex items-center justify-center transition-all',
                            outfitCount === n
              ...
              <>
                ...
                <div className="rounded-xl border border-purple-500/20 bg-purple-500/5 p-4 space-y-3">
                  <div className="text-xs font-semibold text-purple-300/80 uppercase tracking-wider flex items-center gap-2">
                    <CheckCircle2 size={14} className="text-purple-400" />
                    Avatar Created
                  </div>
                  {/* Avatar thumbnail */}
                  {(() => {
                    const outfitResult = outfitGen.result?.results?.length && selectedOutfitIndex !== null
                      ? outfitGen.result.results[selectedOutfitIndex] : null
                    const displayAnchor = outfitResult
                      ? { url: outfitResult.url, label: selectedOutfitPreset ? (SCENARIO_TAG_META.find((t) => t.id === selectedOutfitPreset)?.label || 'Custom') : 'Custom' }
                      : savedBody
                        ? { url: savedBody.url, label: 'Body base' }
                        : savedIdentity
                          ? { url: savedIdentity.url, label: 'Face identity' }
                          : null
                    return displayAnchor ? (
                      <div className="flex items-center gap-3">
                        <div className="w-14 h-14 rounded-xl overflow-hidden border border-purple-500/25 flex-shrink-0">
                          <img
                            src={resolveUrlFn(displayAnchor.url)}
                            alt="Avatar"
                ...
                {/* Character name removed — avatars don't get names in the wizard */}
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1.5 text-[11px]">
                    <div className="flex justify-between">
                      <span className="text-white/35">Gender</span>
                      <span className="text-white/60 capitalize">{selectedGender || 'Not set'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/35">Style</span>
                      <span className="text-white/60">
                        {selectedStyle
                          ? CHARACTER_STYLE_PRESETS.find((s) => s.id === selectedStyle)?.label || selectedStyle
                          : 'Default'}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/35">Profession</span>
                      <span className="text-white/60">
                        {selectedProfession
                          ? PROFESSIONS.find((p) => p.id === selectedProfession)?.label || 'Custom'
                          : 'Not set'}
                      </span>
                    </div>
                    {needsBodyStep && (
                      <div className="flex justify-between">
                        <span className="text-white/35">Body</span>
                        <span className="text-white/60 capitalize">{bodyType} / {bodyPosture}</span>
                      </div>
                    )}
                    <div className="flex justify-between">
                      <span className="text-white/35">Outfit</span>
                      <span className="text-white/60">
                        {outfitGen.result?.results?.length
                          ? (selectedOutfitPreset
                              ? SCENARIO_TAG_META.find((t) => t.id === selectedOutfitPreset)?.label
                              : customOutfitPrompt || 'Custom')
                          : 'None (optional)'}
                      </span>
                    </div>
                  </div>
                ...
                <div className="border-t border-white/[0.06] pt-4 space-y-2">
                  ...
                  <p className="text-[11px] text-white/30">
                    Save your character, then open the Character Sheet to generate more outfits and explore variations.
                  </p>
                </div>
              </>
            ) : null}
      ...
      <div className="flex-shrink-0 border-t border-white/[0.06] px-5 py-3 flex items-center justify-between">
        {phase === 'identity' ? (
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {gen.loading ? (
                  ...
                  <><Sparkles size={16} /> Generate Faces ({faceCount})</>
                )}
              </button>

              {/* Save Identity (only when face selected) */}
              {gen.result?.results?.length && selectedFaceIndex !== null ? (
                <button
                  onClick={handleSaveIdentity}
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold bg-gradient-to-r from-green-600 to-emerald-600 text-white shadow-lg shadow-green-500/20 hover:shadow-green-500/30 hover:brightness-110 active:scale-[0.98] transition-all"
                >
          ...
          <>
            <button
              onClick={handleBackToIdentity}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-white/50 hover:text-white/70 hover:bg-white/[0.04] transition-all"
            >
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {bodyGen.loading ? (
                  ...
                  <><User size={16} /> {bodyFraming === 'headshot' ? 'Generate Half Body' : 'Generate Body'} ({bodyCount})</>
                )}
              </button>

              {bodyGen.result?.results?.length && selectedBodyIndex !== null ? (
                <button
          ...
          <>
            <button
              onClick={needsBodyStep ? handleBackToBody : handleBackToIdentity}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-white/50 hover:text-white/70 hover:bg-white/[0.04] transition-all"
            >
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                disabled={outfitGen.loading || !effectiveOutfitPrompt}
                className={[
                  'flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all',
              ...
              >
                {outfitGen.loading ? (
                  ...
                  <><Shirt size={16} /> Generate Outfit ({outfitCount})</>
                )}
              </button>

              {/* Continue to Finalize */}
              {outfitGen.result?.results?.length && selectedOutfitIndex !== null ? (
                <button
              ...
              {outfitGen.loading && (
                <button onClick={outfitGen.cancel}
                  className="flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-sm font-medium border border-red-500/30 bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all"
                >
                  <X size={14} /> Cancel
                </button>
          ...
          <>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setPhase(savedBody ? 'body' : 'identity')}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-white/50 hover:text-white/70 hover:bg-white/[0.04] transition-all"
              >
              ...
              {(savedBody || savedIdentity) && (
                <button
                  onClick={() => setPhase('outfit')}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium border border-cyan-500/25 bg-cyan-500/10 text-cyan-300 hover:bg-cyan-500/20 transition-all"
                >
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <CheckCircle2 size={16} /> Save Avatar
              </button>
            </div>
          </>
        ) : null}
...
function MiniLabel({ children, className = '' }: { children: React.ReactNode; className?: string }) {
...
function OptionPill({ label, active, onClick, accent = 'purple' }: {
  label: string; active: boolean; onClick: () => void; accent?: 'purple' | 'rose' | 'cyan'
}) {
  const colors = {
...
function SkinSwatch({ tone, active, onClick, label }: {
  tone: string; active: boolean; onClick: () => void; label: string
}) {
  return (
    <button onClick={onClick} title={label}
      ...
      style={{ backgroundColor: SKIN_TONE_COLORS[tone] || '#888' }}
    />
  )
}
