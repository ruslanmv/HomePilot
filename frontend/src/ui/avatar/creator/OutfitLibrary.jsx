...
// Component
// ---------------------------------------------------------------------------
export function OutfitLibrary({ items, activeIdentityId, activeOutfitUrl, onSelectOutfit, onDeleteOutfit, resolveUrl, }) {
    ...
    const outfits = activeIdentityId
        ? items.filter((i) => i.parentId === activeIdentityId && i.scenarioTag)
        : [];
    if (!activeIdentityId)
        return null;
    return (<div className="space-y-2">
      ...
      {outfits.length > 0 ? (<div className="flex flex-wrap gap-1.5">
          {outfits.map((outfit) => {
                ...
                return (<div key={outfit.id} className="relative group">
                <button onClick={() => onSelectOutfit?.(outfit)} className={[
                    ...
                    ].join(' ')} title={meta ? `${meta.icon} ${meta.label}` : 'Outfit'}>
                  <img src={resolveUrl(outfit.url)} alt={meta?.label || 'Outfit'} className="w-full h-full object-cover" loading="lazy"/>
                ...
                {onDeleteOutfit && (<button onClick={(e) => {
                            e.stopPropagation();
                            onDeleteOutfit(outfit);
                        }} className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-red-500/80 hover:bg-red-500 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-sm" title="Delete outfit">
                    <Trash2 size={8} className="text-white"/>
                  </button>)}
              </div>);
            })}
        </div>) : (<p className="text-[10px] text-white/15 text-center py-1.5">
