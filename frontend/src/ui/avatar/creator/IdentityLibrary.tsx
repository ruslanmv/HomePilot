...
export interface IdentityLibraryProps {
  /** Full gallery items (used for filtering). */
  items: GalleryItem[]
  /** IDs of identities created during this wizard session. */
  sessionIds: Set<string>
  /** IDs of identities imported from the library. */
  importedIds: Set<string>
  /** Currently active identity id. */
  activeIdentityId: string | null
  onSelectIdentity: (item: GalleryItem) => void
  onNewIdentity: () => void
  /** Callback when user imports an identity from the library picker. */
  onImportIdentity: (item: GalleryItem) => void
...
export function IdentityLibrary({
  ...
  const allIdentities = items.filter(
    (i) => i.role === 'anchor' && !i.parentId,
  )
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {sessionIdentities.slice(0, 9).map((identity) => {
          ...
          return (
            <button
              ...
              title={identity.wizardMeta?.professionLabel || `Seed ${identity.seed ?? '?'}`}
            >
              <img
                src={resolveUrl(identity.url)}
                alt={identity.wizardMeta?.professionLabel || 'Identity'}
              ...
              {/* Active indicator */}
              {active && (
                <div className="absolute top-0.5 right-0.5 w-2 h-2 rounded-full bg-purple-400 shadow-[0_0_4px_rgba(168,85,247,0.6)]" />
              )}
            </button>
          )
        })}
        ...
        <button
          ...
          className="aspect-square rounded-lg border-2 border-dashed border-white/[0.08] hover:border-white/15 flex items-center justify-center transition-colors group"
      ...
      {libraryIdentities.length > 0 && (
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 rounded-lg text-[9px] font-medium text-cyan-400/60 hover:text-cyan-300/80 hover:bg-cyan-500/[0.06] border border-cyan-500/10 hover:border-cyan-500/20 transition-all"
        >
          <Download size={10} />
      ...
      {showPicker && (
        <div className="rounded-xl border border-white/[0.08] bg-black/60 backdrop-blur-sm p-2 space-y-2">
          <div className="flex items-center justify-between">
          ...
          <div className="grid grid-cols-3 gap-1.5 max-h-[120px] overflow-y-auto scrollbar-hide">
            {libraryIdentities.map((identity) => (
              <button
                ...
                className="relative aspect-square rounded-lg overflow-hidden border-2 border-white/[0.06] hover:border-cyan-500/40 transition-all group"
                title={identity.wizardMeta?.professionLabel || `Seed ${identity.seed ?? '?'}`}
              >
                <img
                  src={resolveUrl(identity.url)}
