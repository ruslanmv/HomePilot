...
export interface CharacterWizardProps {
  ...
  onSaveGeneration?: (anchor: AvatarResult, portraits: AvatarResult[], mode: AvatarMode, prompt?: string, referenceUrl?: string, nsfw?: boolean) => void
}
...
export function CharacterWizard({ backendUrl, apiKey, globalModelImages, onClose, onSaveGeneration }: CharacterWizardProps) {
  ...
  const randomizeAppearance = useCallback(() => {
    ...
    setDraft((prev) => ({
      ...
      outfitStyle: pick(OUTFIT_STYLES_SFW).id,
  ...
  const handleCreateAvatar = useCallback(() => {
    ...
    if (onSaveGeneration) {
      onSaveGeneration(chosen, portraits, draft.generationMode as AvatarMode, prompt, draft.referenceUrl, isSpicy)
    } else {
      gallery.addAnchorWithPortraits(chosen, portraits, draft.generationMode as AvatarMode, prompt, draft.referenceUrl, { nsfw: isSpicy || undefined })
    }
    ...
    return (
      <div className="max-w-md mx-auto space-y-6">
        ...
        <div className="grid grid-cols-4 gap-1.5">
          {CHARACTER_STYLE_PRESETS.filter((s) => s.category === 'standard').slice(0, 4).map((s) => (
            <button key={s.id}
              onClick={() => update({ outfitStyle: s.id })}
              className={[
