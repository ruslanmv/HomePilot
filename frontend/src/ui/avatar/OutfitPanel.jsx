...
import { Shirt, X, Wand2, Loader2, AlertTriangle, Download, PenLine, Maximize2, Copy, Check, } from 'lucide-react';
...
// Component
// ---------------------------------------------------------------------------
export function OutfitPanel({ anchor, backendUrl, apiKey, nsfwMode, checkpointOverride, onResults, onSendToEdit, onOpenLightbox, onClose, }) {
    ...
    const presets = OUTFIT_PRESETS.filter((p) => p.category === 'sfw' || nsfwMode);
    ...
    const handleGenerate = useCallback(async () => {
        ...
        const framingType = anchor.framingType || 'half_body';
    ...
    const handleCopySeed = useCallback((seed) => {
        navigator.clipboard.writeText(String(seed)).catch(() => { });
        setCopiedSeed(seed);
        setTimeout(() => setCopiedSeed(null), 1500);
    }, []);
    return (<div className="mt-6 pt-5 border-t border-cyan-500/20 animate-fadeIn">
      {/* Header */}
      ...
      <div className="flex items-center gap-3 mb-4 px-3 py-2.5 rounded-xl bg-white/[0.03] border border-white/8">
        <div className="w-14 h-14 rounded-lg overflow-hidden border border-white/10 flex-shrink-0">
          <img src={anchorUrl} alt="Identity anchor" className="w-full h-full object-cover"/>
        </div>
      ...
      <div className="mb-3 space-y-3">
                ...
                {sfwPresets.map((p) => {
                    ...
                    return (<button key={p.id} onClick={() => {
                            ...
                            if (selectedPreset !== p.id)
                                setCustomPrompt('');
                        }} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-all ${selectedPreset === p.id
                            ? 'border-cyan-500/40 bg-cyan-500/15 text-cyan-300'
                            : 'border-white/8 bg-white/[0.03] text-white/50 hover:bg-white/5 hover:text-white/70'}`} title={p.prompt}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {romancePresets.map((p) => {
                    ...
                    return (<button key={p.id} onClick={() => {
                            ...
                            : 'border-white/8 bg-white/[0.03] text-white/50 hover:bg-white/5 hover:text-rose-300/70'}`} title={p.prompt}>
                      {tagMeta && <span className="text-sm leading-none">{tagMeta.icon}</span>}
                      {p.label}
                    </button>);
                })}
              ...
              <div className="flex flex-wrap gap-1.5">
                {explicitPresets.map((p) => {
                    ...
                    return (<button key={p.id} onClick={() => {
                            ...
                            : 'border-white/8 bg-white/[0.03] text-white/50 hover:bg-white/5 hover:text-red-300/70'}`} title={p.prompt}>
                      {tagMeta && <span className="text-sm leading-none">{tagMeta.icon}</span>}
                      {p.label}
                    </button>);
                })}
        ...
        <button onClick={() => {
            ...
            setCustomPrompt('');
        }} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-all ${!selectedPreset && !customPrompt.trim()
            ? 'border-cyan-500/40 bg-cyan-500/15 text-cyan-300'
      ...
      <div className="mb-4">
        ...
        <input value={customPrompt} onChange={(e) => {
            ...
            if (e.target.value.trim())
                setSelectedPreset(null);
        }} placeholder="e.g. red cocktail dress, rooftop bar, golden hour" className="w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm placeholder:text-white/25 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20 transition-all" onKeyDown={(e) => {
      ...
      <div className="flex items-center gap-3 mb-4">
        <button onClick={handleGenerate} disabled={!canGenerate} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${canGenerate
        ...
        <div className="flex items-center gap-1">
          {[1, 4, 8].map((n) => (<button key={n} onClick={() => setCount(n)} className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all border ${count === n
      ...
      {/* Error */}
      {outfit.error && (<div className="flex items-center gap-2 px-3 py-2 mb-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-xs">
          <AlertTriangle size={14}/>
          <span>{outfit.error}</span>
        </div>)}

      {/* Warnings */}
      {outfit.warnings.length > 0 && (<div className="flex items-start gap-2 px-3 py-2 mb-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs">
          <AlertTriangle size={14} className="mt-0.5 shrink-0"/>
          <div>
            {outfit.warnings.map((w, i) => (<div key={i}>{w}</div>))}
          </div>
        </div>)}

      {/* Loading skeleton */}
      {outfit.loading && outfit.results.length === 0 && (<AvatarGeneratingLoader label="Generating outfits…" hint="Creating outfit variations for your character" count={count}/>)}

      {/* Results grid */}
      {outfit.results.length > 0 && (<div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {outfit.results.map((item, i) => {
                ...
                return (<div key={i} className="group relative rounded-xl overflow-hidden border border-white/8 bg-white/[0.02] hover:border-white/15 transition-all">
                  <div className="aspect-[2/3] bg-white/[0.03] cursor-pointer relative" onClick={() => onOpenLightbox?.(imgUrl)}>
                    <img src={imgUrl} alt={`Outfit variation ${i + 1}`} className="w-full h-full object-cover" loading="lazy"/>
                    {/* Hover overlay */}
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center gap-2">
                      {onOpenLightbox && (<button onClick={(e) => { e.stopPropagation(); onOpenLightbox(imgUrl); }} className="p-2 bg-white/10 backdrop-blur-md rounded-lg text-white hover:bg-white/20 transition-colors" title="View full size">
                          <Maximize2 size={16}/>
                        </button>)}
                      {onSendToEdit && (<button onClick={(e) => { e.stopPropagation(); onSendToEdit(imgUrl); }} className="p-2 bg-purple-500/30 backdrop-blur-md rounded-lg text-purple-200 hover:bg-purple-500/50 transition-colors" title="Open in Edit">
                          <PenLine size={16}/>
                        </button>)}
                      <button onClick={(e) => {
                        e.stopPropagation();
                  ...
                  <div className="px-2.5 py-2">
                    <button onClick={() => item.seed !== undefined && handleCopySeed(item.seed)} className="text-[11px] text-white/40 font-mono hover:text-white/70 transition-colors cursor-pointer">
                      {copiedSeed === item.seed ? (<span className="flex items-center gap-1 text-green-400">
