...
import {
  previewPersonaPackage,
  importPersonaPackage,
  importPersonaAtomic,
  exportPersona,
  resolvePersonaDeps,
  installPersonaDeps,
} from './personaPortability'
...
function DependencyRow({ item }: { item: DependencyItem }) {
  return (
    <div className="flex items-center gap-3 py-2 px-3 rounded-lg bg-white/[0.03] border border-white/[0.06]">
      ...
      {item.source_type === 'community_bundle' && (
  ...
  return (
    <div className={`p-4 rounded-xl border transition-all ${
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        {status.source_type && (
          <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-white/40 shrink-0">
            {status.source_type}
          </span>
        )}
      </div>
      ...
      <div className={`text-xs leading-relaxed ${
        isFailed ? 'text-red-300' : isComplete ? 'text-emerald-300/70' : 'text-white/50'
      }`}>
      ...
      {/* Tools discovered */}
      {isComplete && status.tools_discovered > 0 && (
...
function StepBreadcrumb({ step, totalSteps }: { step: number; totalSteps: number }) {
      ...
      <div className="w-full max-w-2xl bg-[#1a1a2e] rounded-2xl border border-white/10 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
          ...
          {step === STEP_MCP && (
            <div className="space-y-5">
              ...
              {!mcpInstalling && !mcpResult && mcpPlan && (
                <div className="space-y-3">
                  {/* Already available */}
                  {mcpPlan.servers_already_available.length > 0 && (
                    <div>
                      <div className="text-xs font-semibold text-emerald-300/70 mb-2">Available</div>
                      {mcpPlan.servers_already_available.map((s: any, i: number) => (
                        <div key={i} className="flex items-center gap-2 py-1.5 px-3 text-xs text-white/50">
                  ...
                  {/* To install */}
                  {mcpPlan.servers_to_install.length > 0 && (
                    <div>
                  ...
                  <div className="flex justify-center pt-2">
                    <button
                      ...
                      className="flex items-center gap-2 px-6 py-2.5 bg-purple-500 hover:bg-purple-600 text-white text-sm font-semibold rounded-full transition-colors"
                    >
                      <Download size={16} />
          ...
          {step === STEP_COMPLETE && importedProject && (
        ...
        <div className="px-6 py-4 border-t border-white/10 bg-[#1a1a2e] flex justify-between items-center">
          ...
          {step === STEP_PREVIEW && (
            <>
              <button
                onClick={() => { setStep(STEP_UPLOAD); setFile(null); setPreview(null); setError(null) }}
          ...
          {step === STEP_MCP && (
            <>
              <button
                onClick={() => { setStep(STEP_PREVIEW); setMcpPlan(null); setMcpResult(null); setError(null) }}
                disabled={mcpInstalling}
                className="flex items-center gap-1 px-4 py-2 text-sm font-medium text-white/50 hover:text-white transition-colors disabled:opacity-30"
              >
                <ChevronLeft size={16} />
                Back
              </button>
              {mcpResult?.all_satisfied && !importedProject && (
                <button
                  onClick={doImport}
                  disabled={importing}
                  className="flex items-center gap-2 px-6 py-2.5 bg-emerald-500 hover:bg-emerald-600 disabled:opacity-40 text-white text-sm font-semibold rounded-full transition-colors"
                >
                  {importing ? (
                    <>
                      <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
              ...
              {mcpResult && !mcpResult.all_satisfied && (
                <button
                  onClick={doImport}
                  disabled={importing}
                  className="flex items-center gap-2 px-6 py-2.5 bg-amber-500 hover:bg-amber-600 disabled:opacity-40 text-white text-sm font-semibold rounded-full transition-colors"
                >
                  <SkipForward size={16} />
