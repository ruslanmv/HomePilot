...
| `vectordb` is project-scoped end to end | `vectordb.py:58, 111, 155, 178` | ✅ MS15 added `namespace`, default `"project"`, byte-identical name; `collection_name()` is now the single place the name is built |
| `hp-teams` (9106) is registered in Forge but has no implementation; `microsoft_graph_server.py` exists | `seed_all.py:67`, `ls agentic/integrations/mcp/` | ✅ MS22 **deferred** tier 2 and made the deferral visible: the entry declares an external source, so a local build would be a second implementation behind one id |
...
| **W6** | Together | MS18 ✅ → MS19 ✅ → MS20 ✅ | Live grounded chat, meeting as 8th 👥 activity, card on avatar surface. Carried: **matrix rows 11–16** |
| **W7** | Capability | MS21 ✅ → MS22 ✅ | MeetingSense MCP server (9107) + Forge; Teams tier 2 **explicitly deferred**. Carried: **matrix rows 11–16**, and **¹ `make test-mcp-servers` is red for a pre-existing reason** |
...
| **W11** | The mount | MS29 | The integration batch the plan never had: one provider, one button, and a persona that knows your screen is shared |
| **W12** | Chat first | MS32 | The composer goes back to being a composer: Meeting moves to the header, setup copy becomes contextual, the env-var hint moves to Settings |
| **W13** | The ended meeting | MS33 | The payoff wave: summary, decisions, actions and Ask replace the transcript as the ended card; every server capability that had no door gets one |
...
| # | Batch | Scope | Acceptance | Status |
|---|---|---|---|---|
| **MS15** | Embeddings + retrieval | `vectordb.py` gains `namespace: str = "project"` — every existing call resolves to the same collection name; meetings use `namespace="meeting"` so `get_project_document_count` / `delete_project_knowledge` never see them. On stop, embed segments + captions into per-meeting and global `meetings` collections; internal `ms.search`; MS13 uses it beyond the 90 s window | 37 tests, 16 mutations each fail. Existing project collection names asserted against a **fixed hash** rather than the expression that builds them; cross-meeting query cites `Vendor review · 00:02:00`. **One collection filtered by `meeting_id`, not one per meeting** — both queries a per-meeting collection would serve are the global one with and without a filter, and the delete has to run filtered either way. **Delete now clears three stores.** Two weak tests found by mutation: an over-fetch and a time-order sort both survived because the fixture's scores agreed with the property | ✅ |
| **MS16** | Binding + resume | `ms_threads`, `ms_artifacts`; frozen card hydrates on reopening; "New thread from this meeting" → new conversation with a brief + attached `meeting_id`; "Attach to project" pushes `transcript_md` + captions through the existing project upload path (this is the only route by which a meeting reaches project jobs, and it needs no new job type) | 33 tests, 15 mutations each fail. The origin thread is recorded on **start**, not stop, so a meeting interrupted by a restart still hydrates. The conversation route is declared **above** `/{meeting_id}` — a path parameter first would swallow "conversations" and 404 every hydration. Delete clears both new tables. One hole found by mutation: a section whose items all render blank still printed its heading — the same hole MS14's note sections had | ✅ |
| **MS17** ∥ | Auto-metadata | Calendar match via `google_calendar` / `microsoft_graph` MCP → title, attendees, link; source detection from the shared window title (Teams / Zoom / Meet / Webex) | 72 tests, 14 mutations each fail. **Two real bugs the tests found:** markers matched as substrings read "Cisco Webex Meetings" — and any shared document called "Meeting notes" — as a Meet call, and a regex counts `_` as a word character so "Webex_Meetings" matched nothing. Calendar match against an injected invoker; a live connection is **matrix row 16** | ✅ |

### W6 — Together

| # | Batch | Scope | Acceptance | Status |
|---|---|---|---|---|
| **MS18** | Live context provider | Optional hook in `backend/app/personalities/prompt_builder.py`: when a conversation has a live `meeting_id`, prepend `[LIVE MEETING CONTEXT]` — D9 tiers 1+2 only: last 90 s verbatim, current slide caption, notes, recap — **hard-capped at 900 tokens, truncating verbatim first and never the recap**; behind `_TOGETHER`. `limit=6` in `main.py` is not touched | pytest: prompt byte-identical when no live meeting or flag off; block present otherwise; **a 3-hour fixture stays ≤ 900 tokens and still contains the recap**; the transcript body never appears in the prompt | 26 tests, 13 mutations each fail. Two weak tests found: the budget was asserted against the module's own constant (so raising it passed), and nothing covered the orchestrator seam — now checked at the source, which is the right weight for a one-keyword claim. **`limit=6` in `main.py` is untouched** | ✅ |
| **MS19** | The eighth activity | `meeting.js` joins the 👥 launcher. **Borrows B11's consent machine and `CapturePipeline`** rather than calling `getDisplayMedia` — asks for screen + mic after the choice, never before; revocation stops capture within a frame | 21 jest tests in `3D-Avatar-Chatbot` + 6 vitest here, 14 mutations each fail. `meeting` registered as a **compound consent source** (screen then mic); the addon grew `startWithStreams` so the launcher's one-call-site rule holds. Revoke asserted **with no timers**. Two pre-existing tests updated honestly: `capture.test.js` pinned SOURCES exactly, `composition.test.js` caught the unloaded global. A real bug found: `stop()` released the grant after revoking, so its own listener announced a second stop | ✅ |
| **MS20** | Card on the avatar surface | Meeting card rendered through the existing `display` panel as a `cards` kind — one data source, two renderers. Panels are capped (64 KB, row-limited) so the live card sends a **summary projection**, not the transcript | 21 tests, 15 mutations each fail. A 400-segment, 40-slide meeting validates and fits 64 KB in ≤ 12 rows. The cap is **read from `panels.MAX_ROWS`**, not retyped; cards truncate from the end so the header is dropped last; a refused panel is dropped rather than escalated into a live session. Two mutations first landed on identical guard lines in *other* bridge methods — the same first-occurrence trap MS19 hit | ✅ |

### W7 — Capability

| # | Batch | Scope | Acceptance | Status |
|---|---|---|---|---|
| **MS21** | MCP server | `agentic/integrations/mcp/meetingsense_server.py` on **9107** (D3) with `ms.list_meetings get_meeting get_transcript search get_live_context get_slide update_action suggest set_mode export`; write tools policy-gated; added to `docker-compose.mcp.yml`, Makefile start/stop/health | 48 tests, 14 mutations each fail. **Transport changed from the row's implied in-process import to HTTP**: the MCP image ships `agentic/` and no `backend/`, so an import would work from the Makefile and fail in the container. Four backend routes added (`meetings`, `search`, live block, notes amendment). Tests run the **real router** behind an ASGI transport, so a tool naming a route the backend does not serve fails here. The port test reads the Makefile's own port lists. **Tools are `hp.ms.*`, not `ms.*`** — `test_agentic_health.py` enforces an `hp.` prefix on every Forge tool, and a tested repo invariant beats this row's shorthand | ✅ |
| **MS22** | Forge + Teams | Register in `seed_all.py`, gateway/virtual-server YAML, suite manifests; Chief-of-Staff A2A gets a "this week's meetings" example. **`hp-teams` (9106) is registered but unbuilt**: build the thin server for tier-2 "post to meeting chat" (template: `microsoft_graph_server.py`) **or** mark tier 2 ⏸ and make the UI say "unavailable" rather than fail at click time | 18 tests, nine mutations each fail. Catalog lists 9107 in all four registration points; **tier 2 marked ⏸** — the second option the row offered, because the `hp-teams` entry already declares an *external* source and a local build would put two implementations behind one id. Tier-2 state is visible because `ServerDef` now carries `availability`/`unavailable_reason`, the API always returns them, and `install()` refuses with the reason instead of timing out on a missing module. **`make test-mcp-servers` is NOT green and was not made green**: `tests/test_mcp_servers.py` is entirely `async def` with no asyncio plugin and fails 164/164 on a clean tree — one of the 18 pre-existing failures, out of this batch's scope, and reported rather than worked around | ✅ ¹ |

### W8 — Engine

| # | Batch | Scope | Acceptance | Status |
|---|---|---|---|---|
| **MS23** | LangGraph graph | `meetingsense/agent/{graph,state,modes}.py` + `nodes/` (perceive reflect decide answer coach act recall deliver); `Reflect` wraps MS12, `Recall` wraps MS15, `Act` uses `agentic/runtime_tool_router.py`; behind `_AGENT`, fixed loop when off | 41 tests, 19 mutations each fail. **The acceptance is met and is the headline test**: the fixed loop and the graph are driven over the same recorded events with the same stubbed engine and compared frame for frame, *and* on how they drive the engine (`run` count, `force` flags). A third test reads `session._maybe_notes`, so the copied loop cannot drift. **Two schedulers, one set of behaviour**: langgraph is not installed here, so the topology is data and a 20-line walker also executes it — `langgraph_personas` imports it at module scope and its whole suite is one of the 18 uncollectable ones. **A real gap the tests found**: a `slide` event routed to `answer` with nothing to answer; the caption is now the question, and an uncaptioned slide plans nothing. Four mutation survivors were all unreachable guards, now tested by calling the nodes directly | ✅ |
| **MS24** | Sub-agents + policy | `meetingsense/agent/subagents.py` — `SlideReader` (caption → title/claim/topics, a re-shown slide is a *return*), `ActionExtractor` (window → proposed owners and deadlines, capped); neither writes, both propose and `reflect` folds them through MS12's `merge`. Per-meeting tool pre-approval in `ms_artifacts`, additive, revoke recorded not deleted. `hp.ms.set_mode` enforced server-side: `resolve_mode` reads the store every turn and a per-turn `mode` is only a default | 45 tests, 42 mutations each fail. **Acceptance met**: an unapproved `Act` call is refused *and recorded in `errors`* (a refusal nobody can see is one nobody can approve), an approved one runs, and a client cannot compose a mode — an unknown name resolves down to the floor and a stored mode beats a client-supplied one, with the conflict reported. **Two gates, kept apart**: `approved_tools=None` approves nothing, and an unreadable policy store lands on the floor, never the ceiling. **Nine mutation survivors, all real**: the sharpest were a re-approval test asserting on a computed return value rather than reading back, and an unreadable-store test that `perceive`'s outer belt satisfied without the resolver deciding anything. **One real defect**: `_merge_actions` imported `_key` two dots up instead of three, so every extraction raised and was swallowed by the "never worth the notes" guard | ✅ |

### W9 — Modes & voice

| # | Batch | Scope | Acceptance | Status |
|---|---|---|---|---|
| **MS25** | Chips | `meetingsense/chips.py` — five deterministic triggers, no model; `chip`/`chip_result` frames and a `chip_action` frame back; `ChipRow.tsx` on the card; `accept()` runs a proposal through `agentic/runtime_tool_router.py` inside MS24's per-meeting approval. Behind `MEETINGSENSE_MODES` | 86 pytest + 31 vitest, 44 + 34 mutations each fail. **Acceptance met**: vitest covers render, dismiss (local, not a deletion) and that *rendering runs nothing*; the must-not-fire fixtures are one parametrised list — `monday.com`, `example.com/2026-04-20/notes`, "so we're going with the second option?", "so Ana will send the terms?", "does that make sense?", `report.pdf`. **An id crosses the wire, never a chip**, so the page cannot rewrite what the user agreed to. **Two survivors were unreachable code rather than untested behaviour**: a broad `try` inside `detect` swallowed the junk-segment guard and would have swallowed a broken trigger (removed — detection is pure), and a renderability check inline in a closure could not be called (now `usable()`) | ✅ |
| **MS26** | Participant + Presenter | `agent/mode_prompts.py` (framing layered *above* `ASK_SYSTEM`, never replacing it), `agent/participant.py` (addressed-by-name → answer; aimed-at-user → draft), `agent/presenter.py` (attached deck, windowed pacing, audience queue). Trigger sets and two new permissions — `addressed`, `queues` — are **columns on `modes.py`, not branches in the graph** | 91 tests, 55 mutations each fail. **Acceptance met**: `test_modes_e2e.py` replays one recorded window through each mode and asserts they disagree — the named question is ignored / answered aloud / queued, and the question aimed at the user is ignored / chipped with a draft / queued. **Three real defects, all found by those fixtures**: `chips.frame` dropped `draft`, so drafting worked and delivered nothing; a question naming the assistant was answered *and* chipped; and pacing compared against a section's end, making the first minute of a long section read as "eight minutes ahead". **Twenty-four survivors taught the shape**: an e2e suite passes while the whole disagrees somewhere, so `test_modes_units.py` pins each rule alone. **MS25 narrowed**: the `question` chip is Participant's and above; Note-taker offers no shoulder tap | ✅ |
| **MS27** | Coach + Practice + voice | `agent/coaching.py` (prep material is the only source; `POST/GET/DELETE /{id}/prep`), `agent/practice.py` (rehearsal brief → a `voice_call/` session; barge-in straight to their registry), `agent/voice_out.py` (tier-3 TTS via `get_tts_provider`, virtual-device detection, desktop-only refusal with a named reason), `VoiceSetup.tsx` wizard, mode badge on the pill, per-mode consent copy | 83 pytest + 23 vitest, 47 + 21 mutations each fail. **The OCR refusal is enforced three ways** — an allow-list in `context()`, a `scrub()` that drops rows carrying a slide's fingerprint, and a source test that fails if the module mentions a keyframe — and **verified by breaching it**: one added keyframe read trips three independent tests. Coach with no prep says nothing. Practice opens through `create_session` so the entitlement check is theirs, keeps no turn state of its own, and does not echo the resume token. Voice out never falls back to the speakers. **Eleven survivors**: seven were an untested prep lifecycle, two were assertions weaker than their claims (badge *inside* the live region vs *announced*; the browser refusal tested without `onCheck` wired), two were dead guards. **Manual matrix rows 17–21 added; 18–20 are unreachable by any test and the matrix is still unsigned** | ✅ |

### W11 — The mount

| # | Batch | Scope | Acceptance | Status |
|---|---|---|---|---|
| **MS29** | The mount | The integration layer the plan never had. `MeetingSenseProvider` (one component around the app shell; pill + card + consent hang off it), `MeetingButton` (one click starts, notes and slides on, MS5's popover behind a chevron), `homepilot-meetingsense.js` added to `index.html`, and `meetingsense/screen_context.py` + a guarded call in `prompt_builder.py` so a persona knows when a screen is being shared | 29 vitest + 33 pytest, 41 mutations each fail. **The finding**: W0–W10 mounted nothing — every component was a tested island, `window.hpMeetingSense` did not exist in the app, and MS5's `attach()` was never called. **Acceptance**: with nothing recording the provider contributes no node at all, asserted as `innerHTML` against the unwrapped app; one click starts; consent precedes capture and is remembered only if ticked; a blocked button names its cause. **Two real survivors**: `begin()` never checked the server flag though the context exposes it, and the outer render guard restated its own inner guards. **Two of my tests asserted behaviour the product deliberately lacks** — that Stop stops (MS6 made it a countdown) and that consent remembers itself unasked — and were corrected rather than the behaviour changed | ✅ |

### W13 — The ended meeting

| # | Batch | Scope | Acceptance | Status |
|---|---|---|---|---|
| **MS33** | The ended meeting | The audit's two ❌ rows, closed. `meetingRecord.ts` (pure: three-shape notes unwrap, item normalisation, duration/day labels, citation parsing, `segmentAt`), `useMeetingRecord.ts` (fetches `GET /{id}` when a meeting ends; bounded retry while the final notes window is still being flushed), `MeetingSummary.tsx` (Summary → Decisions → Actions → Open questions), `AskField.tsx` (`POST /{id}/ask`, answers with clickable stamps), `MeetingMenu.tsx` (the `•••`: rename, export, discuss in new chat, add to project, delete), `CapturePopover.tsx` (the chevron MS32 dropped). Backend gains `PATCH /v1/meetingsense/{id}` — `store.UPDATABLE` had allowed a rename since MS17 and nothing could reach it | 48 vitest + 7 pytest; 46 of 49 mutations fail (three equivalent: an `Array.isArray` guard the key-check subsumes, a `typeof end` guard the `isFinite` check subsumes, and an empty-`cited` early-out). **The finding**: five server capabilities — ask, thread, attach, delete, export — had existed for waves with no UI at all, and `MeetingDetail.tsx` was never mounted. The card showed a transcript, which is the recording rather than the value. **Acceptance**: the ended card leads with the summary and demotes the transcript behind a disclosure naming its line count; a citation opens the transcript at the line *already in progress*, never the next one; a stamp is a link only when the server's `cited` vouched for it. **Safe now, deliberate later**: rename and export and new-thread are immediate; attach requires an explicit project (D4); delete names what goes. **A real bug found by a test I wrote to fail**: `store.update_meeting` rejected `""` but not `"   "`, so a cleared rename box erased a title MS17's calendar had found — fixed at the store, where the rule holds for every caller. **Two redundant guards removed rather than tested**: a ref latching a collapse an effect's dependency array already latches, and an `ended &&` restating what the disclosure button guarantees. **Three survivors were one rule written twice** — a `disabled` attribute and a handler guard that could drift — collapsed to `canAttach` / `canRename` read by both | ✅ |

### W12 — Chat first

| # | Batch | Scope | Acceptance | Status |
|---|---|---|---|---|
| **MS32** | Chat first | Undoes MS29's placement without undoing MS29. `MeetingAction.tsx` — the header control beside Call, three states (36px icon / spinner / `● 08:42` pill) — plus `MeetingPanel.tsx` (`meetingBlock` + the setup and live popovers) and `MeetingTranscriptionCard.tsx` (Settings → Voice Assistant). `MeetingControls` gains `phase`, `phaseText`, `elapsedMs`, `micMuted`, `mute`, `undo`, `undoSecondsLeft`. Three edits in `App.tsx` (import, header mount, composer block removed) and one import + one line in `SettingsPanel.tsx`. `MeetingButton.tsx` is left in the tree, unedited and still tested — it is simply no longer mounted | 36 vitest, 30 of 32 mutations fail (two equivalent: a guard split in half, and a `live ? null :` short-circuit the render already subsumes). **The finding this batch exists for**: MS29 was right that a control people go looking for is pressed too late, and wrong about the price — under the composer it put an optional feature *and the server's `Set WHISPER_MODEL …` string* beneath the message box of every chat, forever. **Acceptance**: an unconfigured install looks exactly like a configured one until the control is pressed; no block's copy matches `/WHISPER_MODEL\|STT_BASE_URL\|_[A-Z]{2,}/`, checked across every status × conversation pair rather than only the one whose cause is the missing provider; a live meeting shows `08:42` in the button's `aria-label`, and the first click opens a panel rather than stopping a recording from a 36px target. **No Pause**: the recorder has mute and stop, and a "Pause" that means "mute the microphone" would tell somebody their meeting is paused while the room is still captured — asserted by a test that fails if the string appears. **Four real holes**, all in my own tests: a `settings: false` block was never pressed, so a Settings button for a problem Settings cannot fix went unnoticed; a live meeting whose conversation had gone away was untested; two `waitFor(() => expect(html).toBe(''))` assertions passed on their first tick, *before* the probe resolved, so they asserted "has not rendered yet" — which every component satisfies; and a `hint` alongside a working provider was never tried. **One guard removed rather than tested**: `if (starting) return` was the third copy of a rule `disabled` and the provider's `begin` already enforce | ✅ |
