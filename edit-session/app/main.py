...
async def set_active_image(
    ...
    store = get_store()
    rec = store.set_active(conversation_id, url)
    ...
    if instruction and instruction.strip():
        ...
        # Capture returned images into history
        images = _extract_images(out)
        for img in images:
            store.push_history(conversation_id, img)
...
async def select_image(conversation_id: str, req: SelectImageRequest):
    ...
    store = get_store()
    rec = store.set_active(conversation_id, req.image_url)

    return SetActiveImageResponse(
        conversation_id=conversation_id,
...
async def get_session(conversation_id: str):
    ...
    Returns the active image URL and history.
    """
    store = get_store()
    rec = store.get(conversation_id)
...
@app.get(
    ...
    description="Get the image history for an edit session."
)
async def get_history(conversation_id: str):
    ...
    in this session, most recent first.
    """
    store = get_store()
    rec = store.get(conversation_id)
...
async def revert_to_history(
    ...
    if index < 0 or index >= len(rec.history):
        raise HTTPException(
            status_code=400,
            detail=f"Invalid history index. Valid range: 0-{len(rec.history)-1}"
        )

    target_url = rec.history[index]
    rec = store.set_active(conversation_id, target_url)
