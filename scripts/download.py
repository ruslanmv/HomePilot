...
INSTALL_PATHS = {
    "image": COMFYUI_ROOT / "models" / "checkpoints",
    "video": COMFYUI_ROOT / "models" / "checkpoints",
}
...
def download_catalog_model(model_id: str, output_dir: Optional[str] = None) -> int:
    ...
    if not result:
        print(f"ERROR: Model '{model_id}' not found in catalog")
        return 1
    ...
    if not download_url:
        print(f"ERROR: No download URL for model '{model_id}'")
    ...
    if output_dir:
        ...
        install_path = model_data.get("install_path", "models/checkpoints/")
        if provider == "comfyui":
    ...
    print(f"\nDownloading: {model_data.get('label', model_id)}")
    print(f"Provider: {provider}")
    print(f"Type: {model_type}")
    print(f"Size: {model_data.get('size_gb', '?')} GB")
...
def download_civitai_model(
    ...
    print(f"\n🧪 EXPERIMENTAL: Downloading from Civitai (version {version_id})")
    ...
    print(f"Size: {best_file.get('sizeKB', 0) / 1024:.1f} MB")
    print(f"Destination: {dest_path}")
    print()
