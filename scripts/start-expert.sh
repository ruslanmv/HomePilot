...
BACKEND_PORT="${EXPERT_MODE_BACKEND_PORT:-18000}"
FRONTEND_PORT="${EXPERT_MODE_FRONTEND_PORT:-13000}"
EDIT_SESSION_PORT="${EXPERT_MODE_EDIT_SESSION_PORT:-18010}"
COMFY_PORT="${EXPERT_MODE_COMFY_PORT:-18188}"
MCP_GATEWAY_PORT="${EXPERT_MODE_MCP_GATEWAY_PORT:-14444}"
MCP_GATEWAY_HOST="${EXPERT_MODE_MCP_GATEWAY_HOST:-127.0.0.1}"
MCPGATEWAY_URL="http://${MCP_GATEWAY_HOST}:${MCP_GATEWAY_PORT}"
MCP_WEB_SEARCH_PORT="${EXPERT_MODE_MCP_WEB_SEARCH_PORT:-19151}"
MCP_DOC_RETRIEVAL_PORT="${EXPERT_MODE_MCP_DOC_RETRIEVAL_PORT:-19152}"
MCP_MEMORY_STORE_PORT="${EXPERT_MODE_MCP_MEMORY_STORE_PORT:-19153}"
MCP_SAFETY_POLICY_PORT="${EXPERT_MODE_MCP_SAFETY_POLICY_PORT:-19154}"
MCP_OBSERVABILITY_PORT="${EXPERT_MODE_MCP_OBSERVABILITY_PORT:-19155}"
...
echo "════════════════════════════════════════════════════════════════"
echo "Starting HomePilot Expert sandbox"
echo "Backend:      http://localhost:${BACKEND_PORT}"
echo "Edit-Session: http://localhost:${EDIT_SESSION_PORT}"
echo "Frontend:     http://localhost:${FRONTEND_PORT}"
echo "ComfyUI:      http://localhost:${COMFY_PORT} (expert port)"
echo "MCP Gateway:  ${MCPGATEWAY_URL} (expert port)"
echo "Expert MCP set (expert):"
...
if [[ -f "$ROOT/ComfyUI/main.py" && -x "$ROOT/ComfyUI/.venv/bin/python" ]]; then
  ...
  echo "⚠️  ComfyUI not found; expert will run without image/video generation backend."
fi

echo "Starting Expert MCP servers for expert..."
(
...
if [[ "${AGENTIC:-1}" == "1" ]] && ([[ -d "$ROOT/mcp-context-forge/.venv" ]] || command -v mcpgateway >/dev/null 2>&1); then
  echo "Starting expert MCP gateway for Expert orchestration..."
  (
    cd "$ROOT"
  ...
  echo "⚠️  MCP not installed; expert starts without MCP gateway (run: make install-expert)."
fi
